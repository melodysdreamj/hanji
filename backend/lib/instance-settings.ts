...
export const signupPolicyLabels: Record<SignupPolicy, string> = {
  public: 'anyone',
  closed: 'admin-provisioned only',
};

export function parseSignupPolicy(value: unknown, fallback: SignupPolicy = 'public'): SignupPolicy {
  if (typeof value !== 'string') return fallback;
  const policy = value.trim().toLowerCase();
  if (policy === 'public' || policy === 'closed') return policy;
  // Legacy restrictive policies (invite_only / verified_domains) predate the
  // move to server-provisioned accounts. Collapse them to 'closed' so stored
  // rows keep meaning "self-service signup is off" instead of throwing.
  if (policy === 'invite_only' || policy === 'verified_domains') return 'closed';
...
function normalizeInstanceSettings(row: InstanceSettings | null | undefined): InstanceSettings {
  ...
  return {
    ...
    masterEmail:
      typeof row?.masterEmail === 'string' && row.masterEmail.trim()
        ? row.masterEmail.trim().toLowerCase()
        : null,
...
export async function upsertInstanceSettings(
  ...
  if (patch.masterUserId === undefined) delete normalizedPatch.masterUserId;
  if (patch.masterEmail === undefined) {
    delete normalizedPatch.masterEmail;
