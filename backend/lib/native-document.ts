...
export const NATIVE_FORMAT = HANJI_NATIVE_FORMAT;
...
// These synchronous-request bounds keep parse + validation + remap below a
// 128 MiB Worker isolate while still allowing substantial subtree transfers.
// Larger workspaces must be split until native transfer moves to a resumable
// background job. The byte limit is UTF-8, not JS code units.
...
function localStoredFileLocator(value: string) {
...
const STORED_FILE_ID_KEYS = new Set([
  'uploadid',
  'fileuploadid',
  'filekey',
  'storagekey',
...
/**
 * Final schema-independent export fence. Product-specific stripping above
 * preserves friendly placeholders, but a stale/changed property schema or an
 * unusual nested block payload must never smuggle a private storage locator
 * into a `files.included=false` document.
 */
export function scrubNativeStoredFileReferences(value: unknown): {
  value: unknown;
  stripped: number;
} {
  let stripped = 0;
  const scrub = (current: unknown): unknown => {
    if (typeof current === 'string') {
      if (!localStoredFileLocator(current)) return current;
      stripped += 1;
      return { name: attachmentName(current), strippedFile: true };
    }
    if (Array.isArray(current)) return current.map((entry) => scrub(entry));
    if (!isRecord(current)) return current;

    const out: Record<string, unknown> = {};
    let removedDirectReference = false;
    for (const [key, raw] of Object.entries(current)) {
      const normalized = key.toLowerCase();
      if (STORED_FILE_ID_KEYS.has(normalized)) {
        removedDirectReference = true;
        continue;
      }
      if (typeof raw === 'string' && localStoredFileLocator(raw)) {
        removedDirectReference = true;
        continue;
      }
      out[key] = scrub(raw);
    }
    if (removedDirectReference) {
      if (current.strippedFile !== true) stripped += 1;
      out.strippedFile = true;
      const name = current.name ?? current.fileName;
      if (!('name' in out) && typeof name === 'string' && name.trim()) out.name = name.trim();
    }
    return out;
  };
  return { value: scrub(value), stripped };
}
...
export function sanitizeNativeEntitiesForExport(entities: NativeEntities): SanitizedNativeEntities {
  ...
  const finalizeStoredFiles = <T>(value: T, entityId: string, detail: string): T => {
    const result = scrubNativeStoredFileReferences(value);
    strippedReferences += result.stripped;
    if (result.stripped > 0) {
      warnings.push({ code: 'stripped_file', entityId, detail });
    }
  ...
  const pages = entities.pages.map((page) => {
    ...
    return finalizeStoredFiles(next, page.id, 'untyped local storage reference');
  });
  ...
  const blocks = entities.blocks.map((block) => {
    ...
    return finalizeStoredFiles(next, block.id, 'untyped local storage reference');
  });
  ...
  const dbProperties = entities.dbProperties.map((property) => {
    ...
    return finalizeStoredFiles(next, property.id, 'untyped local storage reference');
  });
  ...
  const dbViews = entities.dbViews.map((view) => {
    ...
    return finalizeStoredFiles(next, view.id, 'untyped local storage reference');
  });
  ...
  const dbTemplates = entities.dbTemplates.map((template) => {
    ...
    return finalizeStoredFiles(next, template.id, 'untyped local storage reference');
  });
  ...
  const comments = entities.comments.map((comment) => {
    ...
    return finalizeStoredFiles(next, comment.id, 'untyped local storage reference');
  });
...
export function validateNativeEnvelope(
  ...
  const normalizedValue = normalizeLegacyHanjiNativeDocument(value);
  const document = requiredRecord(normalizedValue, 'document');
  if (!isHanjiNativeFormat(document.format)) invalidNative('format', `must equal "${NATIVE_FORMAT}".`);
  ...
  return normalizedValue as NativeExportEnvelope;
}
...
export function isNativeEnvelope(value: unknown): boolean {
  if (!value || typeof value !== 'object') return false;
  const record = value as Record<string, unknown>;
  if (!isHanjiNativeFormat(record.format)) return false;
