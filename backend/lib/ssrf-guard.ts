...
// `NOTIONLIKE_SSRF_DNS_CHECK=off` skips the resolution step for air-gapped or
// DoH-unreachable self-hosted runtimes. The literal-host checks still apply.
function dnsCheckDisabled(): boolean {
  const flag = ssrfEnv('NOTIONLIKE_SSRF_DNS_CHECK')?.toLowerCase();
...
async function resolveAddresses(hostname: string): Promise<string[]> {
  const endpoint = ssrfEnv('NOTIONLIKE_SSRF_DOH_URL') ?? DEFAULT_DOH_URL;
  const lookups = [
    { type: 'A', code: 1 },
    { type: 'AAAA', code: 28 },
  ].map(async ({ type, code }) => {
