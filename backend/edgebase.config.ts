...
const APP_ORIGIN =
  envValue('NOTIONLIKE_APP_ORIGIN') ??
  envValue('EDGEBASE_APP_ORIGIN') ??
  'http://localhost:8787';
const PASSKEY_RP_ID =
  envValue('NOTIONLIKE_PASSKEY_RP_ID') ??
  envValue('EDGEBASE_PASSKEY_RP_ID') ??
  originHostname(APP_ORIGIN);
const PASSKEY_ORIGINS = envList('NOTIONLIKE_PASSKEY_ORIGINS', 'EDGEBASE_PASSKEY_ORIGINS');
const AUTH_EMAIL_FROM =
  envValue('NOTIONLIKE_AUTH_EMAIL_FROM') ??
  envValue('EDGEBASE_EMAIL_FROM') ??
  'noreply@localhost';
const CLOUDFLARE_EMAIL_API_TOKEN =
  envValue('NOTIONLIKE_CLOUDFLARE_EMAIL_API_TOKEN') ??
  envValue('EDGEBASE_EMAIL_CLOUDFLARE_API_TOKEN') ??
  envValue('EDGEBASE_EMAIL_API_KEY');
const CLOUDFLARE_EMAIL_ACCOUNT_ID =
  envValue('NOTIONLIKE_CLOUDFLARE_EMAIL_ACCOUNT_ID') ??
  envValue('EDGEBASE_EMAIL_CLOUDFLARE_ACCOUNT_ID');
const CLOUDFLARE_EMAIL_BINDING =
  envValue('NOTIONLIKE_CLOUDFLARE_EMAIL_BINDING') ??
  envValue('EDGEBASE_EMAIL_CLOUDFLARE_BINDING') ??
  'EMAIL';
const OAUTH_PROVIDER_NAMES = envList(
  'NOTIONLIKE_AUTH_OAUTH_PROVIDERS',
...
const OAUTH_PROVIDERS = Object.fromEntries(
  OAUTH_PROVIDER_NAMES.map((provider) => {
    ...
    const clientId =
      envValue(`NOTIONLIKE_OAUTH_${envKey}`) ??
      envValue(`EDGEBASE_OAUTH_${envKey}`) ??
      envValue(envKey);
    const clientSecret =
      envValue(`NOTIONLIKE_OAUTH_${secretKey}`) ??
      envValue(`EDGEBASE_OAUTH_${secretKey}`) ??
      envValue(secretKey);
...
function roomAccessDebugEnabled() {
  return envValue('NOTIONLIKE_DEBUG_ROOM_ACCESS') === '1';
}
...
/**
 * Notionlike — local backend data model.
 *
...
const appTables = {
        ...
        file_uploads: {
          schema: {
            ...
            status: { type: 'string', default: 'pending' }, // pending | uploaded | deleted | expired
            url: { type: 'string' },
            createdBy: { type: 'string' },
            expiresAt: { type: 'datetime' },
            completedAt: { type: 'datetime' },
...
export default defineConfig({
  ...
  auth: {
    ...
    // Dev bootstrap only. Product entry uses EdgeBase account auth. The static
    // flag only registers the /signin/anonymous route; the real gate is the
    // per-request rule below.
    anonymousAuth: true,
    ...
    access: {
      ...
      // Gate on a loopback ip: dev and CI connect from 127.0.0.1/::1, while any
      // external production client presents a real ip and is denied 403. (Self-
      // hosting behind a proxy that collapses the client ip to loopback should
      // additionally disable anonymousAuth in its deploy config.)
  ...
  cors: {
    ...
    // here. Browser CORS is only needed for the separate Vite dev server
    // (web dev on :3000 calls http://localhost:8787 directly). Self-hosters
    // using VITE_EDGEBASE_URL must keep the app/API same-site for the Strict
    // cookie. APP_ORIGIN is included as an exact credentialed origin so a
    // production same-site split works without source edits; wildcard origins
    // remain intentionally rejected by EdgeBase.
    origin: [
      'http://localhost:3000',
      'http://127.0.0.1:3000',
      'http://[::1]:3000',
