...
// OPERATIONAL REQUIREMENT — email-based bootstrap trusts context.auth.email,
// which EdgeBase does NOT guarantee to be a verified address: the password
// signup flow mints a session JWT immediately with the unverified,
// caller-chosen email (edgebase packages/server/src/routes/auth.ts createUser
// `verified: false` → createSessionAndTokens), and the function auth middleware
// copies that claim into context.auth.email with no verified check
// (packages/server/src/middleware/auth.ts buildAuthContextFromPayload). The one
// mitigation is that signup rejects an already-registered email, so an
// attacker can only claim a NOT-YET-REGISTERED address.
//
// Consequences for operators:
//  - On an OPEN-SIGNUP instance, prefer NOTIONLIKE_INSTANCE_ADMIN_USER_IDS over
//    NOTIONLIKE_INSTANCE_ADMIN_EMAILS: an attacker who registers the configured
//    admin email before the legitimate admin does self-promotes to instance
//    admin on their first authenticated request.
//  - If you must use the email allowlist, register the admin account first (or
//    keep signup closed) so the address is already taken and cannot be claimed.
// The email path is retained because it is the documented first-admin bootstrap
// convenience; do not redesign the flow here.
export function instanceAdminAuthority(opts: {
  ...
  if (actorEmail && opts.envAdminEmails.some((email) => normalizeEmail(email) === actorEmail)) {
    // See the OPERATIONAL REQUIREMENT note above: actorEmail is unverified.
    return 'environment';
  }
  return null;
}
...
async function requireInstanceAdmin(context: FunctionContext) {
  ...
  const envAdminIds = envList(context.env, 'NOTIONLIKE_INSTANCE_ADMIN_USER_IDS', 'EDGEBASE_INSTANCE_ADMIN_USER_IDS');
  const envAdminEmails = envList(context.env, 'NOTIONLIKE_INSTANCE_ADMIN_EMAILS', 'EDGEBASE_INSTANCE_ADMIN_EMAILS')
    .map((email) => email.toLowerCase());
  const actorEmail = normalizeEmail(context.auth?.email);
  ...
  const bootstrapHint = configuredAdminIds.length === 0 && envAdminIds.length === 0 && envAdminEmails.length === 0
    ? ' Configure NOTIONLIKE_INSTANCE_ADMIN_USER_IDS or NOTIONLIKE_INSTANCE_ADMIN_EMAILS to bootstrap the first administrator.'
    : '';
      ...
      environment: [
        {
          ...
          configured: envList(env, 'NOTIONLIKE_INSTANCE_ADMIN_USER_IDS', 'EDGEBASE_INSTANCE_ADMIN_USER_IDS').length > 0,
          detail: '환경변수 기반 인스턴스 관리자 ID',
        },
        {
          key: 'instance-admin-emails',
          label: '관리자 이메일 allowlist',
          configured: envList(env, 'NOTIONLIKE_INSTANCE_ADMIN_EMAILS', 'EDGEBASE_INSTANCE_ADMIN_EMAILS').length > 0,
          detail: '환경변수 기반 인스턴스 관리자 이메일',
        },
        {
          key: 'strict-admins',
          label: 'Strict 관리자 모드',
          configured: envFlag(env, 'NOTIONLIKE_STRICT_INSTANCE_ADMINS', 'EDGEBASE_STRICT_INSTANCE_ADMINS'),
          detail: '하위 호환용 플래그입니다. 소유자 자동 승격은 항상 꺼져 있고, 환경변수 allowlist만 bootstrap에 사용됩니다.',
        },
        {
          key: 'email',
          label: '이메일 발송',
