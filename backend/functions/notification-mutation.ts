...
import type { DbRef as AppDbRef, Page } from '../lib/app-types';
...
async function assertNotificationScopeAccess(
  ...
  if (workspace.deletionPendingAt) {
    throw Object.assign(new Error('Workspace deletion is already in progress.'), {
      code: 409,
      status: 409,
    });
  }
...
function sanitizeNotificationInput(raw: unknown): NotificationInput {
  ...
  if ((input.blockId || input.commentId) && !input.pageId) {
    throw new Error('pageId is required when blockId or commentId is present.');
  }
  const targetPageId = pageIdFromNotificationTarget(input.target);
  if (targetPageId && input.pageId !== targetPageId) {
    throw new Error('Notification target page does not match pageId.');
  }
  return input;
}
...
function pageIdFromNotificationTarget(target: string | undefined) {
...
function notificationPageId(record: Pick<NotificationRecord, 'pageId' | 'target'>) {
  return record.pageId ?? pageIdFromNotificationTarget(record.target);
}

async function notificationPageState(
  admin: AdminDbAccessor,
  workspaceId: string,
  pageId: string,
  actorId: string,
  actorEmail?: string | null,
): Promise<'live' | 'hidden' | 'orphan'> {
  const workspace = await getExisting(
    admin.db('app').table<Workspace>('workspaces'),
    workspaceId,
  );
  if (!workspace || workspace.deletionPendingAt) return 'orphan';
  const contentDb = boundedDb(admin, workspaceId);
  const page = await getExisting(contentDb.table<Page>('pages'), pageId);
  if (!page || page.workspaceId !== workspaceId || page.deletionPendingAt) return 'orphan';
  if (page.inTrash) return 'hidden';
  try {
    return (await pageAccessRole(contentDb as unknown as AppDbRef, page, actorId, undefined, actorEmail))
      ? 'live'
      : 'hidden';
  } catch {
    return 'hidden';
...
async function notificationsForUser(
  db: DbRef,
  admin: AdminDbAccessor,
  workspaceId: string,
  actorId: string,
  ...
  for (let index = 0; index < pageIds.length; index += 20) {
    ...
    const resolved = await Promise.all(chunk.map(async (pageId) => [
      pageId,
      await notificationPageState(admin, workspaceId, pageId, actorId, actorEmail),
    ] as const));
  ...
  for (let index = 0; index < orphanIds.length; index += 400) {
    await db.transact(orphanIds.slice(index, index + 400).map((id) => ({
      table: 'notifications',
      op: 'delete' as const,
      id,
    }))).catch((error) => {
      console.error('[notifications] orphan cleanup failed:', error);
    });
  }
...
async function listNotifications(
  ...
  const all = sortNotifications(await notificationsForUser(db, admin, workspaceId, actorId, actorEmail));
  const records = all
...
async function syncNotifications(
  ...
  const routedDb = boundedDb(admin, workspaceId) as unknown as AppDbRef;

  for (const activity of activities) {
    const referencedPageId = activity.pageId ?? pageIdFromNotificationTarget(activity.target);
    if (referencedPageId && !validatedPageIds.has(referencedPageId)) {
      const state = await notificationPageState(admin, workspaceId, referencedPageId, actorId, actorEmail);
      if (state !== 'live') {
        throw Object.assign(new Error('Notification target page is unavailable.'), { status: 409 });
      }
      validatedPageIds.add(referencedPageId);
    }
    ...
    if (current) {
      ...
      try {
        ...
        await notifications.delete(updated.id).catch((compensationError) => {
          console.error('[notifications] sync update rollback failed:', compensationError);
      ...
      try {
        await assertNotificationTargetAvailable(routedDb, inserted);
      } catch (error) {
        await notifications.delete(inserted.id).catch((compensationError) => {
          console.error('[notifications] sync insert rollback failed:', compensationError);
          throw compensationError;
        });
        throw error;
      }
...
async function markNotificationsRead(
  ...
  const notifications = db.table<NotificationRecord>('notifications');
  const records = await notificationsForUser(db, admin, workspaceId, actorId, actorEmail);
