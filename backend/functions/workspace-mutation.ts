...
import { MAX_RAW_TRANSACT_OPS, boundedDb, discoverPermissionWorkspaceIds, ensurePageWorkspaceIndex, type AdminDbAccessor } from '../lib/workspace-db';
...
function cleanPatch(patch: Record<string, unknown>): Partial<Workspace> {
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    if (key === 'icon') {
      ...
      else if (typeof value === 'string') out.icon = value;
      continue;
    }
    if (key === 'domain') {
...
function appOriginForInvitation(body: Record<string, unknown>, request?: Request) {
  return (
    urlOrigin(body.appOrigin) ??
    urlOrigin(body.origin) ??
    urlOrigin(request?.headers.get('Origin')) ??
    urlOrigin(request?.url)
  );
}

function workspaceInviteUrl(
  workspace: Workspace,
  invitation: WorkspaceInvitation,
  body: Record<string, unknown>,
  request?: Request,
) {
  const origin = appOriginForInvitation(body, request) ?? '';
  const workspaceKey = workspace.domain || workspace.id;
  const path = `/workspace/${encodeURIComponent(workspaceKey)}`;
  return `${origin}${path}?invite=${encodeURIComponent(invitation.token)}`;
}

function invitationEmailHtml(workspace: Workspace, invitation: WorkspaceInvitation, inviteUrl: string) {
  const workspaceName = escapeHtml(workspace.name || 'Untitled workspace');
  const email = escapeHtml(invitation.email);
  const url = escapeHtml(inviteUrl);
  const role = escapeHtml(roleLabel(invitation.role).toLowerCase());
  return [
    '<div style="font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, \'Segoe UI\', sans-serif; color: #1f1f1f; line-height: 1.5;">',
    `<h1 style="font-size: 20px; margin: 0 0 12px;">You were invited to ${workspaceName}</h1>`,
    `<p style="margin: 0 0 16px;">Sign in with <strong>${email}</strong> to join this workspace as a ${role}.</p>`,
    `<p style="margin: 0 0 20px;"><a href="${url}" style="display: inline-block; background: #2383e2; color: #fff; padding: 9px 12px; border-radius: 6px; text-decoration: none;">Open workspace</a></p>`,
    `<p style="margin: 0; color: #6f6f6f; font-size: 13px;">If the button does not work, open this link: ${url}</p>`,
    '</div>',
  ].join('');
}

async function deliverWorkspaceInvitationEmail(
  db: DbRef,
  invitationsTable: TableRef<WorkspaceInvitation>,
  workspace: Workspace,
  invitation: WorkspaceInvitation,
  ...
  if (!emailSender) {
    const updated = await invitationsTable.update(invitation.id, {
      emailDeliveryStatus: 'not_configured',
      emailDeliveryError: 'Email provider is not configured.',
    });
    if (workspace.organizationId) {
      await recordOrganizationAudit(db, {
        ...
        actorId,
        action: 'workspace_invitation.email_not_configured',
  ...
  const inviteUrl = workspaceInviteUrl(workspace, invitation, body, request);
  const subject = `You were invited to ${workspace.name || 'a workspace'}`;
  const text = [
    `You were invited to ${workspace.name || 'Untitled workspace'}.`,
    '',
    `Sign in with ${invitation.email} to join as ${roleLabel(invitation.role).toLowerCase()}.`,
    inviteUrl,
  ].join('\n');
  ...
  try {
    const result = await emailSender.send({
      ...
      subject,
      text,
      html: invitationEmailHtml(workspace, invitation, inviteUrl),
    });
    const patch: Partial<WorkspaceInvitation> = result.success
      ? {
      ...
      : {
          ...
          emailDeliveryError: 'Email provider returned failure.',
        };
    const updated = await invitationsTable.update(invitation.id, patch);
    if (workspace.organizationId) {
      await recordOrganizationAudit(db, {
        organizationId: workspace.organizationId,
        workspaceId: workspace.id,
        actorId,
        action: result.success ? 'workspace_invitation.email_sent' : 'workspace_invitation.email_failed',
        targetType: 'workspace_invitation',
        targetId: invitation.id,
        metadata: {
    ...
    const updated = await invitationsTable.update(invitation.id, {
      emailDeliveryStatus: 'failed',
      emailDeliveryError: message.slice(0, 240),
    });
    if (workspace.organizationId) {
      await recordOrganizationAudit(db, {
        organizationId: workspace.organizationId,
        workspaceId: workspace.id,
        actorId,
        action: 'workspace_invitation.email_failed',
        targetType: 'workspace_invitation',
        targetId: invitation.id,
        metadata: { email: invitation.email, role: invitation.role },
        occurredAt: nowIso(),
...
function clientSourceFromRequest(request?: Request) {
  const source = request?.headers.get('X-Notionlike-Client-Source')?.trim().toLowerCase();
...
function parseMcpClientMetadata(body: Record<string, unknown>, request?: Request) {
  ...
  const readOnly = request?.headers.get('X-Notionlike-MCP-Read-Only') === 'true' || client.readOnly === true;
  return {
    source,
    clientId:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Client-ID'), 'clientId') ??
      optionalClientAuditString(client.clientId, 'client.clientId'),
    clientName:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Client-Name'), 'clientName') ??
      optionalClientAuditString(client.clientName, 'client.clientName'),
    readOnly,
    subjectType:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Subject-Type'), 'subjectType', 80) ??
      optionalClientAuditString(client.subjectType, 'client.subjectType', 80),
    subjectId:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Subject-ID'), 'subjectId') ??
      optionalClientAuditString(client.subjectId, 'client.subjectId'),
    policyIssuer:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Policy-Issuer'), 'policyIssuer') ??
      optionalClientAuditString(client.issuer, 'client.issuer'),
    policyAudience:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Policy-Audience'), 'policyAudience') ??
      optionalClientAuditString(client.audience, 'client.audience'),
    transport:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Transport'), 'transport', 80) ??
      optionalClientAuditString(client.transport, 'client.transport', 80),
    provisioningId:
      optionalClientAuditString(request?.headers.get('X-Notionlike-MCP-Provisioning-ID'), 'provisioningId') ??
...
async function updateOrganizationSettings(db: DbRef, body: Record<string, unknown>, actorId: string) {
  ...
  if ('signupPolicy' in body) {
    ...
    if (
      instancePatch.signupPolicy === 'verified_domains' &&
      !(ctx.organizationDomains ?? []).some((domain) => (domain.status ?? 'pending') === 'verified')
    ) {
      throw new Error(`Verify an organization domain before limiting signup to ${signupPolicyLabels[instancePatch.signupPolicy]}.`);
    }
...
async function upsertNotification(db: DbRef, record: Omit<NotificationRecord, 'id'>) {
...
async function emitMembershipNotification(
  ...
  await bestEffort('workspace-mutation membership notification', upsertNotification(db, {
    workspaceId: workspace.id,
    userId: member.userId,
...
async function createWorkspace(
  ...
  // synchronously so the fresh workspace's pages route immediately.
...
async function updateWorkspace(db: DbRef, body: Record<string, unknown>, actorId: string) {
...
async function deleteWorkspaceNotionImportArtifacts(db: DbRef, workspaceId: string) {
  ...
  for (const item of items) await itemsTable.delete(item.id);
  for (const mapping of mappings) await mappingsTable.delete(mapping.id);
  for (const job of jobs) await jobsTable.delete(job.id);
  for (const connection of connections) await connectionsTable.delete(connection.id);
...
async function deleteWorkspace(
  ...
  const undeletedUploads = uploads.filter((item) => item.status !== 'deleted' && item.key);
  for (let i = 0; i < undeletedUploads.length; i += 10) {
    await Promise.all(
      undeletedUploads.slice(i, i + 10).map((item) =>
        bestEffort(
          'workspace-mutation deleteStoredFile',
          deleteStoredFile(storage, request, item.bucket || FILE_BUCKET, item.key as string),
        ),
      ),
    );
  }
  const notionImportCleanup = await deleteWorkspaceNotionImportArtifacts(contentReadDb, id);
  ...
  const centralOps: TransactOperation[] = [
    ...
    ...ctx.members.map((member): TransactOperation => ({ table: 'workspace_members', op: 'delete', id: member.id })),
...
async function updateMyProfile(db: DbRef, body: Record<string, unknown>, actorId: string) {
  const workspaceId = requireString(body.workspaceId ?? body.id, 'workspaceId');
  const ctx = await workspaceContext(db, workspaceId, actorId);
...
async function inviteMember(
  ...
  if (!userId) {
    ...
    await assertOrganizationInviteAllowed(db, ctx.workspace, email, role);
    const displayName = optionalString(body.displayName, 'displayName');
    const pending = ctx.invitations.find((invitation) => normalizeEmail(invitation.email) === email) ?? null;
    const invitationAction = pending ? 'workspace_invitation.update' : 'workspace_invitation.create';
    const invitationSeed = pending
      ? await ctx.invitationsTable.update(pending.id, {
          displayName,
          role,
          status: 'pending',
      ...
      : await ctx.invitationsTable.insert({
          workspaceId: ctx.workspace.id,
          email,
          displayName,
          role,
    ...
    const invitation = await deliverWorkspaceInvitationEmail(
      db,
      ctx.invitationsTable,
      ctx.workspace,
      invitationSeed,
      emailSender,
      body,
    ...
    await recordWorkspaceAudit(
      ...
      invitation.id,
      {
        ...
        displayName: invitation.displayName ?? null,
        role: invitation.role,
        deliveryStatus: invitation.emailDeliveryStatus ?? null,
      },
    );
    const invitations = sortInvitations([
      ...ctx.invitations.filter((item) => item.id !== invitation.id),
      invitation,
    ]);
    return {
      workspace: ctx.workspace,
...
async function acceptInvitation(
  ...
  const token = optionalString(body.token, 'token');
  const invitationId = optionalString(body.invitationId, 'invitationId');
  if (!token && !invitationId) throw new Error('Invitation token or id is required.');

  // Bind acceptance to the authenticated account's OWN email, never a
  // client-supplied body.email. In this product the login email IS the account
  // identity (email+password, no email-link ownership step), so the session
  // email from the auth context is the authoritative principal. Fail closed
  // when it cannot be determined: an invite (including admin-role invites) must
  // never be redeemable by an account whose email we cannot verify.
  //
  // Step-up seam: a future 2FA/step-up check for sensitive-role invites belongs
  // right here, after the email binding and before any membership write.
  if (!actorEmail) throw new Error('Authenticated account email is required to accept an invitation.');

  const workspaces = db.table<Workspace>('workspaces');
  const membersTable = db.table<WorkspaceMember>('workspace_members');
  const invitationsTable = db.table<WorkspaceInvitation>('workspace_invitations');
  const invitation = token
    ? (await listAll(invitationsTable.where('token', '==', token)))[0] ?? null
    : invitationId
      ? await getExisting(invitationsTable, invitationId)
      : null;
  if (!invitation) throw new Error('Workspace invitation was not found.');
  if (normalizeEmail(invitation.email) !== actorEmail) {
    throw new Error('Invitation email does not match.');
  }
  if (!invitationIsPending(invitation)) {
    if ((invitation.status ?? 'pending') === 'pending' && invitation.expiresAt) {
      await bestEffort('workspace-mutation invitationsTable.update', invitationsTable.update(invitation.id, { status: 'expired' }));
      throw new Error('Workspace invitation has expired.');
    }
    throw new Error('Workspace invitation is not pending.');
  }

  const workspace = await getExisting(workspaces, invitation.workspaceId);
  if (!workspace) throw new Error('Workspace was not found.');
  const role = parseMemberRole(invitation.role, 'member');
  if (!manageableRoles.has(role)) throw new Error('Workspace invitation role is invalid.');
  await assertOrganizationInviteAllowed(db, workspace, normalizeEmail(invitation.email), role);

  const now = nowIso();
  const workspaceMembers = await listAll(membersTable.where('workspaceId', '==', workspace.id));
  const existing = workspaceMembers.find((member) => member.userId === actorId) ?? null;
  await assertOrganizationMemberCanJoinWorkspace(db, workspace, actorId);
  const existingRole = existing
    ? workspace.ownerId === actorId
      ? 'owner'
      : parseMemberRole(existing.role, 'member')
    : null;
  const acceptedRole = existingRole && roleRank[existingRole] >= roleRank[role] ? existingRole : role;
  const memberPatch: Partial<WorkspaceMember> = {
    displayName: invitation.displayName ?? existing?.displayName ?? null,
    email: normalizeEmail(invitation.email),
    role: acceptedRole,
  };
  const member =
    existing
      ? await membersTable.update(existing.id, memberPatch)
  ...
  await upsertOrganizationMemberForWorkspaceMember(db, workspace, member, invitation.createdBy ?? actorId);
  const acceptedInvitation = await invitationsTable.update(invitation.id, {
    status: 'accepted',
    acceptedBy: actorId,
    acceptedAt: now,
  });
  await recordWorkspaceAudit(
    ...
    acceptedInvitation.id,
    {
      ...
      acceptedRole,
      workspaceMemberId: member.id,
      invitedBy: acceptedInvitation.createdBy ?? null,
    },
    now,
  );
  const members = sortMembers([
    ...workspaceMembers.filter((item) => item.id !== member.id),
    member,
  ]);
  const invitations = sortInvitations(
    (await listAll(invitationsTable.where('workspaceId', '==', workspace.id))).filter(
      (item) => item.id !== acceptedInvitation.id,
    ),
  );
  return {
    workspace,
...
async function removeInvitation(db: DbRef, body: Record<string, unknown>, actorId: string) {
  const workspaceId = requireString(body.workspaceId ?? body.id, 'workspaceId');
  const ctx = await workspaceContext(db, workspaceId, actorId);
  assertWorkspaceAdmin(ctx.currentRole);
  const invitationId = typeof body.invitationId === 'string' && body.invitationId.trim()
    ...
    : null;
  const email = normalizeEmail(body.email);
  const current = invitationId
    ? ctx.invitations.find((invitation) => invitation.id === invitationId)
    : ctx.invitations.find((invitation) => normalizeEmail(invitation.email) === email);
  if (!current) throw new Error('Workspace invitation was not found.');
  const invitation = await ctx.invitationsTable.update(current.id, { status: 'revoked' });
  await recordWorkspaceAudit(
    db,
    ctx.workspace,
    actorId,
  ...
  return {
    ...
    invitations: ctx.invitations.filter((item) => item.id !== invitation.id),
    members: ctx.members,
  };
}
...
export const POST = defineFunction(async (context) => {
  const { auth, admin, request, email, storage } = context as FunctionContext;
  ...
  try {
    switch (action) {
      ...
      case 'createWorkspace':
        return await createWorkspace(db, admin, body, auth.id, normalizeEmail(auth.email));
      case 'get':
      case 'members':
        return await getMembers(db, body, auth.id, normalizeEmail(auth.email));
      case 'update':
        return await updateWorkspace(db, body, auth.id);
      ...
      case 'updateMyProfile':
        return await updateMyProfile(db, body, auth.id);
      case 'inviteMember':
        return await inviteMember(db, body, auth.id, email, request);
      case 'acceptInvitation':
        return await acceptInvitation(db, body, auth.id, normalizeEmail(auth.email));
      case 'removeInvitation':
        return await removeInvitation(db, body, auth.id);
