...
interface FileUpload {
  ...
  contentType?: string;
  size?: number;
  status?: 'pending' | 'uploaded' | 'deleting' | 'deleted' | 'expired' | 'failed';
  url?: string;
  createdBy?: string;
...
async function dispatch(context: FunctionContext) {
  ...
  const fileUploadMutation = parts[0] === 'file_uploads' && (
    (method === 'POST' && parts.length === 1)
    || (method === 'POST' && parts[1] && (parts[2] === 'send' || parts[2] === 'complete'))
    || (method === 'DELETE' && parts[1] && parts.length === 2)
  );
  if (fileUploadMutation) {
    // The compatibility surface previously changed metadata without storing,
    // verifying, quota-accounting, or deleting bytes. Fail closed until a
    // storage-backed implementation can honor the same guarantees as Hanji's
    // native file mutation API.
    return notionError(
      501,
      'internal_server_error',
      'Notion-compatible file upload mutations are not available in this release.',
    );
  }
  const primaryContentMutation = (
    parts[0] === 'pages'
    && (
      (method === 'POST' && parts.length === 1)
      || (method === 'PATCH' && !!parts[1] && parts.length === 2)
    )
  ) || (
    parts[0] === 'blocks'
    ...
    parts[0] === 'data_sources'
    && (
      (method === 'POST' && parts.length === 1)
      || ((method === 'PATCH' || method === 'DELETE') && !!parts[1] && parts.length === 2)
    )
  );
  if (primaryContentMutation) {
    // These compatibility handlers used to write pages, blocks, covers and
    // file-valued database properties directly. That bypassed the native
    // upload ownership, reference, quota and permanent-delete lifecycle.
    // Keep the read-only facade available, but fail every affected write
    // closed until it is routed through the canonical mutation services.
    return notionError(
      501,
      'internal_server_error',
      'Notion-compatible content mutations are not available in this release.',
    );
  }
