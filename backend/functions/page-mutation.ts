...
async function createPage(
  ...
  await assertNoUnownedStoredFileReferences(db, {
    ...
    createPropertyValues: schemaFilePropertyReferences(
      page.properties,
...
async function updatePage(db: DbRef, body: Record<string, unknown>, actorId: string, actorEmail?: string | null) {
  ...
  // Row property normalization, schema validation, reciprocal relations and
  // schema-aware raw-string file lifecycle all live in database-row-mutation.
  // Do not let the generic page endpoint bypass that contract merely because
  // a legacy `files` value is a string the generic reference walker ignores.
  if (current.parentType === 'database' && 'properties' in patch) {
    throw Object.assign(
      new Error('Database row properties must be updated through the database-row mutation endpoint.'),
      { status: 409 },
    );
  }
  ...
  const currentFileReferences = {
    ...
    pagePropertyValues: schemaFilePropertyReferences(
      current.properties,
      propertyKeys(current.properties),
    ),
  };
  const nextFileReferences = {
    icon: 'icon' in nextPatch ? nextPatch.icon : current.icon,
    cover: 'cover' in nextPatch ? nextPatch.cover : current.cover,
    properties: 'properties' in nextPatch ? nextPatch.properties : current.properties,
    pagePropertyValues: schemaFilePropertyReferences(
      'properties' in nextPatch ? nextPatch.properties : current.properties,
  ...
  return withFileWorkspaceLease(db, current.workspaceId, actorId, 'page-file-reference-update', async (lease) => {
    ...
    if (fresh.isLocked && !lockedPageAllowsPatch(patch)) throw new Error('Page is locked.');
    await assertFileTargetsNotDeleting(db, fresh.workspaceId, [fresh.id]);
    const freshCurrentReferences = {
      icon: fresh.icon,
      cover: fresh.cover,
      properties: fresh.properties,
      pagePropertyValues: schemaFilePropertyReferences(
        fresh.properties,
        propertyKeys(fresh.properties),
      ),
    };
    const freshNextReferences = {
      icon: 'icon' in nextPatch ? nextPatch.icon : fresh.icon,
      cover: 'cover' in nextPatch ? nextPatch.cover : fresh.cover,
      properties: 'properties' in nextPatch ? nextPatch.properties : fresh.properties,
      pagePropertyValues: schemaFilePropertyReferences(
        'properties' in nextPatch ? nextPatch.properties : fresh.properties,
        propertyKeys('properties' in nextPatch ? nextPatch.properties : fresh.properties),
      ),
    };
    return updateWithFileReferenceLifecycle(db, {
      ...
      current: fresh,
      data: nextPatch,
      currentReferences: freshCurrentReferences,
      nextReferences: freshNextReferences,
      association: {
        field: 'pageId',
        id: fresh.id,
        // Block and database-property uploads share pageId for authorization,
        // but their canonical reference lives in another record.
        filter: (upload) => !upload.blockId && !upload.propertyId && !upload.databaseId,
      },
      actorId,
    });
  });
...
async function restorePage(db: DbRef, body: Record<string, unknown>, actorId: string, actorEmail?: string | null) {
  ...
  return withFileWorkspaceLease(db, initial.workspaceId, actorId, 'page-restore', async (lease) => {
    await lease.assertOwned();
    const pages = db.table<Page>('pages');
    const root = await getExisting(pages, id);
    if (!root) throw new Error('Page was not found.');
    if (root.deletionPendingAt) {
      throw Object.assign(
        new Error('Permanent page deletion is in progress; retry the deletion instead of restoring.'),
        { status: 409 },
      );
    }
    await assertCanEditPage(db, root, actorId, actorEmail);
...
async function deletePage(
  ...
  const id = requireString(body.id, 'id');
  const root = await getExisting(db.table<Page>('pages'), id);
  if (!root) throw new Error('Page was not found.');
  return withFileWorkspaceLease(db, root.workspaceId, actorId, 'permanent-page-delete', (lease) =>
    deletePageUnderLease(db, admin, body, actorId, actorEmail, storage, request, lease));
}

async function deletePageUnderLease(
  db: DbRef,
  admin: FunctionContext['admin'],
  body: Record<string, unknown>,
  actorId: string,
  ...
  const root = await getExisting(pages, id);
  if (!root) throw new Error('Page was not found.');
  const workspace = await getExisting(db.table<Workspace>('workspaces'), root.workspaceId);
  if (!workspace) throw new Error('Workspace was not found.');
  await assertCanEditPage(db, root, actorId, actorEmail);
  if (!(await sharedCanManagePageAccess(db, root, workspace, actorId, actorEmail))) {
    throw new Error('Permanent delete access required.');
  }
  if (!root.inTrash) {
    throw Object.assign(
      new Error('Page must be moved to trash before permanent deletion.'),
      { status: 409 },
    );
  }
  const workspacePages = await listAll(pages.where('workspaceId', '==', root.workspaceId));
  ...
  // Finish every bounded/fail-closed discovery read before irreversible file
  // or notification cleanup. If a safety ceiling is exceeded, the page stays
  // fenced and wholly retryable instead of becoming a partially erased page.
  ...
  // collectSubtree is ancestor-first; reverse it for deletion so every child
  // commits before its parent and the root remains a stable retry anchor until
  // the final page-delete chunk succeeds.
...
export const POST = defineFunction(async (context) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'delete':
        return await deletePage(db, admin, pageIdBodySchema.parse(body), auth.id, actorEmail, storage, request);
    }
  } catch (error) {
