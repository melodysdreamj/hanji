    ...
    const rawImportItems = neededNotionIds.size <= TARGETED_IMPORT_ORDER_QUERY_LIMIT
      ? (await Promise.all(
          Array.from(neededNotionIds, (notionId) => {
            const activeItems = activeItemsQuery();
            const byNotionId = typeof activeItems.where === 'function'
              ? activeItems.where('notionId', '==', notionId)
              : activeItems;
            return listAll(byNotionId);
          }),
        )).flat()
      : await listAll(
          activeItemsQuery(),
          {
            ...
            label: 'Active Notion import row-order items',
          },
        );
