...
import { MAX_RAW_TRANSACT_OPS, boundedDbFromPageHint, ensurePageWorkspaceIndex, type AdminDbAccessor } from '../lib/workspace-db';
...
async function createDatabaseRow(db: DbRef, admin: AdminDbAccessor, body: Record<string, unknown>, actorId: string, actorEmail?: string | null) {
  const pages = db.table<Page>('pages');
...
async function updateDatabaseRow(db: DbRef, body: Record<string, unknown>, actorId: string, actorEmail?: string | null) {
  const pages = db.table<Page>('pages');
  const propertiesTable = db.table<DbProperty>('db_properties');
  ...
  if ('properties' in patch) {
    ...
    const props = await listAll(propertiesTable.where('databaseId', '==', row.parentId));
...
async function deleteDatabaseRow(
  ...
  // Stored file deletions are storage-side; bound the fan-out.
  ...
  for (let i = 0; i < undeletedUploads.length; i += 10) {
    await Promise.all(
      undeletedUploads.slice(i, i + 10).map((item) =>
        bestEffort('database-row-mutation deleteStoredFile', deleteStoredFile(storage, request, item.bucket || FILE_BUCKET, item.key)),
      ),
    );
  }

  // Row cleanup in chunked transact batches (see page-mutation deletePage):
  // bounded request count, atomic per chunk, pages deleted last.
...
export const POST = defineFunction(async (context) => {
  ...
  try {
    ...
    const db = await boundedDbFromPageHint(admin, body.databaseId, body.id, body.rowId);
    switch (action) {
      case 'create':
        return await createDatabaseRow(db, admin, body, auth.id, actorEmail);
      case 'update':
      ...
      case 'delete':
        return await deleteDatabaseRow(db, body, auth.id, actorEmail, storage, request);
      default:
