...
export const POST = defineFunction(async (context) => {
  ...
  const starterLocale = defaultWorkspaceLocale(request?.headers.get('Accept-Language'));
  ...
  if (!workspace) {
    workspace = await workspaces.insert({
      name: starterLocale === 'ko' ? '내 워크스페이스' : 'My Workspace',
      icon: '📓',
      ownerId: auth.id,
    });
  ...
  const permissions = contentDb.table<PagePermission>('page_permissions');
  if (createdWorkspace) {
    const seededPages = await seedDefaultWorkspacePages(contentDb, workspace, auth.id, starterLocale);
  ...
  // Whether the signed-in account is an instance administrator. Gates the
