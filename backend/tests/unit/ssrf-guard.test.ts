...
describe('hostResolvesToBlockedAddress', () => {
  ...
  it('skips resolution when HANJI_SSRF_DNS_CHECK=off', async () => {
...
describe('readResponseBytesWithLimit', () => {
  it('rejects a declared or streamed body before buffering past the cap', async () => {
    await expect(readResponseBytesWithLimit(
    ...
    await expect(readResponseBytesWithLimit(new Response(stream), 10)).rejects.toThrow(
      'exceeds the allowed size',
    );
  });

  it('returns an exact byte sequence below the cap', async () => {
    expect(await readResponseBytesWithLimit(new Response('hello'), 10)).toEqual(
