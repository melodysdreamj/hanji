...
import { csvCell, storageKeyFromUrl } from '../../functions/import-export';
...
describe('export stored-file origin guard', () => {
  it('recognizes only an exact local storage path, never an arbitrary origin or nested marker', () => {
    expect(storageKeyFromUrl('/api/storage/files/workspaces/ws/private.pdf'))
      .toBe('workspaces/ws/private.pdf');
    expect(storageKeyFromUrl('https://evil.example/api/storage/files/workspaces/ws/private.pdf'))
      .toBeUndefined();
    expect(storageKeyFromUrl('/proxy/api/storage/files/workspaces/ws/private.pdf'))
      .toBeUndefined();
    expect(storageKeyFromUrl('//evil.example/api/storage/files/workspaces/ws/private.pdf'))
      .toBeUndefined();
  });
});
