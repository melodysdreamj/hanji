...
describe('Notion incremental discovery live progress', () => {
  ...
  it('promotes a stored resume credential onto the durable job record', () => {
    expect(notionImportSource).toContain(
...
describe('apply lease and mapping idempotency', () => {
  ...
  it('cancels a stranded discovery and removes its job-scoped lease immediately', async () => {
    ...
    expect(db.tables.notion_import_jobs[0].report).toEqual(expect.objectContaining({
      cancelCleanup: expect.objectContaining({ removedItems: 1, pending: false }),
    }));
  });

  it('cancels an interrupted apply, clears its lease, and hides partial product pages', async () => {
    const applyingJob = job('job-1', {
      status: 'ready',
      phase: 'apply_pages',
      progress: { currentStep: 'apply', currentStatus: 'running' },
    });
    const db = workspaceDb({
      notion_import_jobs: [applyingJob],
      notion_import_items: [{
        ...discoveredPage(),
        id: 'cancelled-apply-item',
        jobId: 'job-1',
      }],
      notion_import_mappings: [{
        id: 'partial-page-mapping',
        workspaceId: 'ws-1',
        jobId: 'job-1',
        notionId: 'partial-notion-page',
        notionType: 'page',
        localId: 'partial-local-page',
        localType: 'page',
        relationKind: 'page',
      }],
      notion_import_apply_locks: [{
    ...
    expect(db.tables.notion_import_apply_locks).toHaveLength(0);
    expect(db.tables.notion_import_items).toHaveLength(0);
    expect(db.tables.pages[0]).toMatchObject({
      id: 'partial-local-page',
      inTrash: true,
      isFavorite: false,
    });
...
describe('discovery progress writes cannot land after the terminal update (#11)', () => {
  ...
  it('single-flights discovery and never overwrites a concurrent cancel', () => {
    ...
    expect(cancelBody).toContain('deleteNotionImportJobItems(db, job.id)');
    expect(source).toContain("const nextJob = await updateNotionJobIfStatus(db, job.id, 'ready'");
    expect(source).toContain("const updated = await updateNotionJobIfStatus(db, job.id, 'ready', {\n    status: 'completed'");
  });
