...
describe('Notion-compatible official-shape routing', () => {
  ...
  it('fails every byte-mutating file-upload compatibility route closed with 501', async () => {
    const { admin, ws1 } = splitAdmin();
    const before = ws1.tables.file_uploads.map((upload) => ({ ...upload }));
    const cases = [
      callRaw(POST, admin, 'POST', 'file_uploads', {
        workspace_id: 'ws1',
        filename: 'new.txt',
        content_type: 'text/plain',
        content_length: 4,
      }),
      callRaw(POST, admin, 'POST', 'file_uploads/upload1/send', { part_number: 1 }),
      callRaw(POST, admin, 'POST', 'file_uploads/upload1/complete', {}),
      callRaw(DELETE, admin, 'DELETE', 'file_uploads/upload1'),
    ];
    ...
    for (const result of await Promise.all(cases)) {
      ...
      expect(result.payload).toMatchObject({
        ...
        message: 'Notion-compatible file upload mutations are not available in this release.',
      });
    }
    expect(ws1.tables.file_uploads).toEqual(before);
  });

  it('fails every primary-content compatibility mutation closed without changing state', async () => {
    const { admin, ws1 } = splitAdmin();
    const before = structuredClone(ws1.tables);
    const cases = [
      callRaw(POST, admin, 'POST', 'pages', {
        parent: { page_id: 'page1' },
        children: [{ type: 'image', image: { external: { url: '/api/storage/files/foreign' } } }],
      }),
      callRaw(PATCH, admin, 'PATCH', 'pages/page1', {
        cover: { type: 'external', external: { url: '/api/storage/files/foreign' } },
      }),
      callRaw(PATCH, admin, 'PATCH', 'blocks/page1/children', {
        children: [{ type: 'file', file: { external: { url: '/api/storage/files/foreign' } } }],
      }),
      callRaw(PATCH, admin, 'PATCH', 'blocks/block1', {
        type: 'image',
        image: { external: { url: '/api/storage/files/foreign' } },
      }),
      callRaw(DELETE, admin, 'DELETE', 'blocks/block1'),
      callRaw(POST, admin, 'POST', 'databases', {
        parent: { workspace_id: 'ws1' },
        cover: { type: 'external', external: { url: '/api/storage/files/foreign' } },
      }),
      callRaw(PATCH, admin, 'PATCH', 'databases/db1', {
        properties: { Files: null },
      }),
      callRaw(POST, admin, 'POST', 'data_sources', {
        parent: { database_id: 'db1' },
        cover: { type: 'external', external: { url: '/api/storage/files/foreign' } },
      }),
      callRaw(PATCH, admin, 'PATCH', 'data_sources/db1', {
        properties: { Files: null },
      }),
      callRaw(DELETE, admin, 'DELETE', 'data_sources/db1'),
    ];
    ...
    for (const result of await Promise.all(cases)) {
      ...
      expect(result.payload).toMatchObject({
        code: 'internal_server_error',
        message: 'Notion-compatible content mutations are not available in this release.',
      });
    }
    expect(ws1.tables).toEqual(before);
  });

  it('does not enter the legacy schema mutation transaction', async () => {
    const { admin, ws1 } = splitAdmin();
    ...
    expect(payload.message).toBe('Notion-compatible content mutations are not available in this release.');
