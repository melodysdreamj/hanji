...
[GNU AGPL-3.0](LICENSE) with the
[Sponsor Banner Exception 2.0](LICENSE-EXCEPTION). The exception conditionally
permits private modifications, proprietary combinations, hosted operation, and
source or object-code redistribution without Corresponding Source when the
canonical sponsor feed and sign-in banner remain functional and enabled by
default. If those conditions are not met, plain AGPL-3.0 applies.

Only submit material for which you have authority to grant both the AGPL-3.0
license and the additional permissions in the exception. This contribution
grant does not assign your copyright or authorize a different commercial
license outside those terms.
