...
async function main() {
  ...
  try {
    ...
    await assertPasswordRecoverySurface(browser, appUrl);
    ...
    console.log('PASS password-only AuthGate surface, fragment-safe password recovery route, password account creation, password sign-in, MFA challenge sign-in, and recovery-code MFA sign-in work through the AuthGate UI.');
...
async function assertPasswordRecoverySurface(browser, baseUrl) {
  const { context, page, errors, endInitialSignedOutRefreshWindow } = await newCheckedPage(browser);
  try {
    await page.goto(resolveUrl(baseUrl, '/auth/reset-password#token=synthetic-reset-token'), {
      waitUntil: 'domcontentloaded',
      timeout: options.timeoutMs,
    });
    await page.getByLabel('New password', { exact: true }).waitFor({ state: 'visible', timeout: options.timeoutMs });
    await endInitialSignedOutRefreshWindow();
    await page.getByLabel('Confirm new password', { exact: true }).waitFor({ state: 'visible', timeout: options.timeoutMs });
    await page.getByRole('button', { name: 'Reset password', exact: true }).waitFor({
      state: 'visible',
      timeout: options.timeoutMs,
    });
    const currentUrl = new URL(page.url());
    assert(!currentUrl.searchParams.has('token'), 'Password reset token remained in the URL query.');
    assert(!new URLSearchParams(currentUrl.hash.slice(1)).has('token'), 'Password reset token remained in browser history.');
    await page.screenshot({
      path: join(options.screenshotDir, 'password-reset-auth.png'),
      fullPage: false,
    });
    assertNoBrowserErrors(errors, 'password reset AuthGate surface');
  } finally {
...
async function assertPasswordOnlySignInSurface(browser, baseUrl, theme) {
  ...
  try {
    await page.addInitScript((themePreference) => {
      window.localStorage.setItem('hanji:theme', themePreference);
    }, theme);
...
async function seedStaleWorkspaceCache(page) {
  await page.addInitScript(() => {
    window.localStorage.setItem('hanji.workspaceId', 'auth-smoke-stale-workspace-id');
  });
}
...
async function newCheckedPage(browser) {
  ...
  // This smoke exercises the AuthGate form itself, so the dev runtime's
  // master auto-login (HANJI_MASTER_DEV_AUTOLOGIN) must stay out of the
  // way — same escape flag the shared harness sets.
  await context.addInitScript(() => {
    try {
      window.localStorage.setItem('hanji:disable-master-autologin', '1');
    } catch {
      // Storage unavailable: the smoke will fail loudly on the form selector.
    }
  });
