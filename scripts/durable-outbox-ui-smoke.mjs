  ...
  await pollUntil(
    () =>
      warm.evaluate(async () => {
        ...
        return !!(await caches.match('/__notionlike_precache__'));
      }),
    (ready) => ready === true,
...
async function newSeededContext(browser, seed, extraLocalStorage = {}) {
  ...
  await context.addInitScript(({ refreshToken, workspaceId, extra }) => {
  ...
  }, { refreshToken: seed.refreshToken, workspaceId: seed.workspaceId, extra: extraLocalStorage });
  return context;
}
...
async function seedPage(baseUrl) {
  ...
  return {
    ...
    outboxDbName: `notionlike-outbox:${session.userId}`,
    recordsDbName: `notionlike-records:${session.userId}`,
  };
}
