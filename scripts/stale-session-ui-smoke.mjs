...
const baseUrl = normalizeBaseUrl(
  process.env.HANJI_EDGEBASE_URL ?? 'http://127.0.0.1:8787',
);
...
async function main() {
  ...
  try {
    ...
    try {
      await context.addInitScript(({ refreshToken, refreshTokenKey }) => {
        window.localStorage.setItem(refreshTokenKey, refreshToken);
        window.localStorage.setItem('hanji.workspaceId', 'stale-session-workspace');
      }, {
