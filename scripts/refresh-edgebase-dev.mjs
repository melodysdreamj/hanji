...
const defaultBaseUrl = process.env.NOTIONLIKE_EDGEBASE_URL ?? 'http://127.0.0.1:8787';
const screenSessionName = process.env.NOTIONLIKE_EDGEBASE_DEV_SCREEN ?? 'notionlike-edgebase-dev';
...
async function stopPortRuntimeIfOwned() {
  ...
  const ownsPort = await isNotionlikeRuntimeReachable();
  if (!ownsPort) {
    throw new Error(
      `${baseUrl} is still in use, but it does not look like this Notionlike EdgeBase runtime. Stop that process manually or pass --url for another runtime.`,
    );
  }

  await killPids(pids, `Notionlike runtime listening on ${baseUrl}`);
}
...
async function waitForRuntimeReady() {
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    if (await isNotionlikeRuntimeReachable()) {
...
async function isNotionlikeRuntimeReachable() {
  try {
    ...
    return body?.ok === true && (body?.service === 'notionlike-edgebase' || body?.app === 'notionlike');
  } catch {
    return false;
  }
}
...
function parseArgs(args) {
  const parsed = {
    buildWeb: process.env.NOTIONLIKE_DEV_REFRESH_BUILD === '0' ? 'skip' : 'auto',
    url: undefined,
    keepExisting: false,
    skipVerify: false,
    timeoutMs: 60_000,
...
function printHelpAndExit() {
  console.log(`Refresh the local Notionlike EdgeBase dev runtime.

Usage:
  npm --prefix backend run dev:refresh

Options:
  --url <url>          Runtime URL. Defaults to NOTIONLIKE_EDGEBASE_URL or ${defaultBaseUrl}.
...
Environment:
  NOTIONLIKE_DEV_REFRESH_BUILD=0 disables the default frontend source freshness build check.
`);
  process.exit(0);
}
