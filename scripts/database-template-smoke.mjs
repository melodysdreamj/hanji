...
async function main() {
  ...
  const englishGenerated = await callFunction(baseUrl, owner.token, 'database-mutation', {
  ...
  createdPageIds.push(englishGenerated.page.id);
  assert(
    englishGenerated.properties.map((property) => property.name).join('|') === 'Name|Status|Tags|Date',
    'omitted locale must preserve English generated database property names',
  );
  assert(englishGenerated.views[0]?.name === 'Calendar', 'omitted locale must preserve the English view name');

  const koreanGenerated = await callFunction(baseUrl, owner.token, 'database-mutation', {
    action: 'createDatabase',
    id: crypto.randomUUID(),
    workspaceId,
    parentId: null,
  ...
  createdPageIds.push(koreanGenerated.page.id);
  assert(
    koreanGenerated.properties.map((property) => property.name).join('|') === '이름|상태|태그|날짜',
    'Korean locale must localize generated database property names',
  );
  assert(koreanGenerated.views[0]?.name === '캘린더', 'Korean locale must localize the generated view name');
  console.log('PASS generated database schema names honor en/ko locale without changing the API default.');
...
async function cleanup() {
  if (!owner?.token || createdPageIds.length === 0) return;
  const baseUrl = normalizeBaseUrl(options.url);
