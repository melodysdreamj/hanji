...
async function main() {
  ...
  // One context for both variants: EdgeBase refresh tokens rotate on use, so
  // a second context injecting the already-consumed seed token would land
  // signed out. The mobile variant reuses the session and only resizes the
  // viewport (the drawer behavior is media-query driven).
  ...
  try {
    await captureWorkspaceSwitcherVariant(page, errors, appUrl, seed, {
      prefix: 'desktop',
      viewport: { width: 1440, height: 1000 },
    });
    await captureWorkspaceSwitcherVariant(page, errors, appUrl, seed, {
    ...
    await captureLongWorkspaceListVariant(page, errors, appUrl, apiUrl, seed);

    console.log('PASS workspace switcher visuals are captured and stay within the Notion-style sidebar contract.');
    for (const name of ['desktop-menu', 'desktop-create-dialog', 'mobile-menu', 'mobile-create-dialog', 'desktop-long-list-menu']) {
...
async function captureWorkspaceSwitcherVariant(page, errors, appUrl, seed, variant) {
  {
    ...
    // "New workspace" opens the creation dialog (blank / Notion / Hanji start
    // choices) instead of an inline menu form.
    ...
    await createDialog.waitFor({ state: 'visible', timeout: options.timeoutMs });
    const input = page.getByRole('textbox', { name: 'Workspace name' });
    await input.fill(seed.createDraftName, { timeout: options.timeoutMs });
    await page.screenshot({
      path: join(options.screenshotDir, `${variant.prefix}-create-dialog.png`),
      fullPage: false,
    });
    await assertWorkspaceCreateDialogContract(page, seed, `${variant.prefix} workspace create dialog`);
    await page.keyboard.press('Escape');
    await createDialog.waitFor({ state: 'hidden', timeout: options.timeoutMs });
...
async function captureLongWorkspaceListVariant(page, errors, appUrl, apiUrl, seed) {
  // Notion contract: with many workspaces, the workspace list scrolls inside
  // the menu while the menu stays inside the viewport and the bottom command
  // rows (New workspace, consoles, Templates, Import, Trash, Log out) remain
  // visible without scrolling the whole menu. Seed names sort before the
  // active workspace so the open-time scroll-into-view path is exercised too.
  await seedLongWorkspaceList(apiUrl, seed, 16);
  await page.setViewportSize({ width: 1440, height: 720 });
  await openWorkspaceHome(page, appUrl, seed);
  let menu = await openWorkspaceMenu(page);
  await waitForWorkspaceMenuState(menu, seed);
  // The freshly seeded workspaces can land after the menu first opened (the
  // app renders the cached workspace list first). The open-time
  // scroll-into-view contract only applies to a menu opened over the full
  // list, so wait for the last seeded name and reopen.
  const lastSeededName = seed.longListWorkspaceNames[seed.longListWorkspaceNames.length - 1];
  await menu.getByText(lastSeededName, { exact: false }).first().waitFor({
    state: 'visible',
    timeout: options.timeoutMs,
  });
  await page.keyboard.press('Escape');
  ...
  await page.screenshot({
    path: join(options.screenshotDir, 'desktop-long-list-menu.png'),
    fullPage: false,
  });

  const metrics = await page.evaluate(() => {
    const menu = document.querySelector('[role="menu"][aria-label="Workspace menu"]');
    const group = menu?.querySelector('[class*="workspaceMenuGroup"]');
    if (!(menu instanceof HTMLElement) || !(group instanceof HTMLElement)) {
      return { ok: false, reason: 'missing workspace menu or workspace list group' };
    }
    const menuRect = menu.getBoundingClientRect();
    const groupRect = group.getBoundingClientRect();
    const active = group.querySelector('[data-workspace-menu-item][data-active="true"]');
    const activeRect = active instanceof HTMLElement ? active.getBoundingClientRect() : null;
    const hiddenCommandRows = Array.from(menu.querySelectorAll(':scope > [class*="workspaceMenuItem"]'))
      .filter((item) => {
        const rect = item.getBoundingClientRect();
        return rect.height === 0 || rect.top < 0 || rect.bottom > window.innerHeight;
      })
      .map((item) => item.textContent?.trim().slice(0, 60) || item.tagName);
    return {
      ok: true,
      activeVisibleInGroup: Boolean(
        activeRect && activeRect.bottom > groupRect.top + 1 && activeRect.top < groupRect.bottom - 1,
      ),
  ...
  assert(metrics.ok, metrics.reason ?? 'long workspace list contract could not run');
  assert(
    metrics.menuTop >= 0 && metrics.menuBottom <= metrics.viewportHeight - 8,
    `long workspace list menu should stay inside the viewport, got top=${Math.round(metrics.menuTop)} bottom=${Math.round(metrics.menuBottom)} in ${metrics.viewportHeight}px`,
  );
  assert(
    metrics.groupScrollHeight > metrics.groupClientHeight + 8,
    `long workspace list should scroll inside the menu, got scrollHeight=${metrics.groupScrollHeight} clientHeight=${metrics.groupClientHeight}`,
  );
  assert(
    metrics.hiddenCommandRows.length === 0,
    `bottom command rows must stay visible with a long workspace list, hidden: ${metrics.hiddenCommandRows.join(' | ')}`,
  );
  assert(metrics.activeVisibleInGroup, 'the active workspace should be scrolled into view when the menu opens');

  await page.keyboard.press('Escape');
  await menu.waitFor({ state: 'hidden', timeout: options.timeoutMs });

  // Outside-click dismissal: the backdrop must cover the whole viewport, not
  // only the sidebar box (a transform on the sidebar would shrink it there),
  // so a click in the editor area has to close the menu.
  menu = await openWorkspaceMenu(page);
  await page.mouse.click(1000, 400);
  await menu.waitFor({ state: 'hidden', timeout: options.timeoutMs });

  assertNoBrowserErrors(errors, 'long workspace list menu');
}

async function seedLongWorkspaceList(baseUrl, seed, count) {
  seed.longListWorkspaceIds = [];
  seed.longListWorkspaceNames = [];
  for (let i = 0; i < count; i += 1) {
    // "Aa ..." sorts before the anonymous default workspace name, pushing the
    // active row below the fold so the menu must scroll it into view.
    const name = `Aa long list ws ${String(i + 1).padStart(2, '0')}`;
    const created = await callFunction(baseUrl, seed.accessToken, 'workspace-mutation', {
    ...
    assert(id, `long workspace list seeding must create "${name}"`);
    seed.longListWorkspaceIds.push(id);
    seed.longListWorkspaceNames.push(name);
  }
}

async function openMobileSidebar(page) {
  // The mobile drawer renders the sidebar with role="dialog", so match by
  // element + aria-label instead of the desktop complementary role.
...
async function assertWorkspaceSwitcherContract(page, menu, seed, variant, label) {
  ...
  assert(metrics.menuItemCount >= 8, `${label} should show workspace and command rows, got ${metrics.menuItemCount}`);
  ...
  if (variant.mobile) {
    ...
    assert(metrics.sidebarWidth >= 220 && metrics.sidebarWidth <= 300, `${label} desktop sidebar should keep Hanji width, got ${Math.round(metrics.sidebarWidth)}px`);
...
async function assertWorkspaceCreateDialogContract(page, seed, label) {
  const metrics = await page.evaluate((draft) => {
    const input = document.getElementById('workspace-create-name');
    const dialog = input instanceof HTMLElement ? input.closest('[role="dialog"]') : null;
    if (!(input instanceof HTMLInputElement) || !(dialog instanceof HTMLElement)) {
      return { ok: false, reason: 'missing workspace create dialog or name input' };
    }
    ...
    return {
      ...
      checkedCount: radios.filter((item) => item.getAttribute('aria-checked') === 'true').length,
      dialogWidth: rect.width,
      dialogTop: rect.top,
      dialogBottom: rect.bottom,
      viewportWidth: window.innerWidth,
      viewportHeight: window.innerHeight,
      draftMatches: input.value === draft,
    };
  }, seed.createDraftName);

  assert(metrics.ok, metrics.reason ?? `${label} contract could not run`);
  assert(metrics.draftMatches, `${label} should preserve the typed workspace draft name`);
  assert(metrics.inputHeight >= 28 && metrics.inputHeight <= 40, `${label} name input should stay compact, got ${Math.round(metrics.inputHeight)}px`);
  assert(metrics.radioCount === 3, `${label} should offer blank/Notion/Hanji start choices, got ${metrics.radioCount}`);
  assert(metrics.checkedCount === 1, `${label} should keep exactly one start choice selected, got ${metrics.checkedCount}`);
  assert(metrics.dialogWidth >= 300 && metrics.dialogWidth <= 480, `${label} dialog width should stay compact, got ${Math.round(metrics.dialogWidth)}px`);
  assert(metrics.dialogTop >= 0 && metrics.dialogBottom <= metrics.viewportHeight, `${label} dialog should stay inside the viewport`);
}
...
async function cleanupSeed(baseUrl, seed) {
  ...
  const ids = [seed.extraWorkspaceId, ...(seed.longListWorkspaceIds ?? [])].filter(Boolean);
