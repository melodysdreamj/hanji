...
async function cleanup() {
  for (const id of created.source) {
    if (sourceToken) await callFunction(baseUrl, sourceToken, 'page-mutation', { action: 'delete', id }).catch(() => {});
  }
  for (const id of created.imported) {
    if (importToken) await callFunction(baseUrl, importToken, 'page-mutation', { action: 'delete', id }).catch(() => {});
  }
}
...
function parseArgs(args) {
  const parsed = { url: process.env.NOTIONLIKE_EDGEBASE_URL ?? DEFAULT_BASE_URL, timeoutMs: 12_000, stamp: Date.now() };
