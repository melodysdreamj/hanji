...
async function main() {
  ...
  owner = await signIn(baseUrl, 'owner');
  viewer = await signIn(baseUrl, 'viewer');
  ...
  const ownerFileBytes = new TextEncoder().encode('hanji file smoke owner');
  ...
  const viewerCompleted = await uploadPreparedFile(
    baseUrl,
    viewer.token,
    viewerPrepared,
  ...
  assert(viewerCompleted?.status === 'uploaded', 'edit access must allow completing its upload');
  ...
  const cleanupCompleted = await uploadPreparedFile(
    baseUrl,
    owner.token,
    cleanupPrepared,
  ...
  assert(
    cleanupCompleted?.status === 'uploaded',
    'page-delete cleanup fixture must consume its one-time upload grant first',
  );
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--password-signup') {
      parsed.passwordSignup = true;
      continue;
    }
...
and cleanup dry-run against a running Hanji EdgeBase runtime.
...
Options:
  ...
  --password-signup       Create synthetic email accounts instead of using dev-only anonymous auth.
`);
}
...
async function signIn(baseUrl, label) {
  if (options.passwordSignup) {
    const suffix = crypto.randomUUID();
    const email = `file-smoke-${label}-${suffix}@example.com`;
    const password = `FileSmoke-${suffix}-Aa!7`;
    const response = await fetchWithTimeout(resolveUrl(baseUrl, '/api/auth/signup'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
      body: JSON.stringify({ email, password, data: { displayName: `File smoke ${label}` } }),
    });
    const body = await readJson(response);
    assert(
      response.status === 201 || response.ok,
      `password signup returned HTTP ${response.status}: ${JSON.stringify(body)}`,
    );
