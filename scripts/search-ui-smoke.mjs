...
async function expectRecentSearchStored(page, query) {
  try {
    await page.waitForFunction(
      (expected) => {
        try {
          const parsed = JSON.parse(window.localStorage.getItem('notionlike:quick-find:recent-searches') ?? '[]');
    ...
    const snapshot = await page.evaluate(() => ({
      value: window.localStorage.getItem('notionlike:quick-find:recent-searches'),
