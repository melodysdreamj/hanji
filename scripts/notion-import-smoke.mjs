...
const DEFAULT_MOCK_NOTION_API_BASE = process.env.NOTIONLIKE_MOCK_NOTION_API_BASE ?? '';
const DEFAULT_TIMEOUT_MS = 8_000;
const DEFAULT_EXPECT_STORED_CONNECTION = process.env.NOTIONLIKE_EXPECT_STORED_NOTION_CONNECTION === '1';
...
async function main() {
  ...
  const inferredKoreanPurchaseInvoiceView = (taskSnapshot.views ?? []).find((view) => view.name === '매입 계산서');
  ...
  assert(inferredKoreanPurchaseInvoiceView, 'Notion import apply must create the Korean purchase invoice inferred-filter view');
  ...
  assert(
    inferredKoreanPurchaseInvoiceView.config?.filterGroup?.filters?.[0]?.propertyId === fundingTypeProp.id &&
      inferredKoreanPurchaseInvoiceView.config.filterGroup.filters[0].operator === 'equals' &&
      inferredKoreanPurchaseInvoiceView.config.filterGroup.filters[0].value === 'funding-tax-purchase-ko' &&
      inferredKoreanPurchaseInvoiceView.config?.inferredFilter?.inferredFrom === 'view_name_select_option',
    'Imported purchase-invoice views with filter:null and quick_filters:null must infer the parenthetical Korean select option',
  );
  ...
  assert(
    Array.isArray(alphaTask.properties?.[assetsProp.id]) &&
      ...
      alphaTask.properties[assetsProp.id].some((file) => file.sourceUrl === SYNTHETIC_PDF_DATA_URL) &&
      alphaTask.properties[assetsProp.id].some((file) => file.notionFileExpiryTime === '2026-03-11T00:00:00.000Z'),
    'Imported files property must copy external and Notion-hosted file references into EdgeBase storage',
  );
  assert(
  ...
  const fileBlock = (rootBlocks.blocks ?? []).find((block) => block.content?.sourceUrl === SYNTHETIC_PDF_DATA_URL);
  assert(fileBlock, 'Imported page must include preserved file reference block');
  ...
  assert(
    fileBlock.content?.notionFileExpiryTime === '2026-03-11T00:00:00.000Z',
    'Imported file blocks must preserve Notion-hosted file expiry metadata',
  );
  assert(
  ...
  assert(
    mcpSnapshot.items?.some((item) => item.notionObject === 'data_source' && item.notionId === '26215f4d-0109-437d-bbb3-034dad275d3e'),
  ...
  const mcpDatabaseId = mappingLocalId(mcpApplied, '26215f4d-0109-437d-bbb3-034dad275d3e', 'database');
  ...
  const mcpAmountProp = propByName(mcpProps, '입력금액');
  const mcpContractRelationProp = propByName(mcpProps, '계약DB');
  const mcpRollupProp = propByName(mcpProps, '계약 - 거래처');
  const mcpDateProp = propByName(mcpProps, '날짜');
  const mcpView = (mcpDatabase.views ?? []).find((view) => view.name === 'Default view');
  ...
  assert(
    mcpContractRelationProp.type === 'relation' &&
      mcpContractRelationProp.config?.relationTargetNotionId === '79914dc6-e2a2-4833-b287-719baed76013',
    'MCP-assisted Notion import must preserve relation targets from collection schema',
  );
  assert(
    mcpRollupProp.type === 'rollup' &&
      mcpRollupProp.config?.rollupRelationPropertyNotionId === 'PD54Zg' &&
      mcpRollupProp.config?.rollupTargetPropertyNotionId === 'TV91dQ',
  ...
  const mcpRowPageId = mappingLocalId(mcpApplied, '3351fd67-9738-81fa-90b9-e560bf0d83fc', 'page');
  ...
  assert(mcpRowPage.page?.properties?.[mcpAmountProp.id] === 600000, 'MCP-assisted Notion row snapshots must preserve number values');
  assert(mcpRowPage.page?.properties?.[propByName(mcpProps, '진행상태').id] === '완료', 'MCP-assisted Notion row snapshots must preserve select labels');
  assert(
    mcpRowPage.page?.properties?.[mcpDateProp.id]?.start === '2026-04-01',
  ...
  assert(
    directMcpSnapshot.items?.some((item) => item.notionObject === 'data_source' && item.notionId === '550500d6-8f8f-4f38-ac42-feed7f3f8592'),
  ...
  const directMcpDatabaseId = mappingLocalId(directMcpApply, '550500d6-8f8f-4f38-ac42-feed7f3f8592', 'database');
  const directMcpDatabase = await callFunction(baseUrl, owner.token, 'page-query', {
    action: 'database',
    databaseId: directMcpDatabaseId,
  });
  assert(propByName(directMcpDatabase.properties ?? [], '보고서명').type === 'title', 'direct MCP data source snapshots must preserve title properties');
  assert(propByName(directMcpDatabase.properties ?? [], '종류').type === 'select', 'direct MCP data source snapshots must preserve select properties');
              ...
              'Funding type': {
                ...
                select: {
                  options: [
	                    ...
	                    { id: 'funding-tax-purchase-ko', name: '세금계산서(매입)', color: 'purple' },
	                  ],
                },
              },
              Project: {
...
function syntheticMcpHiddenDatabaseFetch() {
  return {
    ...
    title: '💰 입출금 관리',
    url: 'https://app.notion.com/p/b691fd67973882ff9fc90124c25cee6b',
    text: `<database url="{{https://app.notion.com/p/b691fd67973882ff9fc90124c25cee6b}}" inline="true">
The title of this Database is: 💰 입출금 관리
<data-sources>
<data-source url="{{collection://26215f4d-0109-437d-bbb3-034dad275d3e}}">
The title of this Data Source is: 💰 입출금 관리
<data-source-state>
${JSON.stringify({
  name: '💰 입출금 관리',
  url: '{{collection://26215f4d-0109-437d-bbb3-034dad275d3e}}',
  schema: {
    건명: { name: '건명', type: 'title', propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/title' },
    날짜: { name: '날짜', type: 'date', propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/date' },
    첨부파일: { name: '첨부파일', type: 'file', propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/file' },
    메모: { name: '메모', type: 'text', propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/memo' },
    입력금액: {
      name: '입력금액',
      type: 'number',
      number_format: 'won',
      propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/amount',
    },
    진행상태: {
      name: '진행상태',
      type: 'select',
      propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/status',
      options: [
        { id: 'status-open', name: '진행', color: 'blue' },
        { id: 'status-done', name: '완료', color: 'green' },
      ],
    },
    계약DB: {
      name: '계약DB',
      type: 'relation',
      propertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/PD54Zg',
      dataSourceUrl: 'collection://79914dc6-e2a2-4833-b287-719baed76013',
    },
    '계약 - 거래처': {
      name: '계약 - 거래처',
      type: 'rollup',
      relationPropertyUrl: 'collectionProperty://26215f4d-0109-437d-bbb3-034dad275d3e/PD54Zg',
      targetPropertyUrl: 'collectionProperty://79914dc6-e2a2-4833-b287-719baed76013/TV91dQ',
      function: 'show_original',
    },
  },
})}
...
<views>
<view url="{{view://9ae1fd67-9738-838d-a435-08d62df85bbf}}">
${JSON.stringify({
  dataSourceUrl: '{{collection://26215f4d-0109-437d-bbb3-034dad275d3e}}',
  displayProperties: ['건명', '날짜', '첨부파일', '메모', '입력금액'],
  name: 'Default view',
  sorts: [{ direction: 'descending', property: '날짜' }],
  type: 'table',
})}
</view>
</views>
...
function syntheticMcpDirectDataSourceFetch() {
  return {
    ...
    title: '보고서',
    url: 'https://app.notion.com/p/54b32b083a0649e3903e8d5c1fe2beca',
    text: `<data-source url="{{collection://550500d6-8f8f-4f38-ac42-feed7f3f8592}}">
The title of this Data Source is: 보고서
<data-source-state>
${JSON.stringify({
  name: '보고서',
  url: '{{collection://550500d6-8f8f-4f38-ac42-feed7f3f8592}}',
  schema: {
    보고서명: { name: '보고서명', type: 'title' },
    종류: {
      name: '종류',
      type: 'select',
      options: [
        { id: 'report-research', name: '리서치·탐색', color: 'blue' },
        { id: 'report-finance', name: '재무·자금', color: 'green' },
      ],
    },
    보고일: { name: '보고일', type: 'date' },
  },
})}
</data-source-state>
</data-source>`,
...
function syntheticMcpRowPageFetch() {
  return {
    metadata: { type: 'page' },
    title: '🧾 [세금계산서(매입)] 임대료 - 시온테크 (2026년 3월)',
    url: 'https://app.notion.com/p/3351fd67973881fa90b9e560bf0d83fc',
    text: `<page url="https://app.notion.com/p/3351fd67973881fa90b9e560bf0d83fc" icon="🧾">
<ancestor-path>
<parent-data-source url="collection://26215f4d-0109-437d-bbb3-034dad275d3e" name="입출금 관리"/>
</ancestor-path>
<properties>
${JSON.stringify({
  'date:날짜:is_datetime': 0,
  'date:날짜:start': '2026-04-01',
  url: 'https://app.notion.com/p/3351fd67973881fa90b9e560bf0d83fc',
  건명: '[세금계산서(매입)] 임대료 - 시온테크 (2026년 3월)',
  계약DB: ['https://app.notion.com/p/32f1fd679738813dade9e8d1c9b8bb5a'],
  메모: '3월분 임대료 600,000 + 부가세 60,000',
  입력금액: 600000,
  자금유형: '세금계산서(매입)',
  진행상태: '완료',
  첨부파일: [SYNTHETIC_PDF_DATA_URL],
})}
</properties>
...
async function verifyMockNotionStoredConnection(baseUrl, token, workspaceId, organizationId, expectStoredConnection) {
  ...
  if (!response.ok) {
    ...
    if (!expectStoredConnection && message.includes('NOTIONLIKE_NOTION_IMPORT_SECRET')) {
      console.log('SKIP Notion stored connection smoke because NOTIONLIKE_NOTION_IMPORT_SECRET is not configured.');
      return;
    }
...
async function verifyMockNotionOAuthConnection(baseUrl, token, workspaceId, organizationId, expectOAuthConnection) {
  ...
  if (!beginResponse.ok) {
    ...
    if (
      ...
      (
        message.includes('NOTIONLIKE_NOTION_OAUTH_CLIENT_ID') ||
        message.includes('NOTIONLIKE_NOTION_OAUTH_CLIENT_SECRET')
      )
    ) {
  ...
  if (!completeResponse.ok) {
    ...
    if (!expectOAuthConnection && message.includes('NOTIONLIKE_NOTION_IMPORT_SECRET')) {
      console.log('SKIP Notion OAuth completion smoke because NOTIONLIKE_NOTION_IMPORT_SECRET is not configured.');
      return;
    }
...
async function verifyMockNotionDatabaseDataSourceDiscovery(baseUrl, token, workspaceId, organizationId) {
  ...
  assert(
    linkedContextView?.config?.notionlikeImportedRowContextFilter === true &&
    ...
    'Mock database-only import must persist row-context linked database filters during migration apply',
	  ...
	  assert(
	    importedTemplateLinkedDbBlock?.content?.childPageId === linkedDatabaseId &&
	      ...
	      templateQuickFilter?.value?.kind === 'notionlike.current_page' &&
...
retry controls against a running Notionlike EdgeBase runtime. It does not call
...
Options:
  ...
  --mock-notion-api-base <url>
                          ...
                          The EdgeBase runtime must also be started with NOTIONLIKE_NOTION_API_BASE=<url>.
  --expect-stored-connection
                          Require encrypted stored Notion connection checks to pass.
                          The EdgeBase runtime must have NOTIONLIKE_NOTION_IMPORT_SECRET configured.
