...
async function main() {
  ...
  try {
    ...
    if (options.onlyRowContextMenu) {
      ...
      console.log('PASS database rows open the compact row menu with Hanji row actions.');
...
async function assertDatabaseFilterSelectChrome(browser, baseUrl, seed) {
  ...
  try {
    ...
    await closeSeededContext(lowViewport.context, seed);
  }
}
...
async function assertNarrowDatabasePageCanvas(browser, baseUrl, seed) {
  ...
  try {
    ...
    assert(
      ...
      `narrow database row height should keep a Hanji compact rhythm: ${JSON.stringify(metrics)}`,
    );
...
async function assertToolbarLayoutMenuClickThrough(page, seed) {
  ...
  assert(
    ...
    `Board layout settings should include Hanji card controls: ${boardSettings}`,
  );
  ...
  assert(
    ...
    `Timeline layout settings should include Hanji timeline controls: ${timelineSettings}`,
  );
...
async function assertToolbarFilterSelectClickThrough(page, seed, screenshotSuffix = '') {
  ...
  await page.getByRole('menuitemradio', { name: '샘플연결', exact: true }).click({ timeout: options.timeoutMs });
  await page.waitForFunction(
    () =>
      document
  ...
  const valueButton = row.locator('button[aria-label="Filter value for 샘플연결"]');
  ...
  const adjacentMenuState = await page.evaluate(() => {
    ...
    return {
      ...
      hasValueMenu: menus.some((menu) => menu.label === 'Filter value for 샘플연결' && menu.text.includes('Current page')),
  ...
  await page.waitForFunction(
    () =>
      document
        .querySelector('[data-filter-row] button[aria-label="Filter value for 샘플연결"]')
        ?.textContent
...
async function assertMobileDatabaseViewVisualContract(page, seed, view) {
  ...
  assert(
    // A trailing fragment on a genuinely scrollable rail is a scroll
    // affordance, not breakage (and its exact width shifts with platform font
    // metrics); only a non-scrollable rail must show every tab in full.
...
function databaseViewReferenceInventory() {
  ...
  return {
    ...
    normalizedContract: {
      ...
      note: 'Notion reference artifacts calibrate hierarchy, density, and reveal behavior. Local implementation should use Hanji tokens and responsive rules rather than copying raw Notion DOM geometry.',
    },
  };
}
...
async function seedDatabase(baseUrl, authBaseUrl = baseUrl, existingSession) {
  ...
  const rowTitles = [
    '한빛도자기상사 (Hanbit Ceramics Trading Co.) 장기 미수 거래처 정리 계획',
    '월별 세금계산서 발행 확인',
    '대표님 보고용 지표 마감',
  ];
...
async function seedSession(context, seed) {
  // Shared harness install: first context bootstraps from the API-issued
  // refresh token; later contexts transplant the rotated HttpOnly cookie
  // captured in openDatabase (rotation reuse detection forbids replaying the
...
Options:
  --url <url>                 App runtime URL. Defaults to HANJI_EDGEBASE_URL or ${DEFAULT_BASE_URL}.
  --api-url <url>             API runtime URL. Defaults to HANJI_EDGEBASE_API_URL or ${DEFAULT_BASE_URL}.
