...
const DEFAULT_APP_URL = process.env.HANJI_EDGEBASE_APP_URL;
...
const DEFAULT_NOTION_IMPORT_SECRET =
  ...
  'hanji-nonvisual-import-secret-use-real-secret-outside-tests';
...
async function ensureRuntime() {
  if (await isRuntimeReachable()) {
    ...
    if (canAssumeMockNotionRuntime()) {
      ...
      console.log('SKIP strict mock Notion import checks for reused runtime; start it with HANJI_NOTION_API_BASE to enable them.');
    }
    return;
  }
...
async function runLiveNotionImportSmoke() {
  ...
  if (!hasLiveNotionToken()) {
    ...
    console.log('SKIP live Notion template smoke because NOTION_TOKEN or HANJI_NOTION_TOKEN is not configured.');
    return;
  }
  if (strictNotionImportChecks || canAssumeMockNotionRuntime()) {
    console.log('\n> optional live Notion template import/apply/share smoke');
...
function hasLiveNotionToken() {
  return Boolean(process.env.NOTION_TOKEN?.trim() || process.env.HANJI_NOTION_TOKEN?.trim());
}

function canAssumeMockNotionRuntime() {
  return normalizeMaybeUrl(process.env.HANJI_NOTION_API_BASE) === mockNotionApiBase;
}
...
function parseArgs(args) {
  const parsed = {
    ...
    expectStoredNotionConnection: process.env.HANJI_EXPECT_STORED_NOTION_CONNECTION === '1',
...
Runs the non-visual Hanji verification suite:
- web build/typecheck
- web lint
...
- optional live Notion API discovery/review smoke when NOTION_TOKEN or HANJI_NOTION_TOKEN is configured
...
Options:
  ...
  --app-url <url>                App URL for browser UI checks. Defaults to HANJI_EDGEBASE_APP_URL or --url.
