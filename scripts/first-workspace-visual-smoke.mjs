...
async function assertStaleSigninVisualContract(page, createdPageId) {
  ...
  assert(
    ...
    `stale sign-in sidebar should keep a Notion-like width, got ${Math.round(metrics.sidebarWidth)}px`,
  );
  assert(
...
async function assertFirstPageDatabaseStarterRuntime(page, parentPageId) {
  ...
  assert(
    metrics.addPropertyButtonLabel === 'Add a property' &&
      metrics.headerChildCount === 2,
    `new starter database should start with Name plus Add property, not default Status/Tags columns: ${JSON.stringify(metrics)}`,
  );
  assert(
  ...
  assert(
    !/\bUntitled\b/.test(metrics.bodyText) &&
    ...
    `new starter database document should not show fallback empty-state/Untitled/Status/Tags copy: ${JSON.stringify(metrics.bodyText)}`,
  );
}
...
async function seedStaleWorkspaceCache(page) {
  await page.addInitScript(() => {
    window.localStorage.setItem('notionlike.workspaceId', 'first-workspace-stale-workspace-id');
