...
async function assertSharedAccountProductState(apiUrl, seed) {
  ...
  await expectFunctionStatus(apiUrl, invitee.accessToken, 'block-mutation', {
    action: 'update',
    id: seed.blockId,
    // Post-DO-migration routing needs the pageId hint before the access
    // check can produce the expected 403 (otherwise routing 400s first).
    pageId: seed.pageId,
    patch: {
  ...
  await expectFunctionStatus(apiUrl, invitee.accessToken, 'file-mutation', {
    ...
    // Routing hint only; access to the hidden page must still be denied.
    pageId: seed.hiddenPageId,
  }, 403);
