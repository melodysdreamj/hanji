...
async function openEditorMentionMenu(page, seed) {
  ...
  // Earlier menu captures may have durably saved their trigger text. Start
  // each browser context from an empty editable so this probe always tests one
  // user-entered `@`, never an accumulated `@@` that is not a mention trigger.
  await editable.fill('', { timeout: options.timeoutMs });
