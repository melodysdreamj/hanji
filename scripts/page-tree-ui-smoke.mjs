...
const PAGE_DRAG_TYPE = 'application/x-notionlike-page-id';
...
async function main() {
  ...
  try {
    ...
    if (options.onlySidebarCollapseMotion) {
      ...
      console.log('PASS desktop sidebar collapse and expand use Notion-like panel slide motion.');
      return;
    }
...
async function assertSidebarCollapseExpandMotion(page) {
  ...
  assert(
    ...
    `desktop sidebar should start near the Notion-like 270px width: ${JSON.stringify(before)}`,
  );
  assert(
  ...
  assert(
    ...
    `closed sidebar panel should keep its Notion-like real width offscreen: ${JSON.stringify(closed)}`,
  );
  ...
  return {
    ...
    normalizedContract: {
      ...
      localDeviationPolicy:
        'Use Notionlike labels and responsive tokens, but preserve the reference surface inventory and reveal rules.',
    },
  };
}
...
async function assertMobileNestedSidebarTreeVisualContract(page, seed) {
  ...
  assert(
    ...
    `mobile sidebar drawer should keep the 280px Notion-like width, got ${Math.round(metrics.sidebarWidth)}px`,
  );
...
async function seedPageTree(baseUrl) {
  ...
  const siblingTitle = `에스제일산후조리원 주식회사 물품공급계약 ${suffix}`;
  ...
  const childDatabase = await callFunction(baseUrl, session.accessToken, 'database-mutation', {
    ...
    seedRows: false,
    properties: [
      { id: randomUUID(), name: '거래처명', type: 'title', position: 1 },
    ],
  });
...
async function seedSession(context, seed) {
  await context.addInitScript(({ refreshToken, workspaceId }) => {
    ...
    window.localStorage.removeItem('notionlike:sidebar-section-collapsed');
  }, {
    refreshToken: seed.refreshToken,
    workspaceId: seed.workspaceId,
  });
