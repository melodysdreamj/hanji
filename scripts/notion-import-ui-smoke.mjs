...
try {
  ...
  if (message.includes('fetch failed') || message.includes('ECONNREFUSED')) {
    console.error('Start the local EdgeBase runtime with NOTIONLIKE_NOTION_API_BASE pointing at the mock API base.');
  }
  process.exitCode = 1;
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log('PASS Notion import UI shows token setup guidance, discovers, auto-refreshes job progress, expands discovery, reviews, and checks stored connections when available without screenshots.');
    } finally {
...
async function assertNotionImportUi(browser, appUrl, apiUrl, seed) {
  ...
  try {
    ...
    await step('discover import from UI (step-3 Progress panel)', async () => {
      // The recent-jobs history list was removed; the import a user starts binds
      // to the step-3 Progress panel (role="status") via the session job.
...
async function expectStatusText(page, parts) {
  await page.waitForFunction(
    ...
    const text = await page.locator('[role="status"]').first().textContent({ timeout: 1000 }).catch(() => '');
    throw new Error(
      `Expected step-3 Progress status to include ${parts.map((part) => JSON.stringify(part)).join(', ')}, got ${JSON.stringify((text ?? '').trim())}: ${error.message}`,
    );
  });
}
...
async function assertStoredTokenConnectionUi(page, apiUrl, seed) {
  ...
  if (probe.connectionStorageAvailable === false) {
    if (options.expectStoredConnection) {
      throw new Error('Stored Notion connection UI requires NOTIONLIKE_NOTION_IMPORT_SECRET, but the runtime reports connection storage as unavailable.');
    }
    ...
    console.log('SKIP Notion import UI stored connection smoke because connection storage is unavailable (NOTIONLIKE_NOTION_IMPORT_SECRET is not configured); verified the save UI stays hidden.');
    return false;
  }
...
async function waitForSavedConnectionOrMissingSecret(page, apiUrl, seed, connectionName) {
  ...
  while (Date.now() < deadline) {
    ...
    if (bodyText?.includes('NOTIONLIKE_NOTION_IMPORT_SECRET')) {
      if (options.expectStoredConnection) {
        throw new Error('Stored Notion connection UI requires NOTIONLIKE_NOTION_IMPORT_SECRET, but the runtime rejected the save.');
      }
      console.log('SKIP Notion import UI stored connection smoke because NOTIONLIKE_NOTION_IMPORT_SECRET is not configured.');
      return null;
    }
...
function parseArgs(args) {
  const parsed = {
    ...
    expectStoredConnection: process.env.NOTIONLIKE_EXPECT_STORED_NOTION_CONNECTION === '1',
    headed: false,
...
Start EdgeBase with NOTIONLIKE_NOTION_API_BASE set to the same mock API base.
