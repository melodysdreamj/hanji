...
try {
  ...
  await runAuthorizationCodeFlow(seed, { verifyReplay: true });

  await verifyHostedMcpCoreTools(
    connection.json.createdToken.accessToken,
    seed.workspaceId,
    seed.accessToken,
  );
} finally {
...
async function mcpToolResult(accessToken, name, args = {}) {
...
async function mcpTool(accessToken, name, args = {}) {
  const { result, response } = await mcpToolResult(accessToken, name, args);
  assert(result.isError !== true, `${name} returned tool error: ${result.content?.[0]?.text ?? response.text}`);
  return result.structuredContent ?? JSON.parse(result.content?.[0]?.text ?? "{}");
}

function mcpToolErrorText(result) {
  return result.content?.map((item) => item?.text ?? "").join("\n") ?? "";
}

async function expectMcpToolUnavailable(accessToken, name, args) {
  const { result } = await mcpToolResult(accessToken, name, args);
  const text = mcpToolErrorText(result);
  assert(result.isError === true, `${name} must fail closed until canonical content mutations are wired`);
  assert(
    text.includes("content mutations are not available in this release"),
    `${name} must explain its fail-closed content-mutation boundary: ${text}`,
  );
}

async function verifyHostedMcpCoreTools(accessToken, workspaceId, productAccessToken) {
  ...
  const listedTools = listed.json.result?.tools ?? [];
  ...
  for (const name of [
    ...
    assert(
      description.includes("unsupported") && description.includes("canonical file lifecycle"),
      `${name} description must disclose the current canonical file-lifecycle boundary`,
    );
  }
  ...
  assert(
    typeof createdPageTask.async_task?.status_url === "string" && createdPageTask.async_task.status_url,
    "notion-create-pages failed async task must advertise a status URL",
  );
  const advertisedTaskResponse = await fetch(createdPageTask.async_task.status_url, {
    headers: {
      accept: "application/json",
      authorization: `Bearer ${accessToken}`,
    },
  });
  const advertisedTaskText = await advertisedTaskResponse.text();
  const advertisedTask = JSON.parse(advertisedTaskText || "{}");
  assert(
    advertisedTaskResponse.ok,
    `advertised async status URL returned HTTP ${advertisedTaskResponse.status}: ${advertisedTaskText}`,
  );
  assert(
    advertisedTask.async_task?.status === "failed",
  ...
  // advertising the official tool names, but primary Notion-compatible writes
  // deliberately fail closed until they share this exact file lifecycle.
  const pageId = crypto.randomUUID();
  const seededPage = await callFunction(
    resolveUrl(baseUrl, "/api/functions/page-mutation"),
    productAccessToken,
    {
      action: "create",
      id: pageId,
  ...
  assert(seededPage.response.ok, `canonical page seed returned HTTP ${seededPage.response.status}: ${seededPage.text}`);
  assert(seededPage.json?.page?.id === pageId, "canonical page seed must return the created page id");
  const blockId = crypto.randomUUID();
  const seededBlock = await callFunction(
    resolveUrl(baseUrl, "/api/functions/block-mutation"),
    productAccessToken,
    {
      action: "create",
      id: blockId,
      ...
      parentId: null,
      type: "paragraph",
      content: { rich: [{ text: `This page was seeded for hosted MCP ${unique}.` }] },
      plainText: `This page was seeded for hosted MCP ${unique}.`,
      position: 1,
    },
  );
  assert(seededBlock.response.ok, `canonical block seed returned HTTP ${seededBlock.response.status}: ${seededBlock.text}`);
  assert(seededBlock.json?.block?.id === blockId, "canonical block seed must return the created block id");
  ...
  assert(updatePolled.async_task?.status === "failed", "notion-update-page must record a failed async task");
  assert(
    updatePolled.async_task?.error?.message?.includes("content mutations are not available in this release"),
    "notion-update-page failed task must explain the canonical content-mutation boundary",
  );
  ...
  assert(
    ...
    "notion-search must find the page seeded through the canonical product API",
  );
  ...
  assert(fetched.page?.id === pageId, "notion-fetch must return the canonical product API seed");
  assert(Array.isArray(fetched.blocks?.results), "notion-fetch must include the seeded page block children");
  ...
  await expectMcpToolUnavailable(accessToken, "notion-create-database", {
    workspace_id: workspaceId,
    title: `${unique} Tasks`,
    properties: {
      Name: { title: {} },
  ...
  const seededDatabase = await callFunction(
    resolveUrl(baseUrl, "/api/functions/database-mutation"),
    productAccessToken,
    {
  ...
  assert(
    seededDatabase.response.ok,
    `canonical database seed returned HTTP ${seededDatabase.response.status}: ${seededDatabase.text}`,
  );
  assert(seededDatabase.json?.page?.id === databaseId, "canonical database seed must return database id");
  const statusPropertyId = crypto.randomUUID();
  const seededProperty = await callFunction(
    resolveUrl(baseUrl, "/api/functions/database-mutation"),
    productAccessToken,
    {
      action: "insert",
      ...
      record: {
        ...
        config: {
          options: [
            { id: crypto.randomUUID(), name: "진행", color: "blue" },
  ...
  assert(
    seededProperty.response.ok,
    `canonical database property seed returned HTTP ${seededProperty.response.status}: ${seededProperty.text}`,
  );

  await expectMcpToolUnavailable(accessToken, "notion-update-data-source", {
    workspace_id: workspaceId,
  ...
  await expectMcpToolUnavailable(accessToken, "notion-create-pages", {
    workspace_id: workspaceId,
  ...
  const seededRow = await callFunction(
    resolveUrl(baseUrl, "/api/functions/database-row-mutation"),
    productAccessToken,
    {
      action: "create",
      id: rowId,
      databaseId,
      title: rowTitle,
      properties: { [statusPropertyId]: "진행" },
    },
  );
  assert(seededRow.response.ok, `canonical row seed returned HTTP ${seededRow.response.status}: ${seededRow.text}`);
  assert(seededRow.json?.row?.id === rowId, "canonical row seed must return database row id");
...
async function runAuthorizationCodeFlow(seed, options = {}) {
  ...
  const consentRequest = consent.text.match(/name="consent_request" value="([^"]+)"/)?.[1];
  assert(consentRequest, "consent UI must include a signed consent request");

  const form = new URLSearchParams();
  form.set("consent_request", consentRequest);
  ...
  if (!verifyReplay) return rotatedToken.json.access_token;
  ...
  const accessAfterReplay = await callMcp(rotatedToken.json.access_token, {
    jsonrpc: "2.0",
    id: 999,
    method: "initialize",
    params: {},
  });
  assert(!accessAfterReplay.response.ok, "refresh-token replay must invalidate the raced grant access token");
  return null;
}
