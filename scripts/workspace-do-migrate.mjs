...
const BASE_URL = (process.env.HANJI_EDGEBASE_URL ?? 'http://127.0.0.1:8787').replace(/\/$/, '');
const OUT_DIR = process.env.HANJI_MIGRATION_DIR ?? join(root, '.edgebase', 'workspace-do-migration');
...
async function restore({ verifyAfter }) {
  ...
  // proven itself on the others). Match by HANJI_PRESERVED_WORKSPACE_HINT.
  const preservedHint = (process.env.HANJI_PRESERVED_WORKSPACE_HINT ?? '').toLowerCase();
