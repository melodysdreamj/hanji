...
async function main() {
  ...
  const imported = await callFunction(baseUrl, owner.token, 'import-export', {
    ...
    markdown: [
      ...
      '  - Nested bullet',
      '- [x] Done item',
      'Styled **bold** and *italic* with [link](https://example.com/import-rich) on [today](notionlike://date/2026-06-25) by [Ada](notionlike://person/user-ada) as `code` and ~~struck~~',
      '> Quoted line',
      '',
      '$$',
      'a + b = c',
  ...
  const appendedMarkdown = await callFunction(baseUrl, owner.token, 'import-export', {
    ...
    markdown: [
      ...
      'Appended **bold** text with [today](notionlike://date/2026-06-25)',
      '',
  ...
  const replacedMarkdown = await callFunction(baseUrl, owner.token, 'import-export', {
    ...
    markdown: [
      ...
      'Replacement **bold** text with [Ada](notionlike://person/user-ada)',
      '',
      '| Mode | Status |',
      '| --- | --- |',
      '| Replace | kept |',
  ...
  assertIncludes(
    exportedCurrentBlocks.markdown,
    `Styled **bold** and *italic* with [link](https://example.com/rich%20text) plus [child mention](/p/${importedChildPageId}) on [today](notionlike://date/2026-06-25) by [Ada](notionlike://person/user-ada) as \`code\` and ~~struck~~`,
    'exported current block markdown',
  );
  assertIncludes(
    exportedCurrentBlocks.markdown,
  ...
  const deletedChild = await callFunction(baseUrl, owner.token, 'page-mutation', {
    action: 'delete',
    id: importedChildPageId,
  });
  assert(
