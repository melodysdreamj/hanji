...
async function main() {
  ...
  // Without NOTIONLIKE_NOTION_IMPORT_SECRET the dialog hides the save-connection
  ...
  try {
    ...
    await captureImportVariant(browser, appUrl, seed, {
      ...
      contract: {
        expectedTexts: ['Import', 'File', 'Notion', 'Prepare Notion token', 'Open Notion token page', 'Setup guide', 'Notion API token', ...connectionSaveTexts, 'Entire workspace', 'Specific pages', 'Import pages in full width', 'Start discovery', 'Queued'],
        ...
        rejectVisibleTexts: ['Connect Notion'],
    ...
    await captureImportVariant(browser, appUrl, seed, {
      ...
      contract: {
        expectedTexts: ['Import', 'File', 'Notion', 'Specific pages', 'Scan accessible roots', 'Paste links or IDs instead (advanced)', 'Start discovery'],
    ...
    await captureImportVariant(browser, appUrl, seed, {
      prefix: 'desktop-notion-jobs',
      viewport: { width: 1440, height: 1000 },
      open: openNotionJobsDialog,
      contract: {
        // The recent-jobs history list was removed; a queued job now surfaces in
        // the step-3 Progress panel (role=status) instead of a workspace-named
        // list row, so we assert the visible "Queued" status there.
        expectedTexts: ['Import', 'File', 'Notion', 'Queued'],
    ...
    await captureImportVariant(browser, appUrl, seed, {
      ...
      contract: {
        expectedTexts: ['Import', 'File', 'Notion', 'Prepare Notion token', 'Open Notion token page', 'Setup guide', 'Notion API token', ...connectionSaveTexts, 'Entire workspace', 'Specific pages', 'Import pages in full width', 'Start discovery', 'Queued'],
        // See desktop-notion-import: the token walkthrough occupies the top, so
        // token controls are below the fold (present via expectedTexts).
        expectedVisibleTexts: ['Prepare Notion token'],
        rejectVisibleTexts: ['Connect Notion'],
        expectFullWidthOptionChecked: true,
        minButtons: 4,
    ...
    await captureImportVariant(browser, appUrl, seed, {
      prefix: 'mobile-notion-jobs',
      viewport: { width: 390, height: 844 },
      mobile: true,
      open: openNotionJobsDialog,
      contract: {
        // See desktop-notion-jobs: queued job now shows in the step-3 panel.
        expectedTexts: ['Import', 'File', 'Notion', 'Queued'],
        expectedVisibleTexts: ['Queued'],
        minButtons: 4,
        minRows: 6,
...
async function openNotionImportDialog(page, baseUrl) {
  ...
  await dialog.getByText('Queued', { exact: false }).first().waitFor({
    state: 'visible',
    timeout: options.timeoutMs,
  });
}
...
async function openNotionPagesScopeDialog(page, baseUrl) {
  await openNotionImportDialog(page, baseUrl);
...
async function openNotionJobsDialog(page, baseUrl) {
  await openNotionImportDialog(page, baseUrl);
  await scrollImportPanelToBottom(page);
...
async function scrollImportPanelToBottom(page) {
  const dialog = page.getByRole('dialog', { name: 'Import', exact: true });
  await dialog.evaluate((dialog) => {
    const nav = dialog?.querySelector('[aria-label="Import sources"]');
