...
const PROPERTY_FIXTURE = {
  ...
  descriptions: {
    title: '회사는 상호명, 개인은 실명으로 입력. 예: 거래처명',
...
async function main() {
  ...
  try {
    ...
    if (options.onlyPropertyHeaderOptions) {
      ...
      console.log('PASS property header menu exposes Notion-like edit/type routes, persists select/status options, closes name edits on Enter, and keeps rollup selectors in view.');
      for (const name of [
...
async function openDueDateEditor(page) {
  await cell(page, 0, 4).getByRole('button', { name: /^Edit date/ }).click({
    timeout: options.timeoutMs,
  });
...
async function seedDatabase(baseUrl) {
  ...
  const rowTitle = `커넥트티앤아이 계약 정산 확인 ${short}`;
