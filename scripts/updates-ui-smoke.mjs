...
async function assertReplyMentionVisible(page, seed) {
  ...
  // A reply that mentions the owner surfaces in both the Updates feed and the
  // Mentions list, so it can appear more than once in the combined view; assert
  // the first occurrence is visible.
  await dialog.getByText(seed.replyPreview, { exact: true }).first().waitFor({
    state: 'visible',
    timeout: options.timeoutMs,
  });
}
...
async function openReplyMentionActivity(page, seed) {
  const dialog = page.getByRole('dialog', { name: 'Updates' });
  await dialog.getByRole('listitem').filter({ hasText: seed.replyPreview }).first().click({
    timeout: options.timeoutMs,
  });
  await page.waitForFunction(
    ([pageId, commentId]) =>
