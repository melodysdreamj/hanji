...
try {
  ...
  console.error(`\nFAIL inline database visual smoke: ${message}`);
...
async function assertInlineDatabasePage(browser, appUrl, apiUrl, seed) {
  ...
  try {
    ...
    console.error(`Inline database visual diagnostics: ${JSON.stringify(diagnostics, null, 2)}`);
    throw error;
  } finally {
...
async function assertLocalInlineDatabaseTitleEditing(page, seed) {
  ...
  assert(
    afterClick.path === parentPath &&
      ...
      afterClick.placeholder === '새 데이터베이스',
...
async function assertInlineDatabaseScopedViewAdd(page, apiUrl, seed) {
  ...
  assert(
    ...
    `inline add-view menu should expose Notion-like view type choices: ${JSON.stringify(menuMetrics)}`,
  );
...
async function assertInlineDatabaseTitleMenuActions(page, apiUrl, seed) {
  ...
  assert(
    ...
    `inline database title menu should expose Notion-like database/view actions: ${JSON.stringify(menuMetrics)}`,
  );
...
async function assertEmptyInlineDatabaseVisualContract(page, seed) {
  ...
  assert(
    ...
    `empty inline database add-property header should expose the Notion-like + property label: ${JSON.stringify(metrics)}`,
  );
  assert(metrics.firstCellBorderBottom === '1px', `empty inline database preview rows should keep Notion-like horizontal rules: ${JSON.stringify(metrics)}`);
...
async function assertPopulatedInlineDatabaseAddPropertyHeader(page, seed) {
  ...
  assert(
    ...
    `populated inline database should expose the Notion-like + property label at the table end: ${JSON.stringify(metrics)}`,
  );
  assert(
...
async function assertInlineDatabaseVisualContract(page, seed, { mobile }) {
  ...
  if (mobile) {
    ...
    assert(
      ...
      `desktop inline database visible tabs/add button and toolbar should have Notion-like breathing room without hiding tabs while excessive space remains, got ${Math.round(metrics.visibleViewChromeToToolbarGap)}px: ${JSON.stringify(metrics)}`,
    );
    assert(
...
async function assertInlineDatabaseToolbarReveal(page, inlineBlockId) {
  ...
  assert(
    ...
    `desktop inline database title actions should collapse to one compact Notion-like ellipsis menu: ${JSON.stringify(metrics)}`,
  );
}
...
async function assertInlineDatabaseSettingsMenu(page, inlineBlockId) {
  ...
  assert(
    ...
    `inline database settings menu should mirror the Notion-like view settings hub before drilling into Properties: ${JSON.stringify(metrics)}`,
  );
}
...
async function assertInlineDatabaseSourcePropertyEditMenu(page) {
  ...
  assert(
    metrics.width >= 290 &&
      ...
      metrics.top >= 8 &&
      metrics.bottom <= metrics.viewportHeight + 28,
    `source property edit should use the compact Notion-like Korean panel: ${JSON.stringify(metrics)}`,
  );
  assert(
    metrics.hasEditCopy &&
...
async function assertInlineDatabaseNewPropertyTypeMenu(page) {
  ...
  assert(
    metrics.width >= 290 &&
      ...
      metrics.top >= 8 &&
      metrics.bottom <= metrics.viewportHeight + 28,
    `new source property type picker should stay in the compact Notion-like panel: ${JSON.stringify(metrics)}`,
  );
  assert(
...
async function seedInlineDatabasePage(baseUrl) {
  ...
  const emptyDatabaseTitle = '새 데이터베이스';
...
async function cleanupSeed(baseUrl, seed) {
  ...
  if (seed.rightDatabaseId) {
    await callFunction(baseUrl, seed.accessToken, 'page-mutation', {
      action: 'delete',
      id: seed.rightDatabaseId,
    }).catch(() => {});
  }
  if (seed.emptyDatabaseId) {
    await callFunction(baseUrl, seed.accessToken, 'page-mutation', {
      action: 'delete',
      id: seed.emptyDatabaseId,
    }).catch(() => {});
  }
}
...
async function loadPlaywright() {
  ...
  throw new Error(
    'Playwright is required for inline database visual smoke. Install it in this repo, set PLAYWRIGHT_MODULE_DIR, or use the local EdgeBase workspace dependencies.',
  );
}
