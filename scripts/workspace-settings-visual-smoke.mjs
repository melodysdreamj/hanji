    ...
    await captureSettingsVariant(browser, appUrl, seed, {
      ...
      captures: [
        ...
        },
        {
          ...
          expectedVisibleTexts: ['Workspace members', 'Invite pending', 'Add members', seed.inviteEmail],
          minControls: 3,
          minRows: 1,
        },
        {
          ...
          draftInviteName: seed.draftInviteName,
          expectedVisibleTexts: ['Workspace members', 'Invite pending', 'Invite', seed.inviteEmail, seed.draftInviteEmail, seed.draftInviteName],
          minControls: 6,
          minRows: 1,
        },
        {
      ...
      captures: [
        {
          ...
          draftInviteEmail: seed.draftInviteEmail,
          draftInviteName: seed.draftInviteName,
          expectedVisibleTexts: ['Workspace members', 'Invite pending', 'Invite', seed.draftInviteEmail, seed.draftInviteName],
          minControls: 6,
          minRows: 1,
        },
        {
...
async function fillInviteDraft(page, capture, surface = 'account-console') {
  ...
  await dialog.getByRole('textbox', { name: 'Member name' }).fill(capture.draftInviteName, {
    timeout: options.timeoutMs,
  });
...
async function scrollSettingsSection(page, section, surface = 'account-console') {
  await page.waitForFunction(({ targetSection, surface }) => {
    ...
    if (navLabel) {
      const nav = dialog.querySelector('aside');
...
async function assertSettingsSectionContract(page, capture, variant) {
  const metrics = await page.evaluate(({ expectedVisibleTexts, section, surface }) => {
    ...
    const inviteNameInput = target.querySelector('input[aria-label="Member name"]');
    ...
    const inviteButton = inviteForm instanceof HTMLElement
      ? Array.from(inviteForm.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Invite')
      : null;
    ...
    return {
      ...
      inviteNameValue: inviteNameInput instanceof HTMLInputElement ? inviteNameInput.value : '',
  ...
  if (capture.inviteDraft) {
    assert(metrics.inviteFormVisible, `${capture.name} should show the member invite form: ${JSON.stringify(metrics)}`);
    assert(metrics.inviteInputCount >= 2, `${capture.name} should show email and name invite inputs: ${JSON.stringify(metrics)}`);
    assert(metrics.inviteEmailValue === capture.draftInviteEmail, `${capture.name} should keep the invite email draft visible: ${JSON.stringify(metrics)}`);
    assert(metrics.inviteNameValue === capture.draftInviteName, `${capture.name} should keep the invite name draft visible: ${JSON.stringify(metrics)}`);
    assert(metrics.inviteRoleValue === (capture.inviteRole ?? 'member'), `${capture.name} should preserve the selected invite role: ${JSON.stringify(metrics)}`);
    assert(metrics.inviteButtonHeight >= 28 && metrics.inviteButtonHeight <= 36, `${capture.name} Invite button should stay compact: ${JSON.stringify(metrics)}`);
  }
  if (variant.mobile) {
...
async function seedWorkspaceSettings(baseUrl) {
  ...
  const inviteEmail = `settings-invite-${suffix}@example.com`;
  const draftInviteEmail = `settings-draft-${suffix}@example.com`;
  const draftInviteName = `Invitee ${String(suffix).slice(-6)}`;
  await callFunction(baseUrl, owner.accessToken, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: inviteEmail,
    displayName: 'Settings Visual Invitee',
    role: 'guest',
  });
