...
// The public page projection intentionally exposes createdBy/lastEditedBy as
// opaque `public-person:*` aliases and keeps createdAt/updatedAt timestamps (see
// publicPageIdentityProjection in backend/functions/share-mutation.ts). Those
// four are the approved public contract, so they are not treated as leaks here;
// the raw owner id must still stay aliased, which the caller checks separately.
// Every other audit field must remain internal.
...
async function main() {
  ...
  await callFunction(baseUrl, owner.token, 'database-mutation', {
    ...
    record: {
      ...
      properties: { __privateDefault: 'must never enter a public snapshot' },
  ...
  await callFunction(baseUrl, owner.token, 'database-mutation', {
    ...
    patch: {
      ...
      properties: {
        __privateDefault: 'must never enter a public snapshot',
        __privateFile: {
  ...
  assertPublicPayloadMinimized(shared, 'direct page share', owner.userId);
  assert(
  ...
  assertPublicPayloadMinimized(databaseShared, 'direct database share', owner.userId);
  ...
  assertPublicPayloadMinimized(directRowShare, 'direct database-row share', owner.userId);
  ...
  assertPublicPayloadMinimized(linkedSourceShare, 'direct linked-source database share', owner.userId);
  ...
  assertPublicPayloadMinimized(unexpiredShared, 'unexpired direct page share', owner.userId);
...
function assertPublicPayloadMinimized(payload, label, forbiddenOwnerId) {
  const leakedPaths = [];
  ...
  if (forbiddenOwnerId) {
    assert(
      !JSON.stringify(payload).includes(forbiddenOwnerId),
      `${label} must alias people rather than leak the raw owner id: ${forbiddenOwnerId}`,
    );
  }
