...
const LIVE_NOTION_TOKEN_ENV_NAMES = ['NOTION_TOKEN', 'NOTIONLIKE_NOTION_TOKEN'];
...
async function inspectImportedOwnerPage(baseUrl, owner, workspaceId, mappings, options) {
  ...
  await context.addInitScript(({ refreshToken, workspaceId: activeWorkspaceId, theme }) => {
    window.localStorage.setItem('edgebase:refresh-token', refreshToken);
    window.localStorage.setItem('notionlike.workspaceId', activeWorkspaceId);
...
async function deleteWorkspacePages(baseUrl, token, workspaceId) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    for (const page of rootPages) {
      if (!page?.id) continue;
      const deleted = await callFunctionWithRetry(baseUrl, token, 'page-mutation', {
        action: 'delete',
        id: page.id,
      });
...
function parseArgs(args) {
  const parsed = {
    ...
    preflight: true,
    requireToken: false,
    rootNotionPageIds: parseStringList(process.env.NOTIONLIKE_LIVE_NOTION_ROOT_PAGE_IDS ?? ''),
...
Checks live Notion API discovery and dry-run review through the Notionlike
product API using NOTION_TOKEN or NOTIONLIKE_NOTION_TOKEN. The smoke never
