...
async function assertSharedPageContract(page, contract, variant) {
  ...
  if (contract.fileLinkText) {
    assert(metrics.fileChipCount > 0, `${variant.prefix} shared files should render as Notion-like file chips: ${JSON.stringify(metrics)}`);
...
async function cleanupSeed(baseUrl, seed) {
  ...
  for (const pageId of [seed.rootPageId].filter(Boolean)) {
    await callFunction(baseUrl, seed.owner.token, 'page-mutation', {
      action: 'delete',
      id: pageId,
    }).catch(() => {});
  }
