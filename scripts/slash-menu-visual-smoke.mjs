...
async function captureTemplateEditorSlashMenuVariant(browser, appUrl, seed, variant) {
  ...
  try {
    ...
    await positionTemplateBlockNearViewportBottom(page, variant.target.templateBlockKey, 112);
    await openTemplateSlashMenu(page, variant.target.templateBlockKey);
    await assertTemplateEditorSlashMenuContract(page, variant.target.templateBlockKey);
    await page.screenshot({
...
async function openSlashMenu(page, blockId) {
  ...
  // A prior visual context may have durably saved its slash trigger. Reset the
  // synthetic command line so this context enters one `/`, not `//`.
  await textbox.fill('', { timeout: options.timeoutMs });
...
async function openTemplateEditor(page, appUrl, target) {
  ...
  await templateMenu.getByRole('button', {
    name: language.toLowerCase().startsWith('ko') ? '편집' : 'Edit',
  }).first().click({
    timeout: options.timeoutMs,
  });
...
async function positionTemplateBlockNearViewportBottom(page, blockKey, bottomGap) {
  await page.evaluate(({ desiredBlockKey, desiredBottomGap }) => {
    const editable = document.querySelector(`[data-template-block-key="${desiredBlockKey}"]`);
  ...
  }, { desiredBlockKey: blockKey, desiredBottomGap: bottomGap });
}

async function openTemplateSlashMenu(page, blockKey) {
  const editable = page.locator(`[data-template-block-key="${blockKey}"]`).first();
  await editable.scrollIntoViewIfNeeded({ timeout: options.timeoutMs });
  await editable.click({ timeout: options.timeoutMs });
  await page.keyboard.type('/');
  const menu = page.locator('[data-template-slash-menu="true"][role="listbox"]');
  await menu.waitFor({ state: 'visible', timeout: options.timeoutMs });
  const language = await page.evaluate(() => document.documentElement.lang);
  await menu.getByRole('option', {
    name: language.toLowerCase().startsWith('ko') ? /^텍스트\b/ : /^Text\b/,
  }).waitFor({
    state: 'visible',
    timeout: options.timeoutMs,
  });
...
async function assertTemplateEditorSlashMenuContract(page, blockKey) {
  const metrics = await page.evaluate((expectedBlockKey) => {
    ...
    const scroll = document.querySelector('[class*="templateEditorScroll"]');
    const editable = document.querySelector(`[data-template-block-key="${expectedBlockKey}"]`);
...
async function assertNewInlineDatabaseCreated(page) {
  ...
  await page.waitForFunction(
    () =>
      !!document.querySelector('[data-inline-database-wrapper] [data-table-head]') &&
      !document.querySelector('[data-inline-database-wrapper] [aria-busy="true"]'),
    null,
    { timeout: options.timeoutMs },
  );
  const metrics = await page.evaluate(() => {
  ...
  const expectedPlaceholder = metrics.language.toLowerCase().startsWith('ko')
    ? '새 데이터베이스'
    : 'New database';
  assert(
    metrics.titleText === '' && metrics.titlePlaceholder === expectedPlaceholder,
    `New database title input should be empty with the localized placeholder ready to type: ${JSON.stringify(metrics)}`,
  );
  assert(
...
async function cleanupSeed(baseUrl, seed) {
  ...
  for (const target of Object.values(seed.targets)) {
    await permanentlyDeletePage(baseUrl, seed.accessToken, target.pageId, { call: callFunction }).catch(() => {});
    if (target.existingDatabaseId) {
      await permanentlyDeletePage(baseUrl, seed.accessToken, target.existingDatabaseId, { call: callFunction }).catch(() => {});
    }
  }
}
...
async function newCheckedPage(browser, contextOptions = {}) {
  ...
  page.on('response', async (response) => {
    ...
    errors.push(
      `${response.status()} ${response.url()} request=${requestBody.slice(0, 300)} response=${responseBody.slice(0, 300)}`,
    );
  });
