...
export const DEFAULT_BASE_URL = process.env.NOTIONLIKE_EDGEBASE_URL ?? 'http://127.0.0.1:8787';
...
export async function installBrowserSession(
  ...
  await context.addInitScript(({
    ...
    if (legacyRefreshToken) {
      ...
      const seedGuardKey = 'notionlike:harness:legacy-auth-seeded';
      if (window.localStorage.getItem(seedGuardKey) !== '1') {
        window.localStorage.setItem(seedGuardKey, '1');
        window.localStorage.setItem('edgebase:refresh-token', legacyRefreshToken);
      }
    }
    if (cookieSessionUserId) {
      window.localStorage.setItem('edgebase:cookie-session', JSON.stringify({
        version: 1,
        userId: cookieSessionUserId,
      }));
    }
    if (id) window.localStorage.setItem('notionlike.workspaceId', id);
...
export async function captureBrowserSession(
  ...
  session.browserCookies = cookies;
  return cookies;
}
...
const GENERIC_UNAUTHORIZED_CONSOLE_ERROR =
  'Failed to load resource: the server responded with a status of 401 (Unauthorized)';

function isAuthRefreshResponse(response) {
  if (response.status() !== 401) return false;
  try {
    return new URL(response.url()).pathname === '/api/auth/refresh';
  } catch {
    return false;
  }
}
 ...
 * `allowInitialSignedOutRefresh401` is enabled, only a generic console error
 * paired with an observed 401 from `/api/auth/refresh` is consumed, and only
 * until `endInitialSignedOutRefreshWindow()` is called. Later 401s stay fatal.
 */
export function watchBrowserErrors(page, {
  ...
  let initialSignedOutWindowOpen = allowInitialSignedOutRefresh401;
  let initialSignedOutAllowanceAvailable = allowInitialSignedOutRefresh401;
  let pendingSignedOutRefreshFailure = false;
  let pendingGenericUnauthorizedError = false;
  const record = (message) => errors.push(prefix ? `${prefix}${message}` : message);

  page.on('pageerror', (error) => record(error.message));
  page.on('console', (message) => {
    if (message.type() !== 'error') return;
    if (initialSignedOutWindowOpen && message.text() === GENERIC_UNAUTHORIZED_CONSOLE_ERROR) {
      ...
      if (initialSignedOutAllowanceAvailable && pendingSignedOutRefreshFailure) {
        pendingSignedOutRefreshFailure = false;
        initialSignedOutAllowanceAvailable = false;
      } else if (initialSignedOutAllowanceAvailable && !pendingGenericUnauthorizedError) {
        ...
        pendingGenericUnauthorizedError = true;
      } else {
        record(message.text());
      }
      return;
  ...
  page.on('response', async (response) => {
    if (initialSignedOutWindowOpen && isAuthRefreshResponse(response)) {
      if (initialSignedOutAllowanceAvailable && pendingGenericUnauthorizedError) {
        pendingGenericUnauthorizedError = false;
        initialSignedOutAllowanceAvailable = false;
      } else if (initialSignedOutAllowanceAvailable && !pendingSignedOutRefreshFailure) {
        pendingSignedOutRefreshFailure = true;
      } else {
        record(`Unexpected repeated 401 ${response.url()}`);
      }
    }
  ...
  return {
    ...
    async endInitialSignedOutRefreshWindow() {
      ...
      if (pendingGenericUnauthorizedError) record(GENERIC_UNAUTHORIZED_CONSOLE_ERROR);
