...
async function assertTrashContract(page, contract, variant) {
  const metrics = await page.evaluate(({ expectedVisibleTexts }) => {
    const main = document.querySelector('main[aria-label="Trash"]');
  ...
  assert(metrics.headingTop - metrics.mainTop >= 32 && metrics.headingTop - metrics.mainTop <= 120, `${variant.prefix} Trash heading should keep a Notion-like top gutter: ${JSON.stringify(metrics)}`);
...
async function loadPlaywright() {
  ...
  throw new Error(
    'Playwright is required for trash visual smoke. Install it in this repo, set PLAYWRIGHT_MODULE_DIR, or use the local EdgeBase workspace dependencies.',
  );
}
