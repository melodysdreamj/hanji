...
async function assertSingleParagraphRichHtmlPaste(page, baseUrl, seed) {
  ...
  const html = [
    ...
    `<span data-color="hanjievil">${invalidColorText}</span> `,
...
async function assertSlashTabs(page, baseUrl, seed) {
  ...
  await group.getByRole('button', { name: 'Add a tab' }).click({ timeout: options.timeoutMs });
  await waitForBlocks(
    baseUrl,
    seed,
    (blocks) => {
...
async function assertPastedHtmlTabs(page, baseUrl, seed) {
  ...
  const html = [
    '<html><body><!--StartFragment--><div data-hanji-copy="true">',
    '<div data-hanji-block-type="tab"><strong>Tabs</strong>',
    `<section data-hanji-tab-label="true" data-hanji-tab-icon="${icon}">`,
    `<p data-hanji-tab-title="true"><span data-hanji-tab-icon-text="true">${icon}</span> ${label}</p>`,
    `<div data-hanji-tab-panel="true"><p>${bodyText}</p></div>`,
    '</section>',
...
async function cleanupSeed(baseUrl, seed) {
  ...
  await permanentlyDeletePage(baseUrl, seed.accessToken, seed.pageId, { call: (baseUrl, token, name, body) => callFunction(baseUrl, token, name, body, 10_000) }).catch(() => {});
}

async function seedSession(context, seed) {
  await installBrowserSession(context, seed, {
    appOrigin: normalizeBaseUrl(options.url),
    authOrigin: normalizeBaseUrl(options.apiUrl),
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--only-slash-keep-text') {
      parsed.onlySlashKeepText = true;
      continue;
    }
    if (arg === '--url') {
