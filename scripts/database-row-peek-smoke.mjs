...
async function main() {
  ...
  try {
    if (options.onlyRowSelect) {
      ...
      console.log('PASS row side peek opens and closes with Hanji slide motion.');
    } else {
...
async function assertRowMultiSelectOptionInteraction(page, seed) {
  ...
  assert(
    ...
    `row multi-select options should keep Hanji dark-menu density and readable chips: ${JSON.stringify(beforeClick.visual)}`,
  );
...
async function cleanupSeed(baseUrl, seed) {
  ...
  if (seed.inlinePageId) {
    await permanentlyDeletePage(baseUrl, seed.accessToken, seed.inlinePageId, { call: callFunction }).catch(() => {});
  }
  await permanentlyDeletePage(baseUrl, seed.accessToken, seed.databaseId, { call: callFunction });
  if (seed.relatedDatabaseId) {
    await permanentlyDeletePage(baseUrl, seed.accessToken, seed.relatedDatabaseId, { call: callFunction }).catch(() => {});
  }
  if (seed.nestedDatabaseId) {
    await permanentlyDeletePage(baseUrl, seed.accessToken, seed.nestedDatabaseId, { call: callFunction }).catch(() => {});
  }
}
...
async function seedSession(context, seed) {
  ...
  await installBrowserSession(context, seed, {
    ...
    localStorage: { 'hanji:theme': options.theme },
  });
}
