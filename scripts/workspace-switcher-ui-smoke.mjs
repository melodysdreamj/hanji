...
async function createWorkspaceFromMenu(page, baseUrl, seed) {
  ...
  // "New workspace" now opens the creation dialog (blank / Notion / Hanji
  // start choices); blank stays the default choice.
  const dialog = page.getByRole('dialog', { name: 'New workspace' });
  await dialog.waitFor({ state: 'visible', timeout: options.timeoutMs });
  await dialog.locator('#workspace-create-name').fill(seed.createdWorkspaceName, {
    timeout: options.timeoutMs,
  });
  await dialog.getByRole('button', { name: 'Create workspace' }).click({ timeout: options.timeoutMs });
