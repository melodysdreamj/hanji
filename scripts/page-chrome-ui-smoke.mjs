  ...
  return {
    ...
    normalizedContract: {
      ...
      localDeviationPolicy:
        'Use Notionlike labels/tokens and responsive gutters while preserving the current-Notion reveal rules and hierarchy.',
    },
  };
}
...
async function collectPageChromeSurfaceInventory(page) {
  return await page.evaluate(() => {
    ...
    return {
      ...
      theme: document.documentElement.dataset.theme ?? window.localStorage.getItem('notionlike:theme') ?? null,
      document: {
        rect: readRect(doc),
