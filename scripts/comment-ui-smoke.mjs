...
async function openPage(page, baseUrl, pageId, title) {
  ...
  // The page title renders in the sidebar, the breadcrumb, and the editor
  // heading, so match the first occurrence; the anchor-block check below is what
  // proves the page body actually loaded.
...
async function cleanupSeed(baseUrl, seed) {
  ...
  await permanentlyDeletePage(baseUrl, seed.accessToken, seed.pageId, { call: callFunction });
}
...
Checks the running Hanji app's comment panel with DOM assertions only.
