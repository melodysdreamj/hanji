...
if (linkedEdgebaseVersion !== pinnedEdgebaseVersion) {
  if (strictRelease) {
    throw new Error(
      `Release link requires EdgeBase ${pinnedEdgebaseVersion}, found ${linkedEdgebaseVersion}.`,
    );
  }
  console.warn(
...
if (args.has('--help') || args.has('-h')) {
  console.log(`Usage: node scripts/link-local-edgebase.mjs [--check] [--strict-release]

Links Hanji's EdgeBase package slots to the local EdgeBase workspace.

Options:
  --check   Verify the local links without modifying node_modules.
  --strict-release  Additionally require a clean, exactly tagged EdgeBase checkout and fresh dist files.
`);
  process.exit(0);
}

for (const arg of args) {
  if (arg !== '--check' && arg !== '--strict-release') {
    throw new Error(`Unknown argument: ${arg}`);
  }
}

function gitOutput(...gitArgs) {
  return execFileSync('git', ['-C', edgebaseRoot, ...gitArgs], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  }).trim();
}

function latestMtime(path, filter = () => true) {
  const stat = statSync(path);
  if (!stat.isDirectory()) return filter(path) ? stat.mtimeMs : 0;
  let latest = 0;
  for (const entry of readdirSync(path)) {
    latest = Math.max(latest, latestMtime(join(path, entry), filter));
  }
  return latest;
}
...
function assertStrictReleaseProvenance() {
  ...
  if (dirty) throw new Error('Strict EdgeBase release link requires a clean checkout.');
  const head = gitOutput('rev-parse', 'HEAD');
  const exactTag = gitOutput('describe', '--tags', '--exact-match', 'HEAD');
  if (![pinnedEdgebaseVersion, `v${pinnedEdgebaseVersion}`].includes(exactTag)) {
    throw new Error(
      `EdgeBase HEAD ${head.slice(0, 12)} is tagged ${exactTag}, expected v${pinnedEdgebaseVersion}.`,
    );
  }
  for (const spec of Object.values(packages)) {
    ...
    const sourceMtime = latestMtime(sourcePath, (path) => !path.endsWith('.map'));
    const distMtime = latestMtime(distPath);
    if (sourceMtime > distMtime) {
      throw new Error(`${spec.path} dist is older than its source; rebuild EdgeBase before release.`);
    }
  }
  console.log(`OK strict EdgeBase provenance: ${exactTag} (${head.slice(0, 12)}) clean and built`);
}
...
if (strictRelease) assertStrictReleaseProvenance();
