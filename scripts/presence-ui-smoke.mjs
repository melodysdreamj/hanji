...
async function assertPresenceRoomState(browser, baseUrl, apiUrl, seed) {
  ...
  try {
    ...
    await viewer.page.evaluate(({ pageId, blockId }) => {
      window.dispatchEvent(new CustomEvent('hanji:page-presence-awareness', {
        detail: {
          pageId,
          blockId,
          mode: 'editing',
...
async function assertPageMetaRoomMutation(ownerPage, viewerPage, apiUrl, seed) {
  ...
  await ownerPage.evaluate(({ icon, pageId, title, updatedAt }) => {
    window.dispatchEvent(new CustomEvent('hanji:page-room-mutation', {
      detail: {
...
async function collectDatabaseAwarenessDiagnostics(page, label) {
  return {
    ...
    ...(await page.evaluate(() => ({
      ...
      debug: window.__hanjiPresenceDebug ?? null,
...
async function assertCrdtRoomSignal(senderPage, receiverPage, seed) {
  ...
  await receiverPage.evaluate(() => {
    ...
    window.__hanjiPresenceSmokeCrdtReceived = null;
    window.addEventListener(
      'hanji:page-crdt-update-received',
      (event) => {
        window.__hanjiPresenceSmokeCrdtReceived = event.detail?.blockId ?? null;
      },
      { once: true },
    );
  });
  ...
  await senderPage.evaluate(({ blockId, operation, pageId, updatedAt }) => {
    window.dispatchEvent(
      new CustomEvent('hanji:page-crdt-update', {
        detail: {
          blockId,
          operation,
          pageId,
  ...
  await receiverPage.waitForFunction(
    ({ blockId }) => window.__hanjiPresenceSmokeCrdtReceived === blockId,
    { blockId: seed.blockId },
    { timeout: options.timeoutMs },
  );
  await receiverPage
...
async function openPage(page, baseUrl, pageId, title, label = 'page') {
  ...
  try {
    ...
    throw new Error([
      ...
      error instanceof Error ? error.message : String(error),
      `browserErrors: ${JSON.stringify(page.__hanjiErrors ?? [])}`,
      JSON.stringify(diagnostics, null, 2),
    ].join('\n'));
  }
...
async function cleanupSeed(baseUrl, seed) {
  ...
  if (seed.databaseId) {
    await permanentlyDeletePage(baseUrl, seed.owner.accessToken, seed.databaseId, { call: callFunction });
  }
