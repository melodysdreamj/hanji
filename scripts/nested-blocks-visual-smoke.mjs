...
async function assertNestedBlocksVisualContract(page, seed, variant) {
  ...
  assertHanjiToggleCaret(toggle, 'toggle parent');
  ...
  assertHanjiToggleCaret(emptyToggle, 'empty toggle');
  ...
  assert(
    ...
    `empty toggle prompt should match the Hanji helper copy: ${JSON.stringify(emptyToggle)}`,
  );
...
function assertHanjiToggleCaret(row, label) {
