...
async function main() {
  ...
  console.log('PASS non-member users cannot list workspace members before invitation acceptance.');

  const invite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email,
  ...
  assert(invitation?.id, 'workspace invite must return an invitation id');
  assert(invitation?.token, 'workspace invite must return an invitation token for accept links');
  assert(invitation?.status === 'pending', 'workspace invite must start pending');
  assert(
    ['sent', 'failed', 'not_configured'].includes(invitation.emailDeliveryStatus),
    'workspace invite must record email delivery status',
  );
  pendingInvitationId = invitation.id;
  console.log('PASS workspace email invitations expose a pending accept token and delivery status.');

  const accepted = await callFunction(baseUrl, invitee.token, 'workspace-mutation', {
    action: 'acceptInvitation',
    token: invitation.token,
    email,
  });
  assert(accepted?.member?.userId === invitee.userId, 'accepted invitation must create an invitee membership');
  assert(accepted.member.role === 'guest', 'accepted invitation must preserve the invited role');
  assert(accepted.invitation?.status === 'accepted', 'accepted invitation must be marked accepted');
  assert(accepted.acceptedInvitationId === invitation.id, 'accept response must identify the accepted invitation');
  assert(
    Array.isArray(accepted.invitations) &&
      !accepted.invitations.some((item) => item.id === invitation.id),
    'accepted invitations must leave the pending invitation list',
  );
  ...
  console.log('PASS invitation acceptance creates a real workspace membership usable by bootstrap.');
  ...
  const roleProbeInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
  ...
  pendingInvitationId = roleProbeInvite?.invitation?.id ?? '';
  assert(pendingInvitationId, 'role probe invitation must be created');
  await callFunction(baseUrl, roleProbe.token, 'workspace-mutation', {
    action: 'acceptInvitation',
    token: roleProbeInvite.invitation.token,
  });
  pendingInvitationId = '';
  ...
  const acceptedInvitationAudit = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'organizationDirectory',
    organizationId,
  ...
  assert(
    acceptedInvitationAudit?.organizationAuditEvents?.some(
      (event) => event.targetId === invitation.id,
    ),
    'accepted workspace invitations must record a filterable organization audit event',
  );
  ...
  const transferInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: transferEmail,
  ...
  assert(transferInvite?.invitation?.token, 'owner transfer invite must return an accept token');
  pendingInvitationId = transferInvite.invitation.id;
  const transferAccepted = await callFunction(baseUrl, transferTarget.token, 'workspace-mutation', {
    action: 'acceptInvitation',
    token: transferInvite.invitation.token,
    email: transferEmail,
  });
  transferMemberId = transferAccepted?.member?.id ?? '';
  assert(transferMemberId, 'owner transfer target must accept into the workspace');
  pendingInvitationId = '';
  ...
  const inviteOnlySignupPolicy = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'updateOrganizationSettings',
    organizationId,
    signupPolicy: 'invite_only',
  });
  assert(
    inviteOnlySignupPolicy?.instanceSettings?.signupPolicy === 'invite_only',
    'updateOrganizationSettings must enable invite-only global signup policy',
  );
  ...
  const invitedSignupEmail = `invited-signup-${suffix}@external.example`;
  const invitedSignupInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: invitedSignupEmail,
    displayName: 'Invited signup smoke',
    role: 'guest',
  });
  assert(invitedSignupInvite?.invitation?.id, 'invite-only signup must allow invited email setup');
  pendingInvitationId = invitedSignupInvite.invitation.id;
  const invitedSignup = await signUpWithPassword(
    baseUrl,
    invitedSignupEmail,
    smokePassword(`${suffix}Invite`),
    'Invited Signup Smoke',
  );
  assert(invitedSignup?.token, 'invited email must be able to create an account under invite-only signup');
  await callFunction(baseUrl, owner.token, 'workspace-mutation', {
  ...
  const verifiedDomainSignupPolicy = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'updateOrganizationSettings',
    organizationId,
    signupPolicy: 'verified_domains',
  });
  assert(
    verifiedDomainSignupPolicy?.instanceSettings?.signupPolicy === 'verified_domains',
    'updateOrganizationSettings must enable verified-domain global signup policy',
  );
  const externalVerifiedModeEmail = `blocked-verified-signup-${suffix}@external.example`;
  const externalVerifiedModePassword = smokePassword(`${suffix}External`);
  await expectAuthSignupStatus(
    baseUrl,
    externalVerifiedModeEmail,
    externalVerifiedModePassword,
    'External Verified Mode Smoke',
    403,
  );
  await expectAuthPasswordSigninStatus(baseUrl, externalVerifiedModeEmail, externalVerifiedModePassword, 401);
  const verifiedDomainSignup = await signUpWithPassword(
    baseUrl,
    `verified-signup-${suffix}@${organizationDomain}`,
    smokePassword(`${suffix}Domain`),
    'Verified Domain Signup Smoke',
  );
  assert(
    verifiedDomainSignup?.token,
    'verified organization domain email must be able to create an account under verified-domain signup',
  );
  ...
  console.log('PASS global signup policy blocks public signup, preserves invited signup, and allows verified domains.');

  await expectFunctionStatus(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: `external-member-${suffix}@external.example`,
    role: 'member',
  }, 400);
  const externalGuestInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: `external-guest-${suffix}@external.example`,
    role: 'guest',
  });
  assert(externalGuestInvite?.invitation?.id, 'external verified-domain invite must allow guests');
  pendingInvitationId = externalGuestInvite.invitation.id;
  await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'removeInvitation',
    workspaceId,
    invitationId: pendingInvitationId,
  ...
  pendingInvitationId = '';
  const internalMemberInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: `internal-member-${suffix}@${organizationDomain}`,
    role: 'member',
  });
  assert(internalMemberInvite?.invitation?.id, 'verified organization domain must allow member invites');
  pendingInvitationId = internalMemberInvite.invitation.id;
  const pendingInviteDirectory = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'organizationDirectory',
    organizationId,
  });
  const pendingInviteProfile = pendingInviteDirectory?.organizationProfiles?.find(
    (profile) => profile.email === `internal-member-${suffix}@${organizationDomain}`,
  );
  assert(
    pendingInviteProfile?.pendingInvitations?.some(
      (invitation) => invitation.invitationId === pendingInvitationId && invitation.workspaceId === workspaceId,
    ),
    'pending workspace invitations must appear in organization profile directory',
  );
  await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'removeInvitation',
    workspaceId,
  ...
  pendingInvitationId = '';
  console.log('PASS verified-domain signup policy enforces internal member invites and external guests.');
  ...
  await expectFunctionStatus(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: `guest-policy-${suffix}@external.example`,
    role: 'guest',
  }, 403);
  await expectFunctionStatus(baseUrl, owner.token, 'share-mutation', {
    action: 'invite',
  ...
  console.log('PASS accepted invitations cannot be reused.');
  ...
  console.log('PASS guest workspace members cannot invite more members.');

  const revokedEmail = `membership-revoked-${suffix}@example.com`;
  const revokedInvite = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'inviteMember',
    workspaceId,
    email: revokedEmail,
    role: 'member',
  });
  assert(revokedInvite?.invitation?.token, 'revoked invite seed must return a token');
  pendingInvitationId = revokedInvite.invitation.id;
  const revoked = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'removeInvitation',
    workspaceId,
    invitationId: pendingInvitationId,
  });
  assert(revoked?.invitation?.status === 'revoked', 'removed invitation must be marked revoked');
  const revokedInvitationAudit = await callFunction(baseUrl, owner.token, 'workspace-mutation', {
    action: 'organizationDirectory',
    organizationId,
  ...
  assert(
    revokedInvitationAudit?.organizationAuditEvents?.some(
      (event) => event.targetId === revoked.invitation.id,
    ),
    'revoked workspace invitations must record a filterable organization audit event',
  );
  pendingInvitationId = '';
  await expectFunctionStatus(baseUrl, invitee.token, 'workspace-mutation', {
    action: 'acceptInvitation',
    token: revokedInvite.invitation.token,
  }, 400);
  console.log('PASS revoked workspace invitations cannot be accepted.');

  console.log('\nPASS workspace invitation acceptance, reuse denial, revoke denial, and guest invite denial work through product APIs.');
}
...
async function deleteWorkspacePages(baseUrl, token, targetWorkspaceId) {
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    for (const page of rootPages) {
      if (!page?.id) continue;
      await callFunction(baseUrl, token, 'page-mutation', {
        action: 'delete',
        id: page.id,
      });
    }
...
denial, and guest invite permissions against a running Notionlike EdgeBase
runtime.
