...
export function lockBoxName(userId: string) {
  return `hanji-lock:${userId}`;
}

let warnedLegacyConflict = false;

async function migrateLegacyPassphraseStorage(userId: string) {
  try {
    const result = await migrateLegacyIndexedDbProvenance(
      {
        legacyName: `${legacyLockBoxName(userId)}::keys`,
        canonicalName: `${lockBoxName(userId)}::keys`,
      },
      [
        {
          legacyName: legacyOutboxDatabaseName(userId),
          canonicalName: `hanji-outbox:${userId}`,
          consumeStores: ["entries"],
          earlyMetaKeys: LEGACY_OUTBOX_EARLY_META_KEYS,
          respectLegacyOutboxLiveness: true,
        },
        {
          legacyName: legacyRecordCacheDatabaseName(userId),
          canonicalName: `hanji-records:${userId}`,
    ...
    if (result !== "preserved-conflict" || warnedLegacyConflict) return;
    warnedLegacyConflict = true;
    console.warn(
      "Legacy local data remains preserved for a live/unknown old tab or different encryption provenance."
    );
  } catch (error) {
    if (!legacyIndexedDbMigrationCanContinue(error)) throw error;
    if (warnedLegacyConflict) return;
    warnedLegacyConflict = true;
    console.warn("Legacy local-data migration was deferred; canonical storage remains available.", error);
  }
}

export async function lockConfigured(userId: string): Promise<boolean> {
  try {
    await migrateLegacyPassphraseStorage(userId);
  } catch {
    return false;
  }
