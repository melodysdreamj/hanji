...
const configuredEdgeBaseUrl = import.meta.env.VITE_EDGEBASE_URL as string | undefined;
const developmentEdgeBaseUrl = (() => {
  if (typeof window === "undefined") return "http://localhost:8787";
  const hostname = window.location.hostname.toLowerCase();
  if (hostname === "localhost" || hostname === "127.0.0.1") {
    return `http://${hostname}:8787`;
  }
  if (hostname === "::1" || hostname === "[::1]") return "http://[::1]:8787";
  return "http://localhost:8787";
})();
const EDGEBASE_URL =
  configuredEdgeBaseUrl ?? (import.meta.env.DEV ? developmentEdgeBaseUrl : runtimeOrigin);
...
const COLLABORATION_CLIENT_ID_KEY = "notionlike.collaborationClientId";
const ALLOW_ANONYMOUS_BOOTSTRAP =
  import.meta.env.VITE_ALLOW_ANONYMOUS_BOOTSTRAP === "true";
let anonymousBootstrapConfigPromise: Promise<boolean> | null = null;
...
async function fetchAnonymousBootstrapConfig() {
  ...
  const json = await response.json() as { allowAnonymousBootstrap?: unknown };
  return json.allowAnonymousBootstrap === true;
}
...
export async function anonymousBootstrapAvailableRemote(): Promise<boolean> {
  ...
  if (!anonymousBootstrapConfigPromise) {
    anonymousBootstrapConfigPromise = fetchAnonymousBootstrapConfig().catch(() => {
      anonymousBootstrapConfigPromise = null;
      return false;
    });
  }
  return anonymousBootstrapConfigPromise;
}
...
export async function requestMagicLinkRemote(email: string): Promise<void> {
  ...
  await getClient().auth.signInWithMagicLink({ email, redirectUrl });
}
...
export function configuredOAuthProviders(): OAuthProviderOption[] {
  const configured = import.meta.env.VITE_AUTH_OAUTH_PROVIDERS as string | undefined;
  return (configured ?? "")
    .split(",")
...
export interface NotionImportConnectionListResult {
  ...
  /** False when the backend cannot store connections (no NOTIONLIKE_NOTION_IMPORT_SECRET). */
...
export interface RemoveWorkspaceInvitationInput {
  workspaceId: string;
  invitationId?: string;
  email?: string;
}
...
export async function inviteWorkspaceMemberRemote(
...
export async function acceptWorkspaceInvitationRemote(
...
export async function removeWorkspaceInvitationRemote(
  input: RemoveWorkspaceInvitationInput
): Promise<WorkspaceMembersResult> {
  return getClient().functions.post<WorkspaceMembersResult>("workspace-mutation", {
...
export async function deletePageRemote(id: string): Promise<string[]> {
...
export async function duplicatePageRemote(id: string): Promise<DuplicatePageResult> {
...
export async function deleteDatabaseRowRemote(id: string): Promise<string[]> {
...
export function deletePropertyRemote(id: string, databaseId?: string) {
  return deleteDatabaseRecordRemote("db_properties", id, databaseId);
}
