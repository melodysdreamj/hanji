...
const PAGE_AWARENESS_EVENT = "notionlike:page-presence-awareness";
const PAGE_TEXT_UPDATE_EVENT = "notionlike:page-text-update";
const PAGE_CRDT_UPDATE_EVENT = "notionlike:page-crdt-update";
export const PAGE_TEXT_UPDATE_RECEIVED_EVENT = "notionlike:page-text-update-received";
export const PAGE_CRDT_UPDATE_RECEIVED_EVENT = "notionlike:page-crdt-update-received";
...
function guestLabel() {
  return pickLabels(PAGE_PRESENCE_LABELS).guest;
}
...
export function usePagePresence(pageId: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    if (window.localStorage.getItem("notionlike.debugPresence") !== "1") return;
    (
      window as Window & {
        __notionlikePresenceDebug?: unknown;
      }
    ).__notionlikePresenceDebug = {
