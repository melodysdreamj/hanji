...
import { assertSafeWorkspaceUpload } from "./fileSecurity";
...
export function workspaceFileApiOrigin() {
  ...
  return new URL(EDGEBASE_URL, base).origin;
}
...
export async function uploadWorkspaceFile(
  ...
  // The platform also forces active content to download, but reject it before
  // any reservation or network request so every Hanji upload surface is safe
  // even against an older or incorrectly configured EdgeBase runtime.
  assertSafeWorkspaceUpload(file);
  const generatedLabels = activePersistentGeneratedLabels();
  await ensureAuth();
  ...
  const prepared = await prepareFileUploadRemote({
    ...
    scope,
    ...target,
    name: file.name || generatedLabels.untitled,
    size: file.size,
  ...
  const metadata = {
    ...
    originalName: file.name || generatedLabels.untitled,
  };
  ...
  return {
    ...
    name:
      completed.name ||
      file.name ||
      completed.key.split("/").at(-1) ||
      generatedLabels.untitled,
