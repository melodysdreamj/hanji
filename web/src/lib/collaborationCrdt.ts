...
function createBlockTextUndoSession({
  ...
  const localOrigin = { source: "hanji:block-text-local", blockId };
  const remoteOrigin = { source: "hanji:block-text-remote", blockId };
...
async function ensureBlockTextUndoSession({
  ...
  if (durableState) {
    try {
      Y.applyUpdate(doc, durableState, "hanji:block-text-remote");
      hydrated = true;
    } catch {
      hydrated = false;
    }
  ...
  return queueBlockTextUndoSessionWrite(blockId, async () => {
    ...
    if (!session) {
      ...
      for (const operation of operations) {
        ...
        if (!update) continue;
        try {
          Y.applyUpdate(doc, update, "hanji:block-text-remote");
          applied += 1;
        } catch {
