...
const PERSON_LABELS = {
  en: { guest: "Guest", you: "You", youSuffix: "you" },
  ko: { guest: "게스트", you: "나", youSuffix: "나" },
} as const;
...
export function personLabel(id: string, currentUserId?: string) {
  if (!id) return "";
  const labels = pickLabels(PERSON_LABELS);
  const label = displayName(peopleByUserId.get(id));
  if (label) return currentUserId && id === currentUserId ? `${label} (${labels.youSuffix})` : label;
  if (currentUserId && id === currentUserId) return labels.you;
  if (id === "local-user") return labels.you;
...
export function actorLabel(id?: string | null, currentUserId?: string) {
  return id ? personLabel(id, currentUserId) : pickLabels(PERSON_LABELS).you;
}
