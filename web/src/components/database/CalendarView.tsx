...
const CALENDAR_CARD_DRAG = "application/x-notionlike-calendar-card";
  ...
  if (!dateProp) {
    return (
      <div className={styles.calendarEmpty}>
        <div className={styles.viewEmptyTitle}>Add a date property</div>
        <div className={styles.viewEmptyDesc}>
          Calendar views need a date property to place pages on the month.
        </div>
        {!readOnly && (
      ...
      <div className={styles.calendarToolbar}>
        ...
        <div className={styles.calendarControls}>
          ...
          <button
            ...
            aria-label={calendarLayout === "week" ? "Previous week" : "Previous month"}
            title={calendarLayout === "week" ? "Previous week" : "Previous month"}
            onClick={() => moveMonth(-1)}
          >
            <ChevronLeft size={14} />
          </button>
          <button
            type="button"
            className={styles.calendarNavButton}
            aria-label={calendarLayout === "week" ? "Next week" : "Next month"}
            title={calendarLayout === "week" ? "Next week" : "Next month"}
            onClick={() => moveMonth(1)}
          >
                    ...
                    >
                      ...
                      <span className={styles.calendarCardMeta}>
                        {metaProps.slice(0, 2).map((prop) => (
                          <PropValue key={prop.id} row={row} prop={prop} interactive={!readOnly && !row.isLocked} onOpenPage={openRow} />
                        ))}
                      </span>
                    </button>
                  );
                ...
                {dayRows.length > CALENDAR_MAX_CARDS && (
                  ...
                  >
                    {expandedDays.has(key)
                      ? "Show less"
                      : `+${dayRows.length - CALENDAR_MAX_CARDS} more`}
                  </button>
                )}
              </div>
            </div>
