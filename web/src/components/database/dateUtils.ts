...
const NOTION_TIMESTAMP_LABELS = {
  en: {
    timestamp: (date: Date, hour12: number, minutes: string) =>
      `${date.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })} ${hour12}:${minutes} ${
  ...
  ko: {
    timestamp: (date: Date, hour12: number, minutes: string) => {
      ...
      return `${date.getFullYear()}년 ${date.getMonth() + 1}월 ${date.getDate()}일 ${period} ${hour12}:${minutes}`;
    },
  },
} as const;
...
export function formatNotionTimestamp(value: unknown) {
  ...
  return pickLabels(NOTION_TIMESTAMP_LABELS).timestamp(date, hour12, minutes);
}
