...
function remoteAwarenessText(awareness: PagePresenceAwareness, count: number) {
  const key = awareness.mode === "selecting" ? "awareness.selecting" : "awareness.editing";
  const extra = count > 1 ? ` +${count - 1}` : "";
  return i18next.t(`tableView:${key}`, { label: awareness.label, extra });
}
...
function parseInternalRowsClipboard(data: DataTransfer, databaseId: string) {
  const raw = readTextWithMime(data, HANJI_TABLE_ROWS_MIME);
  if (!raw) return [];
  try {
...
type TableCalculationValue = TableCalculation | "none";
type TableCalculationOption = { value: TableCalculationValue; label: string };

const BASE_TABLE_CALCULATIONS: TableCalculationValue[] = [
  "none",
  "count_all",
  "count_values",
  "count_unique",
...
const CHECKBOX_TABLE_CALCULATIONS: TableCalculationValue[] = [
  "checked",
  "unchecked",
  "percent_checked",
  "percent_unchecked",
...
const NUMBER_TABLE_CALCULATIONS: TableCalculationValue[] = [
  "sum",
  "average",
  "median",
  "min",
...
const DATE_TABLE_CALCULATIONS: TableCalculationValue[] = [
  "earliest_date",
  "latest_date",
  "date_range",
];

function tableCalculationOptionLabel(value: TableCalculationValue) {
  return i18next.t(`tableView:calculations.${value}`);
}

function tableCalculationsFor(prop: DbProperty): TableCalculationOption[] {
  const values = [...BASE_TABLE_CALCULATIONS];
  if (prop.type === "checkbox") values.push(...CHECKBOX_TABLE_CALCULATIONS);
  if (prop.type === "number" || prop.type === "formula" || prop.type === "rollup") {
    values.push(...NUMBER_TABLE_CALCULATIONS);
  }
  if (prop.type === "date" || prop.type === "created_time" || prop.type === "last_edited_time") {
    values.push(...DATE_TABLE_CALCULATIONS);
  }
  return values.map((value) => ({ value, label: tableCalculationOptionLabel(value) }));
}

function tableCalculationLabel(calculation: TableCalculation) {
  return tableCalculationOptionLabel(calculation).toUpperCase();
}
...
function relativeDateGroup(date: Date | null, prop: DbProperty): Omit<TableGroup, "rows"> {
  if (!date) {
    return {
      key: "__none",
      label: i18next.t("tableView:dateGroup.none", { name: prop.name }),
      rank: 600,
    };
  }
  ...
  if (diff < -1) return { key: "relative:past", label: i18next.t("tableView:dateGroup.past"), rank: 100 };
  if (diff === -1) return { key: "relative:yesterday", label: i18next.t("tableView:dateGroup.yesterday"), rank: 200 };
  if (diff === 0) return { key: "relative:today", label: i18next.t("tableView:dateGroup.today"), rank: 300 };
  if (diff === 1) return { key: "relative:tomorrow", label: i18next.t("tableView:dateGroup.tomorrow"), rank: 400 };
  if (diff <= 7) return { key: "relative:next-7", label: i18next.t("tableView:dateGroup.next7"), rank: 500 };
  return { key: "relative:later", label: i18next.t("tableView:dateGroup.later"), rank: 550 };
}
...
export function TableView({
  ...
  const newRowText = t("tableView:newPage");
  const newPageLabel = t("tableView:newPageIn", { title: dbTitle });
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      notify(
        ids.length === 1
          ? t("tableView:movedRowToTrash")
          : t("tableView:movedRowsToTrash", { count: ids.length }),
        "success",
        {
          label: t("tableView:undo"),
          onClick: async () => {
            try {
              for (const id of ids) await restorePage(id);
              notify(
                ids.length === 1
                  ? t("tableView:restoredRow")
                  : t("tableView:restoredRows", { count: ids.length }),
                "success"
              );
            } catch {
              notify(t("tableView:couldntRestoreRows"), "error");
            }
          },
        }
      );
    } catch {
      notify(t("tableView:couldntMoveRowsToTrash"), "error");
    }
  }
  async function duplicateSelected() {
    ...
    try {
      ...
      if (copiedCount === 0) {
        ...
        notify(
          copiedCount === 1
            ? t("tableView:duplicatedRow")
            : t("tableView:duplicatedRows", { count: copiedCount }),
          "success"
        );
      }
    } catch {
  ...
  async function copySelected() {
    ...
    const ok = await copyTextWithMime(text, HANJI_TABLE_ROWS_MIME, JSON.stringify(payload));
    setCopiedRows(ok);
    if (ok) window.setTimeout(() => setCopiedRows(false), 1200);
    notify(
      ok
        ? selectedShown.length === 1
          ? t("tableView:copiedRowToClipboard")
          : t("tableView:copiedRowsToClipboard")
        : t("tableView:couldntCopyRows"),
      ok ? "success" : "error"
    );
  }
  ...
  const selectedRowsLabel = t("tableView:selectedCount", { count: selectedShown.length });
  ...
  const selectionBar =
    selectedShown.length > 0 ? (
      ...
      >
        ...
        <div className={styles.selectionProperties} aria-label={t("tableView:quickPropertyActions")}>
          ...
          {selectionMenuOpen && (
            <div className={styles.selectionMenu} role="menu">
              ...
              >
                ...
                {copiedRows ? t("tableView:copied") : t("tableView:copy")}
              </button>
              <button
                type="button"
                className={styles.selectionMenuItem}
      ...
      >
        ...
        data-row-gutter={showRowGutter ? "true" : undefined}
	      ...
	      >
	        <div
	          ...
	          role="grid"
	          aria-label={t("tableView:tableAriaLabel", { title: pageDisplayTitle(db) })}
	          aria-rowcount={renderItems.length + 1}
	          aria-colcount={
	            visible.length + (showRowGutter ? 1 : 0) + (!readOnly ? 1 : 0)
	          }
	          aria-busy={loadingRows || undefined}
	        >
		        {/* header */}
		        <div
          ...
          {showRowGutter && (
            <div
              className={styles.rowGutterHead}
              role="columnheader"
              aria-label={t("tableView:rowActions")}
            />
          )}
          {visible.map((p, index) => (
            <PropertyHeader
        ...
        {renderItems.map((entry) => {
          if (entry.kind === "group") {
            ...
            return (
              ...
              >
                <div
                  className={styles.tableGroupHeaderInner}
                  role="gridcell"
                  aria-colspan={
                    visible.length + (showRowGutter ? 1 : 0) + (!readOnly ? 1 : 0)
                  }
                >
                  ...
                  {subtotals.length > 0 && (
                    <span
                      ...
                      aria-label={t("tableView:groupSubtotals", { group: entry.group.label })}
                    >
            ...
            >
              {showRowGutter && (
                <div
                  ...
                  data-table-row-gutter-cell
                  role="gridcell"
                  aria-label={t("tableView:rowActionsFor", { title: rowTitle })}
                >
                  <button
                    type="button"
                    className={styles.rowGutterButton}
                    data-table-row-add
                    title={t("tableView:addRowBelow", { title: rowTitle })}
                    aria-label={t("tableView:addRowBelow", { title: rowTitle })}
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <button
                    ...
                    data-table-row-menu
                    title={canReorderRows ? t("tableView:dragRowOrOpenMenu") : t("tableView:openRowMenu")}
                    aria-label={t("tableView:rowMenu", { title: rowTitle })}
              ...
              {visible.map((p, i) => {
                ...
                return (
                  <div
                    ...
                    role="gridcell"
                    aria-colindex={i + 1 + (showRowGutter ? 1 : 0)}
                    ...
                    aria-label={t("tableView:cellAriaLabel", {
                      property: p.name,
                      title: rowTitle,
                      awareness: remoteAwarenessLabel ? `, ${remoteAwarenessLabel}` : "",
                    })}
                    ...
                    {isTitleCell && showOpenRowButton && (
                      <>
                        <button
                          ...
                          data-table-row-open
                          title={t("tableView:openInSidePeek")}
                          aria-label={t("tableView:openRowInSidePeek", {
                            title: rowTitle,
                            action: t("tableView:openInSidePeek"),
                          })}
              ...
              {shouldRenderTrailingGridCell && (
                <div
                  ...
                  data-table-trailing-grid-cell
                  role="gridcell"
                  aria-label={t("tableView:tableActionsFor", { title: rowTitle })}
                />
              )}
            </div>
          );
        ...
        {rowLoadError && (
          <div className={styles.tableLoadError} data-table-load-error role="status">
            <div className={styles.viewEmptyTitle}>{t("tableView:rowLoadErrorTitle")}</div>
        ...
        {shown.length === 0 && rows.length > 0 && (
          <div className={styles.tableEmpty} data-table-empty-results>
            <div className={styles.viewEmptyTitle}>
              {t("tableView:noResultsTitle")}
            </div>
            <div className={styles.viewEmptyDesc}>
              {t("tableView:noResultsDesc")}
            </div>
          </div>
        )}
        ...
        {hasMoreRows && (
          <button
            ...
            aria-label={
              remainingRowCount === undefined
                ? t("tableView:loadMoreAria")
                : t("tableView:loadMoreAriaCount", { count: remainingRowCount })
            }
          ...
          >
            <span className={styles.loadMoreRowInner}>
              ...
              {rowPage?.loadingMore ? t("tableView:loadingMore") : t("tableView:loadMore")}
            </span>
          </button>
        )}
    ...
    >
      ...
      >
        {summary ? (
          ...
          <span>
            {showDefaultRowCount
              ? t("tableView:rowCount", { count: rows.length })
              : t("tableView:calculate")}
          </span>
        )}
      </button>
      {open && menuPos && createPortal(
        <>
          <button
            ...
            tabIndex={-1}
            aria-label={t("tableView:closeCalculationMenu")}
          />
          <div
            ref={menuRef}
            ...
            role="menu"
            tabIndex={-1}
            aria-label={t("tableView:propertyCalculation", { name: prop.name })}
            onKeyDown={(e) => onTableMenuKeyDown(e, menuRef.current, () => closeMenu(true))}
          >
            <div className={styles.propMenuLabel}>{t("tableView:calculate")}</div>
...
function PropertyHeader({
  ...
  async function duplicateProperty() {
    ...
    try {
      ...
      if (!duplicate) {
        notify(t("tableView:couldntDuplicateProperty"), "error");
        return;
      }
      ...
      notify(t("tableView:duplicatedProperty"), "success");
      closeMenu(true);
    } catch {
      notify(t("tableView:couldntDuplicateProperty"), "error");
    }
  ...
  async function remove() {
    if (readOnly) return;
    if (isTitle) return;
    if (!window.confirm(t("tableView:confirmDeleteProperty", { name: prop.name }))) return;
    try {
      const deleted = await deleteProperty(prop.id);
      if (!deleted) {
        notify(t("tableView:couldntDeleteProperty"), "error");
        return;
      }
      updateConfig({
      ...
      notify(t("tableView:deletedProperty"), "success");
      setOpen(false);
    } catch {
      notify(t("tableView:couldntDeleteProperty"), "error");
    }
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={t("tableView:propertyOptionsFor", { name: prop.name })}
        ...
        {prop.description && (
          <>
            ...
            <span id={`property-description-${prop.id}`} className={styles.srOnly}>
              {t("tableView:descriptionPrefix", { description: prop.description })}
            </span>
          </>
        )}
      </button>
      <button
        ...
        aria-label={t("tableView:resizeColumnValue", {
          name: prop.name,
          width: columnWidth(prop, view),
        })}
        aria-keyshortcuts="ArrowLeft ArrowRight Home"
        disabled={readOnly}
        onPointerDown={startResize}
        onKeyDown={(event) => {
          if (readOnly || isComposingKeyEvent(event)) return;
          if (event.key === "Home") {
            event.preventDefault();
            setWidth(prop.type === "title" ? DEFAULT_TITLE_WIDTH : DEFAULT_PROP_WIDTH);
            return;
          }
          if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return;
          event.preventDefault();
          const step = event.shiftKey ? 40 : 10;
          setWidth(columnWidth(prop, view) + (event.key === "ArrowRight" ? step : -step));
        }}
      />
      {descriptionTooltip &&
        createPortal(
      ...
      {open && menuPos && createPortal(
        <>
          <button
            ...
            onClick={() => closeMenu(true)}
            tabIndex={-1}
            aria-label={t("tableView:closePropertyMenu")}
          />
          <div
            ref={menuRef}
            ...
            {menuPanel === "main" && (
              <>
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:changeType")}</span>
                ...
                <button className={styles.propertyHeaderItem} data-menu-item disabled>
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:aiAutofill")}</span>
                  <span className={styles.propertyHeaderMeta}>{t("tableView:comingSoon")}</span>
                  <ChevronRight size={14} aria-hidden="true" />
                </button>
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:sortAscending")}</span>
                ...
                >
                  <SortIcon size={15} aria-hidden="true" />
                  <span className={styles.propertyHeaderItemText}>{t("tableView:sortDescending")}</span>
                ...
                {activeSort && (
                  <button className={styles.propertyHeaderItem} data-menu-item onClick={clearSort}>
                    ...
                    <span className={styles.propertyHeaderItemText}>{t("tableView:clearSort")}</span>
                  </button>
                )}
                <button className={styles.propertyHeaderItem} data-menu-item disabled>
                  <FilterIcon size={15} aria-hidden="true" />
                  <span className={styles.propertyHeaderItemText}>{t("tableView:filter")}</span>
                  <span className={styles.propertyHeaderMeta}>{t("tableView:manageInViewSettings")}</span>
                </button>
                <button
                  className={styles.propertyHeaderItem}
                  data-menu-item
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:hide")}</span>
                </button>
                <button
                  className={styles.propertyHeaderItem}
                  data-menu-item
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>
                    {(view.config?.wrappedColumns ?? []).includes(prop.id)
                      ? t("tableView:unwrapText")
                      : t("tableView:wrapText")}
                  </span>
                </button>
                <div className={styles.propertyHeaderDivider} />
                <div className={styles.propertyHeaderRow}>
                  ...
                  >
                    ...
                    <span className={styles.propertyHeaderItemText}>{t("tableView:moveLeft")}</span>
                  </button>
                  <button
                    className={styles.propertyHeaderItem}
                    data-menu-item
                  ...
                  >
                    ...
                    <span className={styles.propertyHeaderItemText}>{t("tableView:moveRight")}</span>
                  </button>
                </div>
                <button
                  className={styles.propertyHeaderItem}
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:duplicateProperty")}</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className={styles.propertyHeaderItemText}>{t("tableView:deleteProperty")}</span>
                </button>
              </>
            )}
            {menuPanel === "edit" && (
              <div className={styles.propertyHeaderPanel}>
                <div className={styles.propertyHeaderPanelHead}>
                  ...
                  <strong>{t("tableView:editProperty")}</strong>
                  <button type="button" aria-label={t("tableView:closePropertyMenu")} onClick={() => closeMenu(true)}>
                    <X size={15} aria-hidden="true" />
                  </button>
                </div>
                <label className={styles.propertyHeaderField}>
                  <span>{t("tableView:name")}</span>
                  <input
                    value={prop.name}
                    autoFocus
                ...
                <label className={styles.propertyHeaderField}>
                  <span>{t("tableView:description")}</span>
                  <textarea
                    value={prop.description ?? ""}
                    placeholder={t("tableView:addDescription")}
                    rows={2}
                    onChange={(e) =>
            ...
            {menuPanel === "type" && (
              <div className={styles.propertyHeaderPanel}>
                <div className={styles.propertyHeaderPanelHead}>
                  ...
                  <strong>{t("tableView:changeType")}</strong>
                  <button type="button" aria-label={t("tableView:closePropertyMenu")} onClick={() => closeMenu(true)}>
                    <X size={15} aria-hidden="true" />
                  </button>
                </div>
                ...
                <div className={styles.propertyHeaderTypeList}>
                  ...
                  {filteredPropertyTypeOptions.length === 0 && (
                    <div className={styles.propertiesEmpty}>{t("tableView:noSearchResults")}</div>
                  )}
                </div>
              </div>
            )}
...
function AddPropertyButton({
  ...
  return (
    <div
      ...
      data-add-property-column
      role="columnheader"
      aria-label={t("tableView:tableActions")}
    >
      <button
        ref={btnRef}
        type="button"
        className={styles.addColBtn}
        aria-label={t("tableView:addProperty")}
        aria-haspopup="menu"
        aria-expanded={open}
        onClick={() => {
          if (open) {
        ...
        title={t("tableView:addProperty")}
      >
        <Plus size={15} aria-hidden="true" />
        <span className={styles.addColText}>{t("tableView:addProperty")}</span>
      </button>
      <button
        type="button"
        className={styles.addColMoreBtn}
        aria-label={t("tableView:propertyOptions")}
        aria-haspopup="menu"
        aria-expanded={open}
        onClick={() => {
          if (open) {
        ...
        title={t("tableView:propertyOptions")}
      >
        ...
      </button>
      {open &&
        ...
        createPortal(
          <>
            <button
              ...
              onClick={() => closeMenu(true)}
              tabIndex={-1}
              aria-label={t("tableView:closeAddPropertyMenu")}
            />
            <div
              ref={menuRef}
              ...
              role="menu"
              tabIndex={-1}
              aria-label={t("tableView:newPropertyType")}
              onKeyDown={(e) => onTableMenuKeyDown(e, menuRef.current, () => closeMenu(true))}
            >
              <div className={styles.propMenuLabel}>{t("tableView:newProperty")}</div>
              ...
              {filteredPropertyTypes.map((option) => (
                <button
                  ...
                  data-menu-item
                  role="menuitem"
                  onClick={() => addSelectedProperty(option.type, option.label)}
                >
                  <span className={styles.propGlyph} aria-hidden="true">
                    <PropertyTypeIcon type={option.type} size={15} />
                  </span>
                  <span>{option.label}</span>
                </button>
              ))}
              {filteredPropertyTypes.length === 0 && (
                <div className={styles.propMenuEmpty}>{t("tableView:noPropertyTypesFound")}</div>
              )}
            </div>
          </>,
          document.body
