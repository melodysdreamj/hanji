...
const ROW_PROPERTY_DRAG = "application/x-notionlike-row-property";
const DEFAULT_VISIBLE_ROW_PROPERTY_COUNT = 10;

const ROW_PROPERTIES_LABELS = {
  en: {
    ...
    closeCustomizeMenu: "Close customize properties menu",
    customizeProperties: "Customize properties",
    deleteProperty: "Delete property",
    empty: "Empty",
    hideProperties: "Hide properties",
    hideWhenEmpty: "Hide when empty",
    linkedTo: "Linked to",
    moreProperties: (count: number) => `${count} more properties`,
  ...
  ko: {
    ...
    closeCustomizeMenu: "속성 맞춤 설정 메뉴 닫기",
    customizeProperties: "속성 맞춤 설정",
    deleteProperty: "속성 삭제",
    empty: "비어 있음",
    hideProperties: "속성 숨기기",
    hideWhenEmpty: "빈 값 숨기기",
    linkedTo: "연결된 위치",
    moreProperties: (count: number) => `속성 ${count}개 더 보기`,
    newProperty: "새 속성",
    newPropertyType: "새 속성 유형",
    noPropertiesFound: "속성을 찾을 수 없음",
    noPropertyTypesFound: "속성 유형을 찾을 수 없음",
    properties: "속성",
...
function rowPropertiesLabels() {
  return pickLabels(ROW_PROPERTIES_LABELS);
}
...
export function RowProperties({
  ...
  async function createProperty(type: PropertyType, label: string) {
    ...
    try {
      ...
      notify(rowPropertiesLabels().createdProperty(prop.name), "success");
      setAddOpen(false);
      setAddSearch("");
      window.requestAnimationFrame(() => addButtonRef.current?.focus());
    } catch {
      notify(rowPropertiesLabels().createPropertyFailed, "error");
    }
  }
    ...
    return (
      <>
        <button
          ...
          tabIndex={-1}
          aria-label={rowPropertiesLabels().closeCustomizeMenu}
        />
        <div
          ...
          role="menu"
          tabIndex={-1}
          aria-label={rowPropertiesLabels().customizeProperties}
          onKeyDown={(e) => onMenuKeyDown(e, customizeMenuRef.current)}
        >
          <div className={styles.propMenuLabel}>{rowPropertiesLabels().properties}</div>
          <div className={styles.rowCustomizeSearch}>
            ...
            <input
              ...
              type="text"
              placeholder={rowPropertiesLabels().searchProperties}
              aria-label={rowPropertiesLabels().searchProperties}
              ...
              return (
                <div key={prop.id} className={styles.rowCustomizeItem}>
                  ...
                  >
                    ...
                    <span>{rowPropertiesLabels().hideWhenEmpty}</span>
                  </button>
                </div>
              );
            })}
            {filteredCustomizeProps.length === 0 && (
              <div className={styles.rowCustomizeEmptyState}>{rowPropertiesLabels().noPropertiesFound}</div>
            )}
          </div>
        </div>
      </>
  ...
  return (
    <div className={styles.rowProperties}>
      {displayedProps.length === 0 && hiddenCount === 0 && (
        <div className={styles.rowPropertiesEmpty}>No visible properties yet.</div>
      )}
            ...
            {canOpenPropertyMenus && propertyMenuId === prop.id && (
              <>
                ...
                >
                  ...
                  >
                    <span>{prop.config?.hideInPagePanel ? "Show property" : "Hide property"}</span>
                  </button>
                  <button
                    type="button"
                    className={styles.propertyHeaderItem}
                  ...
                  >
                    {rowPropertiesLabels().deleteProperty}
                  </button>
                </div>
              </>
            )}
      ...
      {hiddenCount > 0 && (
        ...
        >
          ...
          {showHiddenProperties
            ? rowPropertiesLabels().hideProperties
            : rowPropertiesLabels().moreProperties(hiddenCount)}
        </button>
      )}
        ...
        >
          {canShowPropertyManagementControls && (
            ...
            >
              ...
              <span>{rowPropertiesLabels().customizeProperties}</span>
            </button>
          )}
          {customizeOpen &&
      ...
      {showBackReferences && backReferences.length > 0 && (
        <div className={styles.rowBackrefs}>
          <div className={styles.rowBackrefsLabel}>{rowPropertiesLabels().linkedTo}</div>
      ...
      {canShowPropertyManagementControls && (
        <div className={styles.rowAddPropertyWrap}>
          ...
          >
            ...
            <span>{rowPropertiesLabels().addProperty}</span>
          </button>
          {addOpen && (
            <>
              <button
                ...
                onClick={() => closeMenus(true)}
                tabIndex={-1}
                aria-label={rowPropertiesLabels().closeAddPropertyMenu}
              />
              <div
                ref={addMenuRef}
                ...
                role="menu"
                tabIndex={-1}
                aria-label={rowPropertiesLabels().newPropertyType}
                onKeyDown={(e) => onMenuKeyDown(e, addMenuRef.current)}
              >
                <div className={styles.propMenuLabel}>{rowPropertiesLabels().newProperty}</div>
                <label className={styles.propMenuSearch}>
                  ...
                  <input
                    ...
                    placeholder={rowPropertiesLabels().searchPropertyTypes}
                    aria-label={rowPropertiesLabels().searchPropertyTypes}
                  />
                </label>
                ...
                {filteredAddTypes.length === 0 && (
                  <div className={styles.propMenuEmpty}>{rowPropertiesLabels().noPropertyTypesFound}</div>
                )}
              </div>
            </>
          )}
