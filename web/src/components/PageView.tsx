...
function ContentFallback() {
  ...
  return (
    <div
      className={styles.contentFallback}
      aria-busy="true"
      aria-label={t("pageView:loadingPageContent")}
    />
  );
}
...
export function PageView({
  ...
  async function restoreCurrentPage() {
    try {
      ...
      notify(t("pageView:restoredPage"), "success");
    } catch {
      notify(t("pageView:couldntRestorePage"), "error");
    }
  }
  ...
  if (!page) {
    return (
      <>
        <TopBar title={t("pageView:notFoundTopBarTitle")} />
        <div className={styles.missing}>
          <strong>{t("pageView:pageUnavailable")}</strong>
          <p>{t("pageView:pageUnavailableDetail")}</p>
          <div className={styles.missingActions}>
            <button type="button" className={styles.restoreButton} onClick={() => router.push("/")}>
              {t("pageView:openWorkspace")}
            </button>
          </div>
        </div>
      </>
  ...
  if (page.inTrash) {
    return (
      <>
        <TopBar title={t("pageView:inTrashTopBarTitle")} />
        <div className={styles.missing}>
          <strong>{t("pageView:pageInTrash")}</strong>
          <p>{t("pageView:pageInTrashDetail")}</p>
          <div className={styles.missingActions}>
            <button
              type="button"
