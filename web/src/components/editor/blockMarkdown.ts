...
function dateMentionHref(date: string) {
  return `hanji://date/${encodeURIComponent(date)}`;
}

function personMentionHref(userId: string) {
  return `hanji://person/${encodeURIComponent(userId)}`;
}
...
function tabLabelHtml(block: Block, childrenHtml: string) {
  ...
  const iconAttr = icon ? ` data-hanji-tab-icon="${escapeHtml(icon)}"` : "";
  const iconHtml = icon
    ? `<span data-hanji-tab-icon-text="true">${escapeHtml(icon)}</span> `
    : "";
  const titleHtml =
    blockTextHtml(block) ||
    escapeHtml(block.plainText?.trim() || activePersistentGeneratedLabels().untitled);
  return `<section data-hanji-tab-label="true"${iconAttr}><p data-hanji-tab-title="true">${iconHtml}${titleHtml}</p>${childrenHtml ? `<div data-hanji-tab-panel="true">${childrenHtml}</div>` : ""}</section>`;
}
...
function blockHtml(block: Block, childrenHtml: string) {
  ...
  switch (block.type) {
    ...
    case "tab":
      return `<div${style} data-hanji-block-type="tab"><strong>Tabs</strong>${childrenHtml}</div>`;
    case "column_list":
...
function tabLabelMarkdown(block: Block) {
  const icon = typeof block.content?.icon === "string" ? block.content.icon.trim() : "";
  if (!icon) return "";
  const label = blockMarkdown(block).trim() || activePersistentGeneratedLabels().untitled;
...
export function blocksClipboardHtml(innerHtml: string) {
  return `<html><body><!--StartFragment--><div data-hanji-copy="true">${innerHtml}</div><!--EndFragment--></body></html>`;
}
