...
import { getDef, matchBlocks, MD_SHORTCUTS, TEXT_BLOCKS, type BlockDef } from "./blocks";
...
function closeCompetingPageLinkMenus() {
  ...
  document
    .querySelectorAll<HTMLButtonElement>(
      [
        'button[aria-label="Close block actions"]',
        'button[aria-label="Close database row actions"]',
        'button[aria-label="Close page actions"]',
      ].join(",")
    )
...
const PASTED_URL_MENU_REQUEST = "notionlike:pasted-url-menu-request";
...
// User-facing copy for this file, resolved through the shared pickLabels
// locale switch (EN default). ko values are the original literals.
const BLOCK_ITEM_LABELS = {
  en: {
    addView: "Add a view",
    cantCopyBlockHere: "Can't copy block here",
    cantMoveBlockHere: "Can't move block here",
    copiedBlocks: (count: number) =>
      count === 1 ? "Copied block" : `Copied ${count} blocks`,
    copyViewLink: "Copy view link",
    couldntCut: "Couldn't cut",
    cutBlocks: (count: number) => (count === 1 ? "Cut block" : `Cut ${count} blocks`),
    dateDisplayLocale: "en-US",
    databaseNotReady: "The database isn't ready yet",
    databaseTitleHidden: "Database title hidden",
    deletedBlocks: (count: number) =>
      count === 1 ? "Deleted block" : `Deleted ${count} blocks`,
    duplicatedBlocks: (count: number) =>
      count === 1 ? "Duplicated block" : `Duplicated ${count} blocks`,
    duplicateView: "Duplicate view",
    editIcon: "Edit icon",
    editLayout: "Edit layout",
    editTitle: "Edit title",
    emptyTogglePrompt: "Empty toggle. Click or drag a block inside it.",
    groupDate: "Date",
    groupLinkToPage: "Link to page",
    groupNewPage: "New page",
    groupPeople: "People",
    hideTitle: "Hide title",
    manageInCalendar: "Manage in calendar",
    mentionToday: "Today",
    mentionTomorrow: "Tomorrow",
    mentionYesterday: "Yesterday",
    movedBlocks: (count: number) =>
      count === 1 ? "Moved block" : `Moved ${count} blocks`,
    nothingToUndo: "Nothing to undo",
    openDatabase: (dbTitle: string) => `Open ${dbTitle} database`,
    restoredBlocks: (count: number) =>
      count === 1 ? "Restored block" : "Restored blocks",
    undidCopy: "Undid copy",
    undidDuplicate: "Undid duplicate",
    undidMove: "Undid move",
    undo: "Undo",
    uploadComplete: "Complete",
    uploadedFiles: (count: number) =>
      count === 1 ? "Uploaded file" : `Uploaded ${count} files`,
    uploadFailed: (fileName: string) => `Could not upload ${fileName || "that file"}.`,
    uploadFileTooLarge: "File is too large. The current upload limit is 5 GB.",
    uploadFinalizing: "Finalizing",
    uploadingFile: (fileName: string) => `Uploading ${fileName}`,
  ...
  ko: {
    addView: "새 보기 추가",
    cantCopyBlockHere: "여기에는 블록을 복사할 수 없어요",
    cantMoveBlockHere: "여기로는 블록을 옮길 수 없어요",
    copiedBlocks: (count: number) =>
      count === 1 ? "블록을 복사했어요" : `블록 ${count}개를 복사했어요`,
    copyViewLink: "보기 링크 복사",
    couldntCut: "잘라내지 못했어요",
    cutBlocks: (count: number) =>
      count === 1 ? "블록을 잘라냈어요" : `블록 ${count}개를 잘라냈어요`,
    dateDisplayLocale: "ko-KR",
    databaseNotReady: "데이터베이스가 아직 준비되지 않았어요",
    databaseTitleHidden: "데이터베이스 제목을 숨겼어요",
    deletedBlocks: (count: number) =>
      count === 1 ? "블록을 삭제했어요" : `블록 ${count}개를 삭제했어요`,
    duplicatedBlocks: (count: number) =>
      count === 1 ? "블록을 복제했어요" : `블록 ${count}개를 복제했어요`,
    duplicateView: "보기 복제",
    editIcon: "아이콘 편집",
    editLayout: "레이아웃 편집",
    editTitle: "제목 편집",
    emptyTogglePrompt: "빈 토글입니다. 클릭하거나 블록을 내부로 드래그하세요.",
    groupDate: "날짜",
    groupLinkToPage: "페이지 링크",
    groupNewPage: "새 페이지",
    groupPeople: "사람",
    hideTitle: "제목 숨기기",
    manageInCalendar: "캘린더에서 관리하기",
    mentionToday: "오늘",
    mentionTomorrow: "내일",
    mentionYesterday: "어제",
    movedBlocks: (count: number) =>
      count === 1 ? "블록을 옮겼어요" : `블록 ${count}개를 옮겼어요`,
    nothingToUndo: "실행 취소할 항목이 없어요",
    openDatabase: (dbTitle: string) => `${dbTitle} 데이터베이스 열기`,
    restoredBlocks: (count: number) =>
      count === 1 ? "블록을 복원했어요" : "블록을 복원했어요",
    undidCopy: "복사를 취소했어요",
    undidDuplicate: "복제를 취소했어요",
    undidMove: "이동을 취소했어요",
    undo: "실행 취소",
    uploadComplete: "완료",
    uploadedFiles: (count: number) =>
      count === 1 ? "파일을 업로드했어요" : `파일 ${count}개를 업로드했어요`,
    uploadFailed: (fileName: string) =>
      fileName ? `${fileName} 파일을 업로드하지 못했어요.` : "파일을 업로드하지 못했어요.",
    uploadFileTooLarge: "파일이 너무 커요. 현재 업로드 한도는 5 GB예요.",
    uploadFinalizing: "마무리 중",
    uploadingFile: (fileName: string) => `${fileName} 업로드 중`,
    uploadPreparing: "업로드 준비 중",
    uploadUploading: "업로드 중",
    viewDataSource: "데이터 소스 보기",
  },
} as const;

function blockItemLabels() {
  return pickLabels(BLOCK_ITEM_LABELS);
}
...
function inlineDatabaseTitleWidth(title: string): string {
  const units = Array.from(title || "New database").reduce((width, char) => {
...
function blockTypeForPastedAssetUrl(url: string): Extract<PastedUrlConversion, "image" | "video" | "audio" | "file"> | null {
  ...
  let pathname = lower;
  try {
    pathname = new URL(url, "https://notionlike.local").pathname.toLowerCase();
  } catch {
...
const BUTTON_TEMPLATE_TYPES: { type: BlockType; label: string }[] = [
...
function blockAriaLabel(block: Block) {
  const label = getDef(block.type).label;
  const summary = shortBlockText(block);
  return summary ? `${label} block: ${summary}` : `${label} block`;
}

function blockTextBoxLabel(block: Block) {
  return `${getDef(block.type).label} block text`;
}
...
function blockControlFocusSelector(block: Block) {
  ...
  if (block.type === "image") {
    if (!block.content?.url) return 'input[aria-label="Image link"]';
    return blockCaptionVisible(block) ? '[role="textbox"][aria-label="Image caption"]' : null;
  }
  if (block.type === "video") {
    if (!block.content?.url) return 'input[aria-label="Video link"]';
    return blockCaptionVisible(block) ? '[role="textbox"][aria-label="Video caption"]' : null;
  }
  if (block.type === "audio") {
    if (!block.content?.url) return 'input[aria-label="Audio link"]';
    return blockCaptionVisible(block) ? '[role="textbox"][aria-label="Audio caption"]' : null;
  }
  if (block.type === "bookmark") {
    return block.content?.url ? null : 'input[aria-label="Bookmark link"]';
  }
  if (block.type === "embed") {
    if (!block.content?.url) return 'input[aria-label="Embed link"]';
    return blockCaptionVisible(block) ? '[role="textbox"][aria-label="Embed caption"]' : null;
  }
  if (block.type === "file") {
    if (!block.content?.url) return 'input[aria-label="File link"]';
    return blockCaptionVisible(block) ? '[role="textbox"][aria-label="File caption"]' : null;
  }
  return null;
}
  ...
  async function uploadDroppedFiles(files: File[]) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const plainText = file.name || uploaded.name || getDef(type).label;
      ...
      const lines = (st.blocksByPage[block.pageId] ?? [])
        ...
        .map(
          (target) =>
            ...
            getDef(target.type).label
        )
        .filter(Boolean);
      const quote = lines.slice(0, 6).join("\n");
      const suffix = lines.length > 6 ? "\n..." : "";
      return quote ? `${quote}${suffix}` : `${ops.selectedBlockIds.size} selected blocks`;
    };
      ...
      >
        ...
        {commentCount > 0 && (
          <button
            ...
            aria-label={`${commentCount} unresolved comment${commentCount === 1 ? "" : "s"} on ${getDef(block.type).label} block`}
          >
            {commentCount}
          </button>
        )}
        ...
        TOGGLE_BLOCKS.has(block.type) && (
          <div className={styles.children}>
            <button
              ...
              contentEditable={false}
              aria-label="Add a block inside this empty toggle"
              title="Add a block inside this toggle"
...
function TabBlock({
  ...
  function commitRename(tab: Block) {
    if (ops.readOnly) return;
    const label = editingLabel.trim() || "Untitled";
    updateBlock(tab.id, {
    ...
    >
      {childBlocks.length > 0 ? (
        ...
        <button
          ...
          className={styles.toggleEmptyChild}
          contentEditable={false}
          aria-label="Add a block inside this empty tab"
          title="Add a block inside this tab"
...
function ColumnSlot({
  ...
  return (
    <div
      ...
      role="group"
      aria-label={`Column ${columnNumber}`}
    ...
    >
      ...
      {canManage && typeof columnIndex === "number" && (
        <div className={styles.columnControls} contentEditable={false}>
          <button
            type="button"
            aria-label={`Add column after column ${columnNumber}`}
            title="Add column"
          ...
          <button
            type="button"
            aria-label={`Move column ${columnNumber} left`}
          ...
          <button
            type="button"
            aria-label={`Move column ${columnNumber} right`}
          ...
          {managedColumns.length > 2 && (
            <button
              type="button"
              aria-label={`Delete column ${columnNumber}`}
              title="Delete column"
...
function displayUrl(url: string) {
  try {
    const parsed = new URL(url, window.location.origin);
    if (parsed.origin === window.location.origin && url.startsWith("/")) return "Page link";
...
function fileNameFromUrl(url: string) {
  try {
    ...
    return url.split("/").filter(Boolean).at(-1) || "Untitled";
  }
}
...
function organizationProfileMentionLabel(profile: OrganizationProfile) {
  return profile.displayName?.trim() || profile.email?.trim() || profile.userId?.trim() || "Person";
}

function organizationProfileMentionDescription(profile: OrganizationProfile) {
  const parts = [
    profile.email?.trim(),
    profile.organizationRole ? `${profile.organizationRole} in organization` : null,
    profile.status && profile.status !== "active" ? profile.status : null,
  ].filter(Boolean);
  return parts.join(" - ") || "Organization person";
}
  ...
  return (
    <BlockFrame block={block} ops={ops} depth={depth}>
      <div className={styles.simpleTableWrap} contentEditable={false}>
        <div className={styles.simpleTableScroller} role="region" aria-label="Simple table">
          <div className={styles.simpleTableCanvas}>
            <table
              className={styles.simpleTable}
              aria-label={`${table.length} by ${table[0]?.length ?? 0} simple table`}
            >
              <tbody>
                      ...
                      return (
                        ...
                        >
                          <SimpleTableCell
                            ...
                            value={cell}
                            placeholder={
                              rowIndex === 0 && colIndex === 0 ? "Type something" : ""
                            }
                            readOnly={ops.readOnly}
            ...
            {!ops.readOnly && (
              <>
                <button
                  ...
                  aria-label="Add column to simple table"
                  title="Add column"
                ...
                <button
                  ...
                  aria-label="Add row to simple table"
                  title="Add row"
                  onClick={() => addRow(true)}
                >
                  <Plus size={14} aria-hidden="true" />
        ...
        {!ops.readOnly && (
          <div className={styles.simpleTableTools} aria-label="Simple table actions">
            <button
              type="button"
              aria-label="Remove last row from simple table"
            ...
            <button
              type="button"
              aria-label="Remove last column from simple table"
...
function SimpleTableCell({
  ...
  const cellLabel =
    Number.isFinite(rowIndex) && Number.isFinite(columnIndex)
      ? `Table cell ${rowIndex + 1}, ${columnIndex + 1}`
      : "Table cell";
...
function ImageBlock({
  ...
  async function pickImageFile(file?: File) {
    ...
    try {
      setError("");
      setUploadProgress({ phase: "preparing", percent: 0, fileName: file.name || "Image" });
      uploadedUrl = (await uploadWorkspaceFile(file, "blocks/images", {
        pageId: block.pageId,
        blockId: block.id,
      }, {
        onProgress: (progress) => setUploadProgress({ ...progress, fileName: file.name || "Image" }),
      })).url;
    } catch (err) {
      setUploadProgress(null);
      setError(blockUploadErrorMessage(err, "Could not upload that image file."));
      return;
    }
    setError("");
    setDraft("");
  ...
  const body = url ? (
    ...
    >
      ...
      <div className={styles.imageEmptyBody}>
        <div id={titleId} className={styles.imageEmptyTitle}>Embed image</div>
        <div id={hintId} className={styles.imageEmptyHint}>Upload an image or paste a link to display it here.</div>
        <input
          ref={fileInputRef}
      ...
      {previewOpen && url && (
        ...
        >
          ...
          <img
            ...
            src={url}
            alt={spansToPlainText(block.content?.caption) || ""}
          />
        </div>
      )}
    </>
...
function MediaBlock({
  ...
  const title = isVideo ? "Embed video" : "Embed audio";
  const hint = isVideo
    ? "Paste a YouTube/Vimeo/Loom link or a direct video file."
    : "Paste an audio link to play it here.";
  const captionText = blockCaptionText(block);
  const showCaption = blockCaptionVisible(block);
  ...
  function commitMedia(e: React.FormEvent<HTMLFormElement>) {
    ...
    const normalized = safeUrl(draft);
    if (!normalized || normalized.startsWith("#") || /^mailto:/i.test(normalized)) {
      setError(`Paste a valid ${kind} link.`);
      return;
    }
    setError("");
    updateBlock(block.id, {
  ...
  async function pickMediaFile(file?: File) {
    ...
    let uploadedUrl = "";
    try {
      const fallbackName = isVideo ? "Video" : "Audio";
      setError("");
      ...
      setUploadProgress(null);
      setError(blockUploadErrorMessage(err, `Could not upload that ${kind} file.`));
      return;
    }
    setError("");
    setDraft("");
    ...
    <figure className={isVideo ? styles.videoBlock : styles.audioBlock} contentEditable={false}>
      <div className={isVideo ? styles.videoFrame : styles.audioFrame}>
        ...
        <div className={styles.mediaActions}>
          <button
            type="button"
            aria-label={isVideo ? "Replace video" : "Replace audio"}
            onClick={() => {
              setDraft(url.startsWith("data:") ? "" : url);
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            aria-label={`Open ${kind} link in a new tab`}
          >
            Open
          </a>
        </div>
      ...
      {showCaption && (
        <figcaption
          ...
          contentEditable={!ops.readOnly}
          role="textbox"
          aria-label={isVideo ? "Video caption" : "Audio caption"}
          aria-readonly={ops.readOnly}
          aria-multiline="false"
          aria-placeholder="Add a caption"
          suppressContentEditableWarning
    ...
    >
      ...
      <div className={styles.imageEmptyBody}>
        ...
        <div id={hintId} className={styles.imageEmptyHint}>
          Upload a {kind} file or {hint.toLowerCase()}
        </div>
        <input
          ref={fileInputRef}
          className={styles.hiddenFileInput}
        ...
        <div className={styles.imageInputRow}>
          <input
            type="url"
            value={draft}
            aria-label={`${isVideo ? "Video" : "Audio"} link`}
            aria-invalid={!!error}
            aria-describedby={error ? errorId : hintId}
            onChange={(e) => {
              setDraft(e.target.value);
...
function BookmarkBlock({
  ...
  function commitBookmark(e: React.FormEvent<HTMLFormElement>) {
    ...
    const normalized = safeUrl(draft);
    if (!normalized || normalized.startsWith("#") || /^mailto:/i.test(normalized)) {
      setError("Paste a valid web link.");
      return;
    }
    setError("");
    updateBlock(block.id, {
  ...
  const body = url ? (
    <div className={styles.bookmarkWrap} contentEditable={false}>
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label={`Open bookmark ${displayUrl(url)} in a new tab`}
      >
        <span className={styles.bookmarkContent}>
          ...
          <span className={styles.bookmarkUrl}>Link · {url}</span>
        </span>
        <span className={styles.bookmarkThumb}>↗</span>
      </a>
      <div className={styles.bookmarkActions}>
        <a href={url} target="_blank" rel="noreferrer" aria-label="Open bookmark in a new tab">
          <OpenInNew size={13} aria-hidden="true" />
          Open
        </a>
        <button type="button" aria-label="Copy bookmark link" onClick={() => void copyBookmarkLink()}>
          {copied ? "Copied" : "Copy link"}
        </button>
        <button
          type="button"
          aria-label="Replace bookmark link"
          onClick={() => {
            setDraft(url);
            updateBlock(block.id, { content: { ...block.content, url: "" }, plainText: "" });
          }}
    ...
    >
      ...
      <div className={styles.imageEmptyBody}>
        <div id={titleId} className={styles.imageEmptyTitle}>Embed bookmark</div>
        <div id={hintId} className={styles.imageEmptyHint}>Paste a web link to create a bookmark card.</div>
        <div className={styles.imageInputRow}>
          <input
            type="url"
            value={draft}
...
function EmbedBlock({
  ...
  function commitEmbed(e: React.FormEvent<HTMLFormElement>) {
    ...
    const normalized = safeUrl(draft);
    if (!normalized || normalized.startsWith("#") || /^mailto:/i.test(normalized)) {
      setError("Paste a valid embed link.");
      return;
    }
    setError("");
  ...
  const embedSrc = url ? providerEmbedUrl(url) ?? url : "";
  const body = url ? (
          ...
          >
            ...
            <div className={styles.embedActions}>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label="Open embed link in a new tab"
              >
                Open
              </a>
              {!ops.readOnly && (
                <button
                  ...
                  aria-label="Open embed block actions"
                  aria-haspopup="menu"
    ...
    >
      ...
      <div className={styles.imageEmptyBody}>
        <div id={titleId} className={styles.imageEmptyTitle}>Embed link</div>
        <div id={hintId} className={styles.imageEmptyHint}>Paste a link to embed content on the page.</div>
        <div className={styles.imageInputRow}>
          <input
            type="url"
            value={draft}
...
function FileBlock({
  ...
  const errorId = useId();
  const url = useWorkspaceFileUrl(block.content?.url);
  const fileName = block.content?.fileName || (url ? fileNameFromUrl(url) : "Untitled");
  const captionText = blockCaptionText(block);
  const showCaption = blockCaptionVisible(block);
  ...
  function commitFile(e: React.FormEvent<HTMLFormElement>) {
    ...
    const normalized = safeUrl(draft);
    if (!normalized || normalized.startsWith("#") || /^mailto:/i.test(normalized)) {
      setError("Paste a valid file link.");
      return;
    }
  ...
  async function pickAttachedFile(file?: File) {
    ...
    try {
      setError("");
      setUploadProgress({ phase: "preparing", percent: 0, fileName: file.name || "File" });
      uploadedUrl = (await uploadWorkspaceFile(file, "blocks/files", {
        pageId: block.pageId,
        blockId: block.id,
      }, {
        onProgress: (progress) => setUploadProgress({ ...progress, fileName: file.name || "File" }),
      })).url;
    } catch (err) {
      setUploadProgress(null);
      setError(blockUploadErrorMessage(err, "Could not upload that file."));
      return;
    }
    setError("");
    setDraft("");
  ...
  const body = url ? (
    ...
    >
      ...
      <div className={styles.imageEmptyBody}>
        <div id={titleId} className={styles.imageEmptyTitle}>Attach file</div>
        <div id={hintId} className={styles.imageEmptyHint}>Upload a file or paste a file link to attach it here.</div>
        <input
          ref={fileInputRef}
          className={styles.hiddenFileInput}
          type="file"
...
function ChildPageBlock({
  ...
  const title = pageSnapshot?.title?.trim() || block.plainText || "Untitled";
      ...
      <span className={styles.childPageWrap} contentEditable={false}>
        ...
        <span className={styles.childPageActions}>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            aria-label="Open child page in a new tab"
          ...
          <button
            type="button"
            aria-label={copied ? "Copied child page link" : "Copy child page link"}
...
function linkedPageSnapshotFromBlock(block: Block, fallbackKind: Page["kind"]): Page | undefined {
  ...
  return {
    ...
    parentType: "page",
    kind,
    title: title || block.plainText || "Untitled",
    icon,
    iconType,
    position: 0,
...
function LinkToPageBlock({
  ...
  const currentPageTitle = currentPage ? pageTitle(currentPage) : "current page";
      ...
      {linkedPage ? (
        <div className={styles.linkToPageRow}>
          ...
          <span className={styles.linkToPageActions}>
            ...
            <button
              type="button"
              aria-label={copied ? "Copied linked page URL" : "Copy linked page URL"}
              title={copied ? "Copied" : "Copy link"}
      ...
      {pickerOpen && (
        <>
          <button
            ...
            aria-label="Close page link picker"
            ...
            >
              ...
              {canCreate && (
                ...
                >
                  <span className={styles.childPageIcon}>＋</span>
                  <span>
                    <span>New page in {currentPageTitle}</span>
                    <span>{createTitle}</span>
                  </span>
                </button>
              )}
              {itemCount === 0 && <div className={styles.pageLinkEmpty}>No pages found.</div>}
            </div>
          </div>
        </>
      )}
      ...
      <span className={styles.childPageWrap} contentEditable={false}>
        ...
        <span className={styles.childPageActions}>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            aria-label="Open child database in a new tab"
            title="Open in new tab"
          >
            <OpenInNew size={13} aria-hidden="true" />
          </a>
          <button
            type="button"
            aria-label={copied ? "Copied child database link" : "Copy child database link"}
            title={copied ? "Copied" : "Copy link"}
...
function TableOfContentsBlock({
    ...
    JSON.stringify(
      orderedBlocks(s.blocksByPage[block.pageId] ?? [])
        ...
        .map((b) => ({
          ...
          title: spansToPlainText(b.content?.rich).trim() || "Untitled",
        }))
    )
  );
  ...
  return (
    <BlockFrame block={block} ops={ops} depth={depth}>
      <nav className={styles.toc} contentEditable={false}>
        {headings.length === 0 ? (
          <div className={styles.tocEmpty}>Add headings to create a table of contents.</div>
        ) : (
    ...
    >
      <section className={styles.syncedBlock} data-copy={isCopy ? "true" : undefined}>
        <div className={styles.syncedHeader} contentEditable={false}>
          ...
          <span>{isCopy ? "Synced copy" : "Synced block"}</span>
        ...
        <div className={styles.syncedContent} contentEditable={isCopy ? false : undefined}>
          {!source ? (
            <div className={styles.syncedMissing}>Original block is unavailable.</div>
          ) : sourceChildren.length === 0 ? (
            <div className={styles.syncedMissing}>Empty synced block.</div>
          ) : isCopy ? (
  ...
  if (TEXT_BLOCKS.has(block.type)) {
    if (block.type === "to_do") {
      body = (
        <label className={styles.syncedPreviewTodo} data-checked={block.content?.checked ? "true" : undefined}>
          ...
          <span
            ...
            contentEditable={!readOnly}
            role="textbox"
            aria-label="Synced to-do text"
            aria-readonly={readOnly}
            aria-multiline="true"
            aria-placeholder="To-do"
            suppressContentEditableWarning
            spellCheck
            data-rt-editable="true"
      ...
      body = (
        <div
          ...
          contentEditable={!readOnly}
          role="textbox"
          aria-label={`Synced ${blockTextBoxLabel(block)}`}
          aria-readonly={readOnly}
          aria-multiline="true"
          aria-placeholder={getDef(block.type).placeholder ?? ""}
          suppressContentEditableWarning
          spellCheck
          data-rt-editable="true"
          data-placeholder={getDef(block.type).placeholder ?? ""}
          onInput={readOnly ? undefined : onTextInput}
        />
      );
    }
    ...
    body = (
      <div className={styles.syncedPreviewText} data-type="paragraph">
        {text || block.plainText || getDef(block.type).label}
      </div>
    );
  }
...
function normalizeButtonTemplates(templates?: ButtonTemplateBlock[]) {
  return templates && templates.length > 0
    ? templates
    : [makeButtonTemplate("to_do", "New task")];
}
...
function ButtonBlock({
  ...
  const label = block.content?.buttonLabel ?? block.plainText ?? "New button";
  ...
  function removeTemplate(index: number) {
    ...
    saveConfig({ templates: next.length > 0 ? next : [makeButtonTemplate("to_do", "New task")] });
  }

  function addTemplate() {
    saveConfig({
      templates: [...draftTemplatesRef.current, makeButtonTemplate("paragraph", "New content")],
    });
  }
        ...
        <button
          ...
          title={
            isPartialNotionButton && templates.length === 0
              ? "Notion did not expose this imported button action."
              : undefined
          }
        ...
        {configOpen && (
          <>
            <button
              type="button"
              className={styles.menuBackdrop}
              aria-label="Close button configuration"
            ...
            >
              ...
              <div className={styles.buttonTemplateList}>
                <div className={styles.buttonConfigLabel}>Insert blocks</div>
                {draftTemplates.map((template, index) => {
                  const type = BUTTON_TEMPLATE_TYPES.some((option) => option.type === template.type)
                    ? template.type
                    : "paragraph";
                    ...
                    <div className={styles.buttonTemplateItem} key={`${index}-${type}`}>
                      <div className={styles.buttonTemplateTop}>
                        <NotionSelect
                          ...
                          ariaLabel="Template block type"
                          value={type}
                          options={BUTTON_TEMPLATE_TYPES.map((option) => ({
                            value: option.type,
                            label: option.label,
                            icon: <BlockIcon type={option.type} size={15} />,
                          }))}
                        ...
                        <div className={styles.buttonTemplateActions}>
                          <button
                            ...
                            aria-label="Move template block up"
                          >
                          ...
                          <button
                            ...
                            onClick={() => moveTemplate(index, 1)}
                            aria-label="Move template block down"
                          >
                      ...
                      <textarea
                        aria-label={`Template block ${index + 1} text`}
...
function BreadcrumbBlock({
  ...
  const nav = useRouter();
  const setSidebarOpen = useStore((s) => s.setSidebarOpen);
  const { workspace, pages } = useStore(
    useShallow((s) => {
      const out: Page[] = [];
      const seen = new Set<string>();
      let current: Page | undefined = s.pagesById[block.pageId];
      while (current && !seen.has(current.id)) {
        out.unshift(current);
        seen.add(current.id);
        current = current.parentId ? s.pagesById[current.parentId] : undefined;
      }
      return { workspace: s.workspace, pages: out };
    })
  );
  ...
  return (
    <BlockFrame block={block} ops={ops} depth={depth}>
      <nav className={styles.breadcrumbBlock} contentEditable={false}>
        ...
        {pages.length === 0 ? (
          <span className={styles.breadcrumbEmpty}>Untitled</span>
        ) : (
...
function InlineDatabaseBlock({
  ...
  const inlineDatabaseTitle = db ? pageDisplayTitle(db) : "New database";
  const meaningfulImportedSurfaceTitle = meaningfulInlineDatabaseTitle(importedSurfaceTitle);
  const meaningfulDbTitle = meaningfulInlineDatabaseTitle(inlineDatabaseTitle);
  ...
  const inlineTitleText =
    (shouldPreferResolvedLinkedDatabaseTitle ? meaningfulResolvedLinkedDatabaseTitle : undefined) ||
    meaningfulImportedSurfaceTitle ||
    meaningfulDbTitle ||
    meaningfulResolvedLinkedDatabaseTitle ||
    INLINE_DATABASE_PLACEHOLDER_TITLE;
  const inlineTitleIsPlaceholder =
    !meaningfulImportedSurfaceTitle &&
    !meaningfulDbTitle &&
    !meaningfulResolvedLinkedDatabaseTitle;
  const dbTitle = db ? inlineTitleText : "database";
  ...
  function openInlineDatabaseAddView(button: HTMLButtonElement) {
    ...
    const event = new CustomEvent("notionlike:open-inline-add-view", {
      cancelable: true,
    ...
    actionsOpen && actionsAnchor ? (
      <>
        <button
          ...
          tabIndex={-1}
          aria-label="Close database options"
        />
        <div
        ...
        <div className={styles.inlineDatabaseHeader}>
          ...
          {shouldRenderTitleInput ? (
            <input
              ...
              placeholder={INLINE_DATABASE_PLACEHOLDER_TITLE}
              aria-label={`${dbTitle} database title`}
...
function MentionMenu({
  ...
  const results = useMemo(() => {
    ...
    const personRank = Math.min(
      mentionSearchRank(currentUserLabel, "Person collaborator", q),
      "you".includes(q) ? 1 : Number.POSITIVE_INFINITY
    );
  ...
  const picker = (
    <>
      <button
        ...
        onClick={onClose}
        tabIndex={-1}
        aria-label={mode === "page_link" ? "Close page link menu" : "Close mention menu"}
      />
      <div
        ...
        role="listbox"
        tabIndex={-1}
        aria-label={mode === "page_link" ? "Link to page" : "Mention"}
...
function DatabaseSourcePicker({
  ...
  const q = query.trim().toLowerCase();
  const placementLabel = type === "inline_database" ? "inline" : "full-page";
  ...
  const picker = (
    <>
      <button
        ...
        onClick={onClose}
        tabIndex={-1}
        aria-label="Close database source picker"
      />
      <div
        className={styles.databaseSourceMenu}
        style={menuStyle}
        role="dialog"
        aria-label="Choose database source"
        contentEditable={false}
      ...
      >
        ...
        >
          ...
          <span className={styles.databaseSourceText}>
            <span className={styles.databaseSourceTitle}>New database</span>
            <span className={styles.databaseSourcePath}>Create a new {placementLabel} database</span>
          </span>
        </button>
        <div className={styles.databaseSourceLabel}>Existing data sources</div>
        <input
          ref={inputRef}
...
function TextBlock({
  ...
  const placeholder =
    pagePlaceholder && block.type === "paragraph"
      ? pagePlaceholderText ?? "Type '/' for commands"
      : def.placeholder ?? "";
  ...
  function openPageEditorForMention(pageMention: HTMLElement) {
    ...
    const anchor = inlineMenuAnchorFromRect(rect);
    const title = page ? pageTitle(page) : pageMention.textContent?.trim() || "Untitled";
    const path = page ? pagePathOrWorkspaceRoot(page, pagesById) : "Page";
    const { left, top } = anchoredMenuPosition(
      anchor,
  ...
  async function applyClipboardFiles(files: File[]) {
    ...
    for (const file of files) {
      ...
      const plainText = file.name || getDef(type).label;
      const content: BlockContent = {
        url: uploadedUrl,
  ...
  switch (block.type) {
      ...
      body = (
        <div className={styles.todo} data-checked={checked}>
          <input
            ...
            aria-label={checked ? "Mark to-do as incomplete" : "Mark to-do as complete"}
            onChange={() =>
              updateBlock(block.id, {
      ...
      body = (
        ...
        >
          <button
            ...
            aria-label={collapsed ? "Open toggle" : "Close toggle"}
    ...
    case "code": {
      ...
      const languageLabel = CODE_LANGUAGES.find((option) => option.value === language)?.label;
        ...
        >
          ...
          <div className={styles.codeToolbar}>
            <NotionSelect
              ...
              value={language}
              disabled={ops.readOnly}
              options={CODE_LANGUAGES.map((option) => ({
                value: option.value,
                label: option.label,
              }))}
              onChange={(next) =>
            ...
            <Suspense
              fallback={
                <div className={styles.mermaidPreview} contentEditable={false} aria-hidden="true">
                  <div className={styles.mermaidEmpty}>Diagram preview</div>
                </div>
              }
            >
      ...
      {pastedUrlMenu && (
        <TextFloatingMenuPortal>
          <div
            ...
            role="dialog"
            aria-label="Pasted link options"
            contentEditable={false}
            onMouseDown={(e) => e.preventDefault()}
            onKeyDown={onPastedUrlMenuKeyDown}
          >
            <div className={styles.pastedUrlTitle}>Paste as</div>
            {pastedUrlPage && (
              <>
            ...
            {dateCalendar && (
              <div className={styles.inlineDateCalendar}>
                <div className={styles.inlineDateMonth}>
                  <button type="button" aria-label="Previous month" onClick={() => shiftDateCalendar(-1)}>
                    <ChevronLeft size={14} aria-hidden="true" />
                  </button>
                  <span>{dateCalendar.label}</span>
                  <button type="button" aria-label="Next month" onClick={() => shiftDateCalendar(1)}>
          ...
          >
            <div className={styles.inlinePersonTop}>
              ...
              <span className={styles.inlinePersonInfo}>
                ...
                <span className={styles.inlinePersonMeta}>Workspace member</span>
              </span>
            </div>
            <button type="button" className={styles.inlinePersonAction} onClick={() => void copyMentionedPerson()}>
              <Copy size={14} aria-hidden="true" />
              {personEditor.copied ? "Copied" : "Copy name"}
            </button>
          ...
          >
            ...
            <div className={styles.inlinePageActions}>
              ...
              <button type="button" onClick={() => void copyMentionedPageLink()}>
                <Copy size={14} aria-hidden="true" />
                {pageEditor.copied ? "Copied" : "Copy link"}
              </button>
            </div>
          </div>
        </TextFloatingMenuPortal>
