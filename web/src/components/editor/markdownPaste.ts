...
export function parseInternalPastedBlocks(data: DataTransfer): PastedBlock[] {
  const raw = data.getData(NOTIONLIKE_BLOCKS_MIME);
  if (!raw) return [];
  try {
...
function parseWikiPageLink(text: string) {
  ...
  const label = unescapeMarkdownLiteral(text.slice(2, labelEnd)).trim() || "Link to page";
...
function dateFromMentionHref(raw: string) {
  ...
  if (!/^notionlike:\/\/date\//i.test(value)) return "";
  const date = value.replace(/^notionlike:\/\/date\//i, "").split(/[?#]/, 1)[0];
  try {
...
function personIdFromMentionHref(raw: string) {
  const value = raw.trim();
  if (!/^notionlike:\/\/person\//i.test(value)) return "";
  const userId = value.replace(/^notionlike:\/\/person\//i, "").split(/[?#]/, 1)[0];
  try {
...
function parseNotionlikeTabLabel(node: HTMLElement): PastedBlock | null {
  const titleNode = node.querySelector<HTMLElement>(":scope > [data-notionlike-tab-title]");
  const panelNode = node.querySelector<HTMLElement>(":scope > [data-notionlike-tab-panel]");
  const richText = titleNode
    ? richFromHtmlNode(titleNode, "[data-notionlike-tab-icon-text]")
    : richFromHtmlNode(node, "[data-notionlike-tab-panel],[data-notionlike-tab-icon-text]");
  const normalizedRich = richText.length > 0 ? richText : rich("Untitled");
  const icon = (node.getAttribute("data-notionlike-tab-icon") ?? "").trim();
...
function parseNotionlikeTab(node: HTMLElement): PastedBlock | null {
  if (node.getAttribute("data-notionlike-block-type") !== "tab") return null;
  const children = Array.from(node.children)
    .filter((child): child is HTMLElement => child instanceof HTMLElement)
    .filter((child) => child.matches("[data-notionlike-tab-label]"))
    .map(parseNotionlikeTabLabel)
    .filter((child): child is PastedBlock => !!child);
  return {
    type: "tab",
    content: { rich: [] },
...
function parseHtmlNode(node: Node): PastedBlock[] {
  ...
  if (node.matches("[data-notionlike-copy]")) return parseHtmlChildren(node);
  const notionlikeTab = parseNotionlikeTab(node);
  if (notionlikeTab) return [notionlikeTab];
  if (node.matches("[data-notionlike-tab-label]")) {
    const notionlikeTabLabel = parseNotionlikeTabLabel(node);
    return notionlikeTabLabel ? [notionlikeTabLabel] : [];
  }
...
export function parsePastedHtml(html: string): PastedBlock[] {
  ...
  try {
    ...
    const root = doc.body.querySelector("[data-notionlike-copy]") ?? doc.body;
...
export function parsePastedMarkdown(text: string): PastedBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (stripped === "---" || stripped === "***") {
      ...
      push(depth, block("table_of_contents", "Table of contents", { rich: [] }));
    } else if (/^\[breadcrumb\]$/i.test(stripped)) {
      push(depth, block("breadcrumb", "Breadcrumb", { rich: [] }));
    } else if (/^\[synced block\]$/i.test(stripped)) {
      push(depth, block("synced_block", "Synced block", { rich: [] }));
    } else if (/^\[tabs\]$/i.test(stripped)) {
      push(depth, block("tab", "Tabs", { rich: [] }));
    } else if (tabLabel) {
      push(
        depth,
