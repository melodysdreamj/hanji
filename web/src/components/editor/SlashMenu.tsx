...
const RECENT_KEY = "notionlike:slash-menu:recent";
const MENU_WIDTH = 320;
const MENU_MAX_HEIGHT = 400;
const MENU_GAP = 6;
const MENU_MARGIN = 8;
const SLASH_MENU_LABELS = {
  en: {
    blockCommands: "Block commands",
    closeBlockCommands: "Close block commands",
    groups: {
      Basic: "Basic blocks",
      Media: "Media",
      Database: "Database",
      Advanced: "Advanced blocks",
    } as Record<BlockDef["group"], string>,
    itemStatus: (label: string, position: number, total: number) =>
      `${label}, ${position} of ${total}. Press Enter to insert.`,
    listStatus: (total: number) => `${total} block commands available.`,
    noBlocks: "No blocks found",
    noBlocksFor: (query: string) => `No blocks found for "${query}"`,
    recentlyUsed: "Recently used",
  },
  ko: {
    blockCommands: "블록 명령",
    closeBlockCommands: "블록 명령 닫기",
    groups: {
      Basic: "기본 블록",
      Media: "미디어",
      Database: "데이터베이스",
    ...
    itemStatus: (label: string, position: number, total: number) =>
      `${label}, ${total}개 중 ${position}번째. Enter 키를 눌러 삽입하세요.`,
    listStatus: (total: number) => `블록 명령 ${total}개를 사용할 수 있어요.`,
    noBlocks: "블록을 찾을 수 없어요",
    noBlocksFor: (query: string) => `"${query}"에 해당하는 블록을 찾을 수 없어요`,
    recentlyUsed: "최근 사용한 블록",
  },
} as const;

function slashMenuLabels() {
  return pickLabels(SLASH_MENU_LABELS);
}
...
function groupedSections(items: BlockDef[]) {
  const groupLabels = slashMenuLabels().groups;
  const sections: { label: string; items: BlockDef[] }[] = [];
  for (const def of items) {
    const label = groupLabels[def.group];
...
function MenuBackdrop({ onClose }: { onClose: () => void }) {
  return (
    <button
      ...
      onClick={onClose}
      tabIndex={-1}
      aria-label={slashMenuLabels().closeBlockCommands}
    />
  );
}
...
export function SlashMenu({
  ...
  const sections = useMemo(() => {
    ...
    if (recent.length) {
      ...
      next.push({ label: labels.recentlyUsed, items: recent });
    }

    for (const group of GROUP_ORDER) {
      const items = results.filter((def) => def.group === group && !picked.has(defKey(def)));
      if (items.length) next.push({ label: labels.groups[group], items });
    }
    return next;
  }, [recentKeys, results, trimmedQuery]);
  ...
  const fixedStyle = useMemo<CSSProperties | undefined>(() => {
    ...
    const menuHeight = Math.min(desiredHeight, Math.max(minUsefulHeight, room));
    const top = openAbove
  ...
  const menuLabels = slashMenuLabels();
    ...
    const menu = (
      <>
        ...
        <div
          ...
          role="listbox"
          tabIndex={-1}
          aria-label={menuLabels.blockCommands}
          aria-describedby={emptyId}
          onMouseDown={(e) => e.preventDefault()}
        >
          <div id={emptyId} className={styles.slashEmpty} role="status">
            {trimmedQuery ? menuLabels.noBlocksFor(trimmedQuery) : menuLabels.noBlocks}
          </div>
        </div>
      </>
    );
    ...
    <>
      ...
      >
        <div id={statusId} className={styles.srOnly} aria-live="polite">
          {activeDef
            ? menuLabels.itemStatus(blockDefLabel(activeDef), active + 1, visibleItems.length)
            : menuLabels.listStatus(visibleItems.length)}
        </div>
