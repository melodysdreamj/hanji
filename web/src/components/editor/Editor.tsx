...
// locale switch (EN default).
const EDITOR_LABELS = {
  en: {
    changedBlockType: "Changed block type",
    changedColor: "Changed color",
    clearedColor: "Cleared color",
    copiedToClipboard: "Copied to clipboard",
    couldntCopy: "Couldn't copy",
    couldntCreateDatabase: "Couldn't create database.",
    couldntImportMarkdown: "Couldn't import Markdown.",
    databaseCreated: (viewLabel: string) => `${viewLabel} database created.`,
    ...
    importedBlocks: (count: number) =>
      `Imported ${count} block${count === 1 ? "" : "s"}.`,
    moveBlocksTo: (count: number) =>
      count === 1 ? "Move block to" : `Move ${count} blocks to`,
    noMarkdownBlocks: "No Markdown blocks found.",
    nothingToUndo: "Nothing to undo",
    restoredBlocks: "Restored blocks",
    undidDuplicate: "Undid duplicate",
    undo: "Undo",
  },
  ko: {
    changedBlockType: "블록 유형을 바꿨어요",
    changedColor: "색을 바꿨어요",
    clearedColor: "색을 제거했어요",
    copiedToClipboard: "클립보드에 복사했어요",
    couldntCopy: "복사하지 못했어요",
    couldntCreateDatabase: "데이터베이스를 만들지 못했어요.",
    couldntImportMarkdown: "Markdown을 가져오지 못했어요.",
    databaseCreated: (viewLabel: string) => `${viewLabel} 데이터베이스를 만들었어요.`,
    deletedBlocks: (count: number) =>
      count === 1 ? "블록을 삭제했어요" : `블록 ${count}개를 삭제했어요`,
    duplicatedBlocks: (count: number) =>
      count === 1 ? "블록을 복제했어요" : `블록 ${count}개를 복제했어요`,
    importedBlocks: (count: number) => `블록 ${count}개를 가져왔어요.`,
    moveBlocksTo: (count: number) =>
      count === 1 ? "블록 이동" : `블록 ${count}개 이동`,
    noMarkdownBlocks: "Markdown 블록을 찾지 못했어요.",
    nothingToUndo: "실행 취소할 항목이 없어요",
    restoredBlocks: "블록을 복원했어요",
...
function editorLabels() {
  return pickLabels(EDITOR_LABELS);
}
const EDITOR_SELECTION_REQUEST = "notionlike:editor-selection-request";
...
const STARTER_DATABASE_VIEWS: { type: StarterDatabaseView; label: string }[] = [
...
const BLANK_DATABASE_PROPERTIES: NonNullable<CreateDatabaseInput["properties"]> = [
  { name: "Name", type: "title", position: 1 },
];

function starterDatabaseLabel(type: StarterDatabaseView) {
  return STARTER_DATABASE_VIEWS.find((view) => view.type === type)?.label ?? "Database";
}

const STARTER_DATABASE_VIEW_KO_LABELS: Record<StarterDatabaseView, string> = {
  table: "표",
  board: "보드",
  list: "목록",
  timeline: "타임라인",
...
function starterDatabaseToastLabel(type: StarterDatabaseView) {
  return pickLabels({
    en: starterDatabaseLabel(type),
    ko: STARTER_DATABASE_VIEW_KO_LABELS[type] ?? starterDatabaseLabel(type),
  });
}
...
function databaseTitleFromStarterPage(page?: Page, block?: Block | null) {
  ...
  return pageTitle && pageTitle !== "Untitled" ? pageTitle : DEFAULT_DATABASE_TITLE;
}
...
function PageStarter({
  ...
  async function importMarkdownFile(file?: File) {
    ...
    try {
      ...
      if (count > 0) {
        ...
        notify(editorLabels().importedBlocks(count), "success");
      } else {
        notify(editorLabels().noMarkdownBlocks, "default");
      }
    } catch (error) {
      notify(error instanceof Error ? error.message : editorLabels().couldntImportMarkdown, "error");
    } finally {
      setImporting(false);
    }
  }
  ...
  async function createDatabaseView(type: StarterDatabaseView) {
    ...
    try {
      ...
      const db = await st.createDatabase({
        ...
        viewType: type,
        seedRows: false,
        properties: BLANK_DATABASE_PROPERTIES,
      });

      router.push(pageHref(db.id));
      if (replaceCurrentPage) {
        void useStore.getState().deletePage(page.id);
      } else {
      ...
      notify(editorLabels().databaseCreated(starterDatabaseToastLabel(type)), "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : editorLabels().couldntCreateDatabase, "error");
    } finally {
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Page starter"
      onKeyDown={onStarterKeyDown}
    >
      <div className={styles.pageStarterLabel}>Get started</div>
      <div className={styles.pageStarterPrimary} role="group" aria-label="Page starter actions">
        <button
          type="button"
        ...
        >
          ...
          <span className={styles.pageStarterText}>
            {creatingView === "table" ? "Creating..." : "Database"}
          </span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span className={styles.pageStarterText}>Templates</span>
        </button>
        <button
          type="button"
          className={styles.pageStarterButton}
        ...
        >
          ...
          <span className={styles.pageStarterText}>{importing ? "Importing..." : "Import"}</span>
        </button>
        <button
          type="button"
          className={styles.pageStarterButton}
        ...
        >
          ...
          <span className={styles.pageStarterText}>More</span>
        </button>
      </div>
      {moreOpen && (
        <div
          ...
          role="group"
          aria-label="More starter options"
        >
          {STARTER_DATABASE_VIEWS.filter((view) => view.type !== "table").map((view) => (
            ...
            >
              ...
              <span className={styles.pageStarterText}>
                {creatingView === view.type ? "Creating..." : view.label}
              </span>
            </button>
          ))}
        </div>
...
function EditorContentLoadingFallback() {
  return (
    <div className={styles.editorContentLoading} aria-busy="true" aria-label="Loading page body">
...
export function Editor({
  ...
  function blockControlFocusSelector(id: string, type: BlockType) {
    ...
    if (type === "image") {
      ...
      return captionVisible ? '[role="textbox"][aria-label="Image caption"]' : null;
    }
    if (type === "video") {
      if (!block?.content?.url) return 'input[aria-label="Video link"]';
      return captionVisible ? '[role="textbox"][aria-label="Video caption"]' : null;
    }
    if (type === "audio") {
      if (!block?.content?.url) return 'input[aria-label="Audio link"]';
      return captionVisible ? '[role="textbox"][aria-label="Audio caption"]' : null;
    }
    if (type === "bookmark") return block?.content?.url ? null : 'input[aria-label="Bookmark link"]';
    if (type === "embed") {
      if (!block?.content?.url) return 'input[aria-label="Embed link"]';
      return captionVisible ? '[role="textbox"][aria-label="Embed caption"]' : null;
    }
    if (type === "file") {
      if (!block?.content?.url) return 'input[aria-label="File link"]';
      return captionVisible ? '[role="textbox"][aria-label="File caption"]' : null;
    }
    return null;
  }
  ...
  const opsImpl: EditorOps = {
        ...
        if (id && cur) {
          st.updateBlock(
            id,
            {
              type: "inline_database",
              content: { rich: [] },
              plainText: INLINE_DATABASE_PLACEHOLDER_TITLE,
            },
            { history: false }
          );
          setSelectedBlockId(null);
    ...
    async createInlineDatabase(id, viewType = "table") {
      ...
      const title = databaseTitleFromBlock(cur, INLINE_DATABASE_PLACEHOLDER_TITLE);
      st.captureBlockHistory(pageId);
      const db = await st.createDatabase({
        parentId: pageId,
        parentType: "page",
...
function BlockSelectionToolbar({ ops }: { ops: EditorOps }) {
  ...
  async function duplicateSelection() {
    ...
    if (copies.length > 0) {
      notify(labels.duplicatedBlocks(copies.length), "success", {
        label: labels.undo,
        onClick: async () => {
          const restored = await undoBlockChange(ops.pageId);
          notify(restored ? labels.undidDuplicate : labels.nothingToUndo, restored ? "success" : "default");
        },
      });
    }
  }
  ...
  function deleteSelection() {
    const labels = editorLabels();
    ops.deleteSelectedBlocks();
    notify(labels.deletedBlocks(count), "success", {
      label: labels.undo,
      onClick: async () => {
        const restored = await undoBlockChange(ops.pageId);
        notify(restored ? labels.restoredBlocks : labels.nothingToUndo, restored ? "success" : "default");
      },
    });
  }
  ...
  function turnSelection(type: BlockType) {
    ...
    setMenuState(null);
    notify(editorLabels().changedBlockType, "success");
  }
  ...
  function colorSelection(token: string) {
    ...
    setMenuState(null);
    if (changed) notify(token === "default" ? labels.clearedColor : labels.changedColor, "success");
  }
  ...
  function quoteSelectedBlocks() {
    ...
    return quote ? `${quote}${suffix}` : `${count} selected blocks`;
  }
    ...
    <>
      {moveDialog && !ops.templateMode && (
        <BlockMoveToDialog
          ...
          title={editorLabels().moveBlocksTo(count)}
          onChooseDestination={async (destinationPageId) => {
      ...
      {menu === "turn" && (
        ...
        >
          <div className={styles.tbColorLabel}>Turn into</div>
      ...
      {menu === "color" && (
        <div
          ...
          role="menu"
          tabIndex={-1}
          aria-label="Color selected blocks"
          onMouseDown={(e) => e.preventDefault()}
          onKeyDown={onMenuKeyDown}
        >
          <div className={styles.tbColorLabel}>Text color</div>
          ...
          <div className={styles.tbColorLabel}>Background color</div>
      ...
      >
        <span className={styles.blockSelectionCount}>{count} selected</span>
        <button
          type="button"
        ...
        <button
          ...
          aria-label="Bold selected blocks"
        ...
        <button
          ...
          aria-label="Italic selected blocks"
          disabled={!hasMarkableSelectedText}
        ...
        <button
          ...
          aria-label="Underline selected blocks"
          disabled={!hasMarkableSelectedText}
        ...
        <button
          ...
          aria-label="Strikethrough selected blocks"
          disabled={!hasMarkableSelectedText}
        ...
        <button
          type="button"
          className={styles.tbBtn}
          aria-label="Code selected blocks"
          disabled={!hasMarkableSelectedText}
