...
const TURN_OPTIONS: { type: BlockType; label: string; glyph: string }[] = [
  ...
  { type: "heading_1", label: "Heading 1", glyph: "H1" },
  { type: "heading_2", label: "Heading 2", glyph: "H2" },
  { type: "heading_3", label: "Heading 3", glyph: "H3" },
  { type: "heading_4", label: "Heading 4", glyph: "H4" },
  { type: "to_do", label: "To-do list", glyph: "☑" },
  { type: "bulleted_list_item", label: "Bulleted list", glyph: "•" },
  { type: "numbered_list_item", label: "Numbered list", glyph: "1." },
  { type: "toggle", label: "Toggle list", glyph: "▸" },
...
export function SelectionToolbar({ commentOnly = false }: { commentOnly?: boolean } = {}) {
  ...
  const activeColorLabel =
    activeColor === "mixed"
      ? "Mixed"
      : [...TEXT_COLORS, ...BACKGROUND_COLORS].find((color) => color.token === activeColor)?.label ??
        "Default";
      ...
      {!commentOnly && (
        <>
          ...
          <button
            ...
            ref={colorButtonRef}
            className={styles.tbBtn}
            aria-label={`Text color: ${activeColorLabel}`}
            aria-haspopup="menu"
