    ...
    if (span.mention === "person" && span.userId?.trim()) {
      ...
      return (
        <span
          ...
          data-mention="person"
          title={i18next.t("commentsPanel:mention.title", { name: text.replace(/^@/, "") })}
        >
          {text}
        </span>
      );
...
function timeLabel(value?: string) {
  if (!value) return i18next.t("commentsPanel:card.justNow");
...
function commentMentionLabel(profile: OrganizationProfile) {
  return (
    ...
    profile.email?.trim() ||
    profile.userId?.trim() ||
    i18next.t("commentsPanel:mention.person")
  );
}
...
function commentMentionDescription(profile: OrganizationProfile) {
  const parts = [
    ...
    profile.organizationRole
      ? i18next.t("commentsPanel:mention.roleInOrganization", { role: profile.organizationRole })
      : null,
    profile.status && profile.status !== "active" ? profile.status : null,
  ].filter(Boolean);
  return parts.join(" - ") || i18next.t("commentsPanel:mention.organizationPerson");
}
...
function commentMentionProfile(input: {
  ...
  return {
    userId,
    displayName:
      input.displayName?.trim() || personLabel(userId, userId) || i18next.t("commentsPanel:mention.you"),
...
export function CommentsPanel({
  ...
  const { t } = useTranslation(["commentsPanel", "common"]);
  ...
  useEffect(() => {
    restoreFocusRef.current = document.activeElement instanceof HTMLElement
      ...
      : null;
    const restoreIsolation = isolateBodyForModal([backdropRef.current, panelRef.current]);
    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    const frame = window.requestAnimationFrame(() => {
      if (initialQuote?.trim()) draftRef.current?.focus();
  ...
  function onPanelKeyDown(e: React.KeyboardEvent<HTMLElement>) {
    ...
    trapModalTab(e, panelRef.current);
  }
  ...
  const panel = (
    <>
      <button
        ...
        onClick={close}
        tabIndex={-1}
        aria-label={t("commentsPanel:closeAria")}
      />
      <aside
        ref={panelRef}
      ...
      >
        <div className={styles.header}>
          <div>
            ...
            <div className={styles.subtitle}>
              {blockId ? t("commentsPanel:subtitle.block") : t("commentsPanel:subtitle.page")}
            </div>
          </div>
          <button type="button" className={styles.close} onClick={close} aria-label={t("commentsPanel:closeAria")}>
            <X size={15} />
          </button>
        </div>

        <div className={styles.tabs} role="tablist" aria-label={t("commentsPanel:status.aria")}>
          <button
            type="button"
            role="tab"
          ...
          >
            <span>{t("commentsPanel:status.open")}</span>
            <span>{openCount}</span>
          </button>
          <button
            type="button"
          ...
          >
            <span>{t("commentsPanel:status.resolved")}</span>
        ...
        {blockId && (
          <div className={styles.anchorPreview}>
            <span>
              {pendingQuote
                ? t("commentsPanel:anchor.commentingOnSelectedText")
                : t("commentsPanel:anchor.commentingOn")}
            </span>
            <strong>{pendingQuote || targetPreview || t("commentsPanel:anchor.selectedBlock")}</strong>
          </div>
        )}
        ...
        <div className={styles.composer} data-comments-composer>
          <div className={styles.composerBox}>
            <textarea
              ref={draftRef}
              value={draft}
              aria-label={blockId ? t("commentsPanel:composer.addBlockComment") : t("commentsPanel:composer.addPageComment")}
              ...
              placeholder={blockId ? t("commentsPanel:composer.blockPlaceholder") : t("commentsPanel:composer.pagePlaceholder")}
              rows={3}
              onKeyDown={(e) => {
                if (isComposingKeyEvent(e)) return;
            ...
            />
            {draftMentionRange && (
              <div className={styles.mentionMenu} role="listbox" aria-label={t("commentsPanel:mention.commentPeopleAria")}>
                {draftMentionPeople.length > 0 ? draftMentionPeople.map((profile, index) => {
                  ...
                  <div className={styles.mentionEmpty}>{t("commentsPanel:mention.noMatches")}</div>
                )}
              </div>
            )}
            <div className={styles.composerFoot}>
              <span>{blockId ? t("commentsPanel:subtitle.block") : t("commentsPanel:subtitle.page")}</span>
              <div className={styles.composerActions}>
                <button
                  type="button"
                  className={styles.mentionTrigger}
                  aria-label={t("commentsPanel:mention.trigger")}
                  title={t("commentsPanel:mention.trigger")}
                ...
                <button
                  ...
                  onClick={() => void submit()}
                >
                  {t("commentsPanel:composer.submit")}
                </button>
              </div>
            </div>
          </div>
        ...
        <div
          ...
          ref={listRef}
          role="list"
          aria-label={t("commentsPanel:list.threadsAria")}
        >
          {topLevel.length === 0 && (
            <div className={styles.empty} role="status">{t("commentsPanel:list.empty")}</div>
          )}
          {topLevel.length > 0 && visibleThreads.length === 0 && (
            <div className={styles.empty} role="status">
              {commentView === "open" ? t("commentsPanel:list.noOpen") : t("commentsPanel:list.noResolved")}
            </div>
          )}
...
function CommentCard({
  ...
  return (
    <article
      ...
      aria-label={
        comment.resolved
          ? t("commentsPanel:card.commentAriaResolved", { author, time })
          : t("commentsPanel:card.commentAria", { author, time })
      }
    ...
    >
      ...
      <div className={styles.commentBody}>
        <div className={styles.commentMeta}>
          <span>{author}</span>
          <span>{time}</span>
          {comment.editedAt ? <span>{t("commentsPanel:card.edited")}</span> : null}
        </div>
        {comment.blockId && (
          <div className={styles.anchorLabel}>
            {quote
              ? t("commentsPanel:anchor.onSelectedText")
              : isTarget
                ? t("commentsPanel:anchor.onSelectedBlock")
                : t("commentsPanel:anchor.onABlock")}
          </div>
        )}
        ...
        {editing ? (
          ...
          <div className={styles.commentText}>{text ? renderCommentRichText(spans) : t("commentsPanel:card.emptyComment")}</div>
        )}
        <div className={styles.commentActions}>
          {canComment && <button type="button" onClick={onStartReply}>{t("commentsPanel:actions.reply")}</button>}
          {comment.blockId && (
            <button type="button" onClick={onRevealAnchor}>
              {t("commentsPanel:actions.showInPage")}
            </button>
          )}
          {canResolve && (
            <button
            ...
            >
              {comment.resolved ? t("commentsPanel:actions.reopen") : t("commentsPanel:actions.resolve")}
            </button>
          )}
          {canManageOwn && !editing && (
            <button type="button" onClick={startEdit} data-comment-action="edit">
              {t("commentsPanel:actions.edit")}
            </button>
          )}
          {canManageOwn && (
            <button
        ...
        {confirmingDelete && (
          <div className={styles.deleteConfirm} data-testid="comment-delete-confirm">
            <span>
              {replies.length > 0
                ? t("commentsPanel:delete.threadConfirm", { count: replies.length })
                : t("commentsPanel:delete.commentConfirm")}
            </span>
        ...
        {(replies.length > 0 || replying) && (
          <div className={styles.replies} role="list" aria-label={t("commentsPanel:card.repliesAria", { count: replies.length })}>
            {replies.map((reply) => (
              <ReplyCard
                key={reply.id}
                comment={reply}
              ...
              <div className={styles.replyComposer} role="listitem">
                <div className={styles.replyComposerBox}>
                  <textarea
                    value={replyDraft}
                    rows={2}
                    aria-label={t("commentsPanel:actions.reply")}
                    data-comment-reply-input="true"
                    placeholder={t("commentsPanel:reply.placeholder")}
                  ...
                  />
                  {replyMentionRange && (
                    <div className={styles.mentionMenu} role="listbox" aria-label={t("commentsPanel:mention.replyPeopleAria")}>
    ...
    <div className={styles.replyComposer} data-comment-edit-composer>
      <div className={styles.replyComposerBox}>
        <textarea
          value={draft}
          rows={3}
          aria-label={t("commentsPanel:edit.aria")}
...
function ReplyCard({
  ...
  return (
    <article className={styles.reply} role="listitem" aria-label={t("commentsPanel:card.replyAria", { author, time })}>
      ...
      <div className={styles.replyBody}>
        ...
        {editing ? (
          ...
          <div className={styles.commentText}>{text ? renderCommentRichText(spans) : t("commentsPanel:card.emptyReply")}</div>
        )}
        {canManageOwn && !editing && (
          <div className={styles.commentActions}>
            <button
        ...
        {confirmingDelete && (
          <div className={styles.deleteConfirm} data-testid="comment-delete-confirm">
            <span>{t("commentsPanel:delete.commentConfirm")}</span>
            <div className={styles.deleteConfirmActions}>
              <button
                type="button"
                className={styles.secondaryButton}
