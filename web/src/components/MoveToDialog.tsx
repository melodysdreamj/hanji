...
export function MoveToDialog({
  ...
  const { t } = useTranslation("moveToDialog");
  ...
  const destinations = useMemo<Destination[]>(() => {
    ...
    const rootLabel = t("moveToDialog:workspaceRoot.label");
    const rootPath = t("moveToDialog:workspaceRoot.path");
    const root: Destination = {
      type: "workspace",
      id: null,
      label: rootLabel,
      path: rootPath,
      score: scoreDestination(rootLabel, rootPath, q, 0),
      disabled: sourceParentLocked,
      disabledReason: sourceParentLocked ? t("moveToDialog:locked.parent") : undefined,
    };
    const pageDestinations: Destination[] = Object.values(pagesById)
      ...
      .map((page, index) => {
        ...
        return {
          ...
          disabledReason: sourceParentLocked ? t("moveToDialog:locked.parent") : page.isLocked ? t("moveToDialog:locked.page") : undefined,
        };
      });
    return [root, ...pageDestinations]
      ...
      .slice(0, 14);
  }, [movingPage, pageId, pagesById, query, sourceParentLocked, t]);
  ...
  const choose = useCallback(async (destination: Destination) => {
    if (sourceParentLocked) {
      notify(t("moveToDialog:toast.lockedParent"), "default");
      return;
    }
    if (destination.type === "page" && destination.page.isLocked) {
      notify(t("moveToDialog:toast.locked"), "default");
      return;
    }
    ...
    try {
      ...
      notify(t("moveToDialog:toast.moved"), "success");
    } catch {
      notify(t("moveToDialog:toast.moveFailed"), "error");
      return;
    }
    if (onMoved) {
      close(false);
  ...
  }, [close, movePage, notify, onMoved, pageId, pagesById, sourceParentLocked, t]);
  ...
  }, [close, createPage, movePage, notify, onMoved, pageId, pagesById, query, setTreePageExpanded, sourceParentLocked, t]);
  ...
  return (
    <div className={styles.overlay}>
      <button
        ...
        onClick={() => close()}
        tabIndex={-1}
        aria-label={t("moveToDialog:dialog.closeLabel")}
      />
      <div
        ref={dialogRef}
      ...
      >
        <div id={titleId} className={styles.title}>{t("moveToDialog:title")}</div>
        <div className={styles.searchRow}>
          <Search size={17} aria-hidden="true" />
          <input
            ref={inputRef}
            value={query}
            aria-label={t("moveToDialog:search.label")}
            role="combobox"
            aria-expanded="true"
            aria-controls={resultsId}
            aria-activedescendant={activeId}
            aria-autocomplete="list"
            placeholder={t("moveToDialog:search.placeholder")}
            onChange={(e) => {
        ...
        <div
          ...
          ref={resultsRef}
          role="listbox"
          aria-label={t("moveToDialog:results.listLabel")}
        >
          {destinations.map((destination, index) => (
            <button
              ...
              aria-label={t("moveToDialog:results.destinationLabel", { label: destination.label, path: destination.path })}
          ...
          {showCreate && (
            <button
              ...
              aria-label={t("moveToDialog:create.optionLabel", { title: query.trim() })}
            ...
            >
              ...
              <span className={styles.text}>
                <span>{t("moveToDialog:create.label", { title: query.trim() })}</span>
                <span>{sourceParentLocked ? t("moveToDialog:locked.parent") : t("moveToDialog:create.hint")}</span>
              </span>
              {sourceParentLocked && <LockIcon className={styles.lock} size={13} aria-hidden="true" />}
            </button>
          )}
          {destinations.length === 0 && !showCreate && (
            <div className={styles.empty} role="status">{t("moveToDialog:results.empty")}</div>
          )}
        </div>
      </div>
    </div>
