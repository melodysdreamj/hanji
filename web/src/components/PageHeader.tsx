...
const PAGE_HEADER_LABELS = {
  en: {
    ...
    addCover: "Add cover",
    addIcon: "Add icon",
    addPlaceholderComment: "Add a comment...",
    addPageCover: "Add page cover",
    addPageComment: "Add page comment",
    addVerification: "Add verification",
    changeIcon: "Change page icon",
    commentCount: (count: number) => `Comments ${count}`,
    emptyComment: "(empty comment)",
    justNow: "Just now",
    moreComments: (count: number) => `${count} more comment${count === 1 ? "" : "s"}`,
    openAll: "Open all",
    openComments: (count: number) => `Open ${count} unresolved page comment${count === 1 ? "" : "s"}`,
  ...
  ko: {
    ...
    addCover: "커버 추가",
    addIcon: "아이콘 추가",
    addPlaceholderComment: "댓글 추가...",
    addPageCover: "페이지 커버 추가",
    addPageComment: "페이지 댓글 추가",
    addVerification: "인증 추가",
    changeIcon: "페이지 아이콘 변경",
    commentCount: (count: number) => `댓글 ${count}`,
    emptyComment: "(빈 댓글)",
    justNow: "방금 전",
    moreComments: (count: number) => `댓글 ${count}개 더 보기`,
    openAll: "모두 열기",
    openComments: (count: number) => `미해결 페이지 댓글 ${count}개 열기`,
    pageComments: "페이지 댓글",
    pageOptions: "페이지 옵션",
    pageTitle: "페이지 제목",
...
function pageHeaderLabels() {
  return pickLabels(PAGE_HEADER_LABELS);
}
...
function pageCommentTime(value?: string) {
  if (!value) return pageHeaderLabels().justNow;
...
export function PageHeader({
  ...
  function toggleVerification() {
    ...
    notify(pageVerified ? "Verification removed" : "Page verified", "success");
  }
  ...
  return (
    ...
    >
        ...
        >
          ...
          {!readOnly && !page.cover && (
            <button
              type="button"
              className={styles.ctrlBtn}
              aria-label={labels.addPageCover}
              data-page-header-control
              onClick={addCover}
            >
              <ImageIcon size={15} aria-hidden="true" />
              <span>{labels.addCover}</span>
            </button>
          )}
          {showHeaderVerificationControl && (
            <button
              type="button"
              className={styles.ctrlBtn}
              aria-label={pageVerified ? labels.removeVerification : labels.verifyPage}
              data-page-header-control
              onClick={toggleVerification}
            >
              <CheckIcon size={15} aria-hidden="true" />
              <span>{pageVerified ? labels.removeVerification : labels.addVerification}</span>
            </button>
          )}
          {showHeaderCommentControl && (
            <button
              ...
              aria-label={
                pageCommentCount
                  ? labels.openComments(pageCommentCount)
                  : labels.addPageComment
              }
              data-page-header-control
              onClick={() => openComments(pageId)}
            >
              <CommentIcon size={15} aria-hidden="true" />
              <span>{pageCommentCount ? labels.commentCount(pageCommentCount) : labels.addComment}</span>
            </button>
          )}
        </div>
      )}
      ...
      <div
        ...
        aria-placeholder={labels.untitled}
        suppressContentEditableWarning
        data-placeholder={labels.untitled}
            ...
            <div className={styles.pageCommentList}>
              ...
              {pageComments.length > 3 && (
                <button type="button" className={styles.pageCommentMore} onClick={() => openComments(pageId)}>
                  {labels.moreComments(pageComments.length - 3)}
                </button>
              )}
            </div>
          )}
