...
const PAGE_FONTS: { value: PageFont; labelKey: string; sample: string }[] = [
  { value: "default", labelKey: "fonts.default", sample: "Ag" },
  { value: "serif", labelKey: "fonts.serif", sample: "Ag" },
  { value: "mono", labelKey: "fonts.mono", sample: "Ag" },
];
const BACKLINKS_DISPLAY_OPTIONS: { value: BacklinksDisplay; labelKey: string }[] = [
  { value: "default", labelKey: "displayOptions.default" },
  { value: "expanded", labelKey: "displayOptions.expanded" },
  { value: "off", labelKey: "displayOptions.off" },
];
const PAGE_COMMENTS_DISPLAY_OPTIONS: { value: PageCommentsDisplay; labelKey: string }[] = [
  { value: "default", labelKey: "displayOptions.default" },
  { value: "expanded", labelKey: "displayOptions.expanded" },
  { value: "off", labelKey: "displayOptions.off" },
];
...
export function RowMenu({
  ...
  const { t } = useTranslation(["rowMenu", "common"]);
  ...
  const visibleDatabaseRowTitle = rawTitle;
  const visibleMenuTitle = isDatabaseRowMenu ? visibleDatabaseRowTitle : title;
  const menuAriaTitle = visibleMenuTitle || (isDatabaseRowMenu ? t("rowMenu:untitledDatabaseRow") : title);
  const normalizedActionQuery = actionQuery.trim().toLowerCase();
  const matchesAction = (...labels: string[]) =>
    !normalizedActionQuery ||
    labels.some((label) => label.toLowerCase().includes(normalizedActionQuery));
  ...
  const customizeSummaryLabel =
    backlinksDisplay === "default" && pageCommentsDisplay === "default"
      ? t("rowMenu:customize.summaryDefault")
      : t("rowMenu:customize.summaryCustomized");
  ...
  async function onImportFile(file?: File) {
    ...
    try {
      ...
      notify(
        count > 0 ? t("rowMenu:toast.importedBlocks", { count }) : t("rowMenu:toast.nothingToImport"),
        count > 0 ? "success" : "default"
      );
      close();
    } catch {
      notify(t("rowMenu:toast.couldntImportMarkdown"), "error");
    } finally {
      setImporting(false);
    }
  }
  ...
  async function copyPageLink() {
    ...
    notify(ok ? t("rowMenu:toast.copiedLink") : t("rowMenu:toast.couldntCopyLink"), ok ? "success" : "error");
    close();
  }
  ...
  async function exportMarkdown() {
    try {
      ...
      notify(t("rowMenu:toast.exportedMarkdown"), "success");
    } catch {
      notify(t("rowMenu:toast.couldntExportMarkdown"), "error");
    } finally {
      close();
    }
  }
  ...
  async function exportNative() {
    try {
      ...
      notify(
        warnings.length
          ? t("rowMenu:toast.exportedHanjiFileWithWarnings")
          : t("rowMenu:toast.exportedHanjiFile"),
        "success"
      );
    } catch {
      notify(t("rowMenu:toast.couldntExportHanjiFile"), "error");
    } finally {
      close();
    }
  }
  ...
  async function toggleFavoriteStatus() {
    ...
    try {
      ...
      notify(wasFavorite ? t("rowMenu:toast.removedFromFavorites") : t("rowMenu:toast.addedToFavorites"), "success");
    } catch {
      notify(t("rowMenu:toast.couldntUpdateFavorites"), "error");
    } finally {
      close();
    }
  }
  ...
  async function duplicateCurrentPage() {
    ...
    try {
      ...
      notify(t("rowMenu:toast.duplicatedPage"), "success");
  ...
  function toggleLockStatus() {
    ...
    notify(nextLocked ? t("rowMenu:toast.pageLocked") : t("rowMenu:toast.pageUnlocked"), "success");
    close();
  }
  ...
  async function movePageToTrash() {
    try {
      ...
      notify(t("rowMenu:toast.movedToTrash"), "success", {
        label: t("rowMenu:actions.undo"),
        onClick: async () => {
          try {
            await restorePage(pageId);
            notify(t("rowMenu:toast.restoredPage"), "success");
            if (params?.pageId === pageId) router.push(pageHref(pageId));
          } catch {
            notify(t("rowMenu:toast.couldntRestorePage"), "error");
          }
        },
      });
      ...
      notify(t("rowMenu:toast.couldntMoveToTrash"), "error");
    } finally {
      close();
    }
  }
    ...
    return renderMenuSurface(
      <>
        <button
          ...
          tabIndex={-1}
          aria-label={isDatabaseRowMenu ? t("rowMenu:aria.closeDatabaseRowActions") : t("rowMenu:aria.closePageActions")}
          onClick={(e) => {
            e.stopPropagation();
            close();
          }}
        ...
        <div
          ...
          aria-label={
            isDatabaseRowMenu
              ? t("rowMenu:aria.databaseRowActionsFor", { title: menuAriaTitle })
              : t("rowMenu:aria.pageActionsFor", { title: menuAriaTitle })
          }
        ...
        >
          <input
            ...
            data-action-search
            type="search"
            aria-label={t("rowMenu:searchActions")}
            placeholder={t("rowMenu:searchActions")}
            value={actionQuery}
          ...
          />
          <div className={styles.sectionLabel}>{t("rowMenu:sections.page")}</div>
          ...
          {matchesAction("Add to Favorites", "Remove from Favorites", "즐겨찾기") && (
            ...
            >
              ...
              <span>{page.isFavorite ? t("rowMenu:actions.removeFromFavorites") : t("rowMenu:actions.addToFavorites")}</span>
            </button>
          )}
          {matchesAction("Edit icon", "아이콘") && (
            <>
              ...
              >
                ...
                <span>{t("rowMenu:actions.editIcon")}</span>
              </button>
              {iconPickerOpen && (
          ...
          {matchesAction("Edit properties", "속성") && isDatabaseRowMenu && (
            ...
            >
              ...
              <span>{t("rowMenu:actions.editProperties")}</span>
          ...
          {matchesAction("Open in", "다음에서 열기", "Side peek", "Center peek", "Full page", "Open in new tab", "새 탭") && (
            <>
              ...
              >
                ...
                <span>{t("rowMenu:actions.openIn")}</span>
                <ChevronRight
              ...
              {openInOpen && (
                <div className={styles.compactSubmenu} role="group" aria-label={t("rowMenu:aria.openIn")}>
                  ...
                  >
                    {t("rowMenu:actions.openInNewTab")}
                  </button>
                </div>
              )}
            </>
          ...
          {matchesAction("Comments", "댓글") && isDatabaseRowMenu && (
            ...
            >
              ...
              <span>{t("rowMenu:actions.comments")}</span>
            </button>
          )}
          {matchesAction("Copy link", "링크") && (
            ...
            >
              ...
              <span>{t("rowMenu:actions.copyLink")}</span>
          ...
          {matchesAction("Move to", "옮기기") && (
            ...
            >
              ...
              <span>{t("rowMenu:actions.moveTo")}</span>
          ...
          {matchesAction("Move to Trash", "휴지통", "Delete") && (
            ...
            >
              ...
              <span>{t("rowMenu:actions.moveToTrash")}</span>
              <span className={styles.itemHint}>Del</span>
            </button>
          )}
          {!hasVisibleCompactActions && (
            <div className={styles.menuMeta}>
              <span>{t("rowMenu:noMatchingActions")}</span>
            </div>
          )}
          <div className={styles.divider} role="separator" />
          <div className={styles.menuMeta}>
            <span>{t("rowMenu:meta.lastEdited")}</span>
      ...
      {!historyOpen && (
        <>
          <button
            ...
            className={styles.backdrop}
            tabIndex={-1}
            aria-label={t("rowMenu:aria.closePageActions")}
            onClick={(e) => {
              e.stopPropagation();
              close();
            }}
          ...
          <div
            ...
            role="menu"
            tabIndex={-1}
            aria-label={t("rowMenu:aria.pageActionsFor", { title })}
            onClick={(e) => e.stopPropagation()}
            onKeyDown={onMenuKeyDown}
          >
            <div className={styles.pageInfo}>
              ...
              <span className={styles.pageInfoText}>
                ...
                <span>
                  {pageVerified
                    ? t("rowMenu:meta.verifiedPrefix", { label: editedMenuLabel(page.updatedAt ?? page.createdAt) })
            ...
            >
              ...
              <span>
                {page.isFavorite ? t("rowMenu:actions.removeFromFavorites") : t("rowMenu:actions.addToFavorites")}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <OpenInNew size={16} aria-hidden="true" />
              <span>{t("rowMenu:actions.openInNewTab")}</span>
            </button>
            <div className={styles.divider} role="separator" />
            {onRename && (
              <button
              ...
              >
                ...
                <span>{t("rowMenu:actions.rename")}</span>
              </button>
            )}
            {onAddSubpage && (
              <button
              ...
              >
                ...
                <span>{t("rowMenu:actions.addPageInside")}</span>
              </button>
            )}
            <button
              type="button"
            ...
            >
              ...
              <span>{t("rowMenu:actions.exportAsMarkdown")}</span>
            ...
            >
              <Download size={16} aria-hidden="true" />
              <span>{t("rowMenu:actions.exportAsHanjiFile")}</span>
            ...
            >
              ...
              <span>{importing ? t("rowMenu:actions.importing") : t("rowMenu:actions.importMarkdown")}</span>
              <span className={styles.itemHint}>.md</span>
            </button>
            <div className={styles.divider} role="separator" />
            <button
            ...
            >
              ...
              <span>{page.isLocked ? t("rowMenu:actions.unlockPage") : t("rowMenu:actions.lockPage")}</span>
            </button>
            <button
              type="button"
              className={styles.item}
            ...
            >
              <Settings size={16} aria-hidden="true" />
              <span>{t("rowMenu:actions.customizePage")}</span>
            ...
            {customizeOpen && (
              <div className={styles.customizePanel} role="group" aria-label={t("rowMenu:aria.customizePage")}>
                <div className={styles.customizeRow}>
                  <span>{t("rowMenu:customize.backlinks")}</span>
                  <div className={styles.segmentedMenu} role="group" aria-label={t("rowMenu:aria.backlinksDisplay")}>
                ...
                <div className={styles.customizeRow}>
                  <span>{t("rowMenu:customize.pageComments")}</span>
                  <div className={styles.segmentedMenu} role="group" aria-label={t("rowMenu:aria.pageCommentsDisplay")}>
            ...
            <div className={styles.divider} role="separator" />
            <div className={styles.sectionLabel}>{t("rowMenu:sections.style")}</div>
            <div className={styles.fontPicker} role="group" aria-label={t("rowMenu:aria.pageFont")}>
              {PAGE_FONTS.map((font) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span>{t(`rowMenu:${font.labelKey}`)}</span>
                  </button>
                );
              })}
            </div>
            ...
            >
              <span>{t("rowMenu:style.smallText")}</span>
            ...
            >
              <span>{t("rowMenu:style.fullWidth")}</span>
            ...
            <div className={styles.divider} role="separator" />
            <div className={styles.menuMeta}>
              <span>{t("rowMenu:meta.created")}</span>
              <span>{menuTimestampLabel(page.createdAt)}</span>
            </div>
            <div className={styles.menuMeta}>
              <span>{t("rowMenu:meta.createdBy")}</span>
      ...
      <Suspense fallback={null}>
        {historyOpen && (
          <UpdatesPanel
            pageId={pageId}
            placement="topbar"
            title={t("rowMenu:actions.pageHistory")}
            onClose={() => {
