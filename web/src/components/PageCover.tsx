...
function coverUploadProgressLabel(progress: UploadProgress) {
  if (progress.phase === "preparing") return i18next.t("pageCover:progress.preparing");
  if (progress.phase === "finalizing") return i18next.t("pageCover:progress.finalizing");
  if (progress.phase === "complete") return i18next.t("pageCover:progress.complete");
  return i18next.t("pageCover:progress.uploading");
}
...
export function PageCover({
  ...
  const { t } = useTranslation(["pageCover", "common"]);
  ...
  function removeCover() {
    ...
    setMenuOpen(false);
    setMenuStyle(undefined);
    notify(t("pageCover:toast.removed"), "success", {
      label: t("pageCover:toast.undo"),
      onClick: () => {
        updatePage(pageId, { cover: previousCover, coverPosition: previousCoverPosition });
        notify(t("pageCover:toast.restored"), "success");
      },
    });
  ...
  async function pickFile(file?: File) {
    ...
    try {
      ...
      setUploadProgress({
        phase: "preparing",
        percent: 0,
        fileName: file.name || t("pageCover:upload.defaultFileName"),
      });
      ...
      setUploadProgress(null);
      setUploadError(t("pageCover:upload.error"));
      setTab("upload");
    }
  }
  ...
  function applyLink() {
    ...
    if (!isSafeCoverUrl(value)) {
      setLinkError(t("pageCover:link.invalid"));
      return;
    }
    setLinkError("");
    applyCover(value);
  ...
  return (
    <div
      ...
      aria-label={
        repositioning
          ? t("pageCover:aria.coverVerticalPosition")
          : !readOnly
            ? t("pageCover:aria.pageCover")
            : undefined
      }
      ...
      aria-valuetext={repositioning ? t("pageCover:aria.positionFromTop", { position }) : undefined}
      style={{
    ...
    >
      {menuOpen && (
        <button
          ...
          aria-label={t("pageCover:aria.closeCoverMenu")}
          tabIndex={-1}
      ...
      {!readOnly && !repositioning && (
        <div className={styles.actions}>
          <button
            ...
            aria-label={t("pageCover:aria.changePageCover")}
            onClick={() => {
              if (menuOpen) {
                closeCoverMenu(true);
                return;
          ...
          >
            {t("pageCover:actions.changeCover")}
          </button>
          {canReposition && (
            <button
              type="button"
              aria-label={t("pageCover:aria.repositionPageCover")}
              onClick={startReposition}
            >
              {t("pageCover:actions.reposition")}
            </button>
          )}
          <button
            type="button"
            className={styles.dangerAction}
            aria-label={t("pageCover:aria.removePageCover")}
            onClick={removeCover}
          >
            {t("pageCover:actions.remove")}
          </button>
          {menuOpen && (
            <div
              id={coverMenuId}
              ...
              style={menuStyle}
              role="dialog"
              aria-label={t("pageCover:aria.changeCoverDialog")}
              onKeyDown={onMenuKeyDown}
            >
              <div className={styles.tabs} role="tablist" aria-label={t("pageCover:aria.coverSource")}>
                <button
                ...
                >
                  {COVER_PRESETS.map((cover, index) => (
                    <button
                      ...
                      aria-label={t("pageCover:aria.selectCover", { number: index + 1 })}
              ...
              {tab === "upload" && (
                ...
                >
                  ...
                  <button
                    type="button"
                    data-cover-menu-item
                    aria-label={t("pageCover:aria.uploadFromDevice")}
                    disabled={!!uploadProgress}
                    aria-busy={!!uploadProgress}
                    onClick={() => fileInputRef.current?.click()}
                  >
                    {uploadProgress ? t("pageCover:actions.uploading") : t("pageCover:actions.uploadFile")}
                  </button>
                  {uploadProgress && (
                    <div className={styles.uploadProgress} role="status" aria-live="polite">
                      ...
                      <div
                        ...
                        aria-label={t("pageCover:aria.uploadingFile", { fileName: uploadProgress.fileName })}
                      >
              ...
              {tab === "link" && (
                ...
                >
                  <input
                    value={linkDraft}
                    aria-label={t("pageCover:aria.coverImageLink")}
                    ...
                    placeholder={t("pageCover:link.placeholder")}
                    onKeyDown={(e) => {
                      if (isComposingKeyEvent(e)) return;
      ...
      {repositioning && (
        <div className={styles.repositionBar}>
          <span>{t("pageCover:reposition.hint")}</span>
          ...
          <button type="button" data-reposition-action onClick={savePosition}>
            {t("pageCover:actions.savePosition")}
          </button>
        </div>
      )}
    </div>
