...
const FIND_MATCH_HIGHLIGHT = "hanji-page-find-match";
const FIND_ACTIVE_HIGHLIGHT = "hanji-page-find-active";
const FIND_STYLE_ID = "hanji-page-find-highlight-style";
...
export function PageFindBar({
  ...
  const { t } = useTranslation("pageFindBar");
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={t("pageFindBar:dialog.label")}
    ...
    >
      <div className={styles.findInputWrap}>
        ...
        <input
          ref={inputRef}
          value={query}
          aria-label={t("pageFindBar:input.label")}
          aria-keyshortcuts="Enter Shift+Enter Meta+G Control+G F3"
            placeholder={t("pageFindBar:input.placeholder")}
      ...
      <button
        ...
        aria-label={t("pageFindBar:actions.previous")}
      >
        <ArrowUp size={14} />
      </button>
      <button
        ...
        onClick={() => step(1)}
        disabled={matches.length === 0}
        aria-label={t("pageFindBar:actions.next")}
      >
        <ArrowDown size={14} />
      </button>
      <button type="button" className={styles.findButton} onClick={onClose} aria-label={t("pageFindBar:actions.close")}>
        <X size={14} />
      </button>
    </div>
  );
