...
const PAGE_TREE_LABELS = {
  en: {
    ...
    lockedName: (title: string) => `${title} (locked)`,
    dragToMove: (title: string) => `Drag ${title} to move`,
    ...
    actionsTitle: "Delete, duplicate, and more…",
    openActions: (title: string) => `Open page actions for ${title}`,
    addInsideTitle: "Add a page inside",
    addInside: (title: string) => `Add a page inside ${title}`,
    subpages: (title: string) => `${title} subpages`,
  },
  ko: {
    noPagesInside: "하위 페이지 없음",
    pageLocked: "페이지가 잠겨 있습니다",
    pageAccessRequired: "페이지 접근 권한이 필요합니다.",
    copiedBlocks: (count: number) => `블록 ${count}개를 페이지로 복사했습니다`,
    movedBlocks: (count: number) => `블록 ${count}개를 페이지로 이동했습니다`,
    ...
    lockedName: (title: string) => `${title} (잠김)`,
    dragToMove: (title: string) => `${title} 페이지를 드래그해 이동`,
    collapse: (title: string) => `${title} 접기`,
    expand: (title: string) => `${title} 펼치기`,
    rename: (title: string) => `${title} 이름 바꾸기`,
    locked: "잠김",
    lockedPage: "잠긴 페이지",
    actionsTitle: "삭제, 복제 및 기타 작업…",
    openActions: (title: string) => `${title} 페이지 작업 열기`,
    addInsideTitle: "하위 페이지 추가",
    addInside: (title: string) => `${title} 안에 페이지 추가`,
    subpages: (title: string) => `${title} 하위 페이지`,
  },
} as const;
...
function PageTreeItemComponent({
  ...
  const labels = pickLabels(PAGE_TREE_LABELS);
  ...
  async function createSubpage() {
    if (!canAddInside) {
      notify(page?.isLocked ? labels.pageLocked : labels.pageAccessRequired, "default");
      return;
    }
    if (page.isLocked) {
      notify(labels.pageLocked, "default");
      return;
    }
  ...
  function blockDropSuccessMessage(copy: boolean, count: number) {
    return copy ? labels.copiedBlocks(count) : labels.movedBlocks(count);
  }
  ...
  return (
    <>
      <div
        ...
        aria-label={page.isLocked ? labels.lockedName(pageTitle) : pageTitle}
        aria-level={depth + 1}
      ...
      >
        {!flat && (
          <span
            ...
            draggable={canDragThisPage}
            title={labels.dragToMove(pageTitle)}
            aria-hidden="true"
        ...
        <span className={styles.treeLeading} data-tree-leading="true">
          {canToggleDisclosure ? (
            <button
              ...
              onClick={toggle}
              tabIndex={-1}
              aria-label={expanded ? labels.collapse(pageTitle) : labels.expand(pageTitle)}
        ...
        {renaming ? (
          <input
            ...
            value={renameDraft}
            aria-label={labels.rename(pageTitle)}
            spellCheck={false}
        ...
        {page.isLocked && !renaming && (
          <span className={styles.treeLock} title={labels.locked} aria-label={labels.lockedPage}>
        ...
        <span className={styles.treeActions}>
          <button
            ...
            aria-label={labels.openActions(pageTitle)}
            onClick={(e) => {
              e.stopPropagation();
          ...
          {canAddInside && (
            <button
              type="button"
              className={styles.iconBtn}
              title={labels.addInsideTitle}
              aria-label={labels.addInside(pageTitle)}
              onClick={addSubpage}
            >
              <Plus size={16} />
            </button>
      ...
      {expanded && (hasChildren || showEmptyChildren) && (
        <div role="group" aria-label={labels.subpages(pageTitle)}>
          {hasChildren ? (
