...
import { LEGACY_CURRENT_PAGE_FILTER_KIND_READ_ONLY } from "../../../shared/legacy-product-compat";
...
describe("currentPageFilterValue", () => {
  it("round-trips through isCurrentPageFilterValue", () => {
    ...
    expect(
      isCurrentPageFilterValue({ kind: LEGACY_CURRENT_PAGE_FILTER_KIND_READ_ONLY })
    ).toBe(true);
...
describe("applyView filters", () => {
  ...
  it("filters rollup relation targets through the first-hop relation", () => {
    const contractRel = makeProp("샘플연결", "relation");
    const partnerRelId = "contract_partner_relation";
    const partnerRollup = makeProp("연결 - 샘플대상", "rollup", {
