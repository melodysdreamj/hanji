...
import { createIndexedDbOutboxAdapter } from "@edge-base/web";
...
describe("local lock gate", () => {
  ...
  it("a cross-tab mode change unblocks a pending gate and notifies subscribers", async () => {
    ...
    window.dispatchEvent(new StorageEvent("storage", { key: "notionlike.encryption.mode" }));
  ...
  it("enable refuses while queued changes exist, then succeeds after they drain", async () => {
    ...
    const adapter = createIndexedDbOutboxAdapter<OutboxOp>(`notionlike-outbox:${TEST_USER}`);
  ...
  it("serializes durable writes and the re-key under the one cross-tab lock", async () => {
    ...
    try {
      const lockName = `notionlike-outbox:${TEST_USER}`;
      ...
      // The synchronization lock is our own bare name, never the SDK's internal
      // per-tab / sweep liveness locks.
      expect(granted).not.toContain(`${lockName}::sweep`);
    } finally {
