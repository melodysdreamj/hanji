...
import { HANJI_BLOCKS_MIME } from "@/lib/clipboard";
...
describe("parseInlineMarkdown", () => {
  ...
  it("parses page/date/person mention hrefs", () => {
    ...
    expect(parseInlineMarkdown("[@today](hanji://date/2026-07-04)")).toEqual([
      { text: "@today", mention: "date", date: "2026-07-04" },
    ]);
    expect(parseInlineMarkdown("[@Ari](hanji://person/u1)")).toEqual([
      { text: "@Ari", mention: "person", userId: "u1" },
    ]);
  });
...
describe("localized generated paste labels", () => {
  it("keeps canonical Markdown commands while persisting Korean fallback labels", async () => {
    ...
    expect(text(blocks[1].content?.buttonTemplate?.[0] as PastedBlock)).toBe("새 할 일");
  });

  it("persists the active Korean untitled label for an empty pasted tab", async () => {
    await i18next.changeLanguage("ko");
    const [tab] = parsePastedHtml(
      '<section data-hanji-block-type="tab">' +
        '<section data-hanji-tab-label="true" data-hanji-tab-icon="📁">' +
          '<p data-hanji-tab-title="true"><span data-hanji-tab-icon-text="true">📁</span></p>' +
        '</section>' +
      '</section>',
    );
    expect(tab.children?.[0]).toMatchObject({ plainText: "제목 없음" });
    expect(text(tab.children?.[0])).toBe("제목 없음");
    expect(tab.plainText).toBe("탭");
  });
});

describe("parseInternalPastedBlocks", () => {
  const dataTransfer = (raw: string) =>
    ({ getData: (type: string) => (type === HANJI_BLOCKS_MIME ? raw : "") }) as DataTransfer;
