...
describe("cross-page block move", () => {
  it("moves the block subtree and pushes linked twin history entries on both pages", async () => {
    ...
    expect(vi.mocked(updateBlockRemote)).toHaveBeenCalledWith(
      "b1",
      expect.objectContaining({ pageId: "pb", parentId: null }),
      "pb"
    );
  ...
  it("undo from the TARGET page restores both pages without any remote delete or create", async () => {
    await movedFixtureBlocks();
    vi.mocked(updateBlockRemote).mockClear();
    ...
    expect(vi.mocked(updateBlockRemote)).toHaveBeenLastCalledWith(
      "b1",
      expect.objectContaining({ pageId: "pa", parentId: null }),
      "pa"
    );
