...
describe("UpdatesPanel", () => {
  it("renders a tablist with all five filters and All selected", async () => {
    seedActivities();
    await renderPanel();
    // The panel now has TWO tablists (the Updates/Chats mode toggle above the
    // filter row); scope to the filter tablist so the mode tabs stay out.
    const tabs = within(screen.getByRole("tablist", { name: "Update type" })).getAllByRole("tab");
