...
describe("spansToHtml", () => {
  ...
  it("renders person mentions", () => {
    expect(spansToHtml([{ text: "@June", mention: "person", userId: "u1" }])).toBe(
      '<span data-mention="person" data-user-id="u1" contenteditable="false">@June</span>'
    );
  });
...
describe("htmlToSpans", () => {
  ...
  it("reads mention elements", () => {
    ...
    expect(
      parse('<span data-mention="person" data-user-id="u1">@June</span>')
    ).toEqual([{ text: "@June", mention: "person", userId: "u1" }]);
  });
