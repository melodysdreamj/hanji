...
import { LEGACY_HANJI_URI_PREFIX } from "@/lib/legacyNamespace";
...
describe("pageIdFromPageHref", () => {
  ...
  it("extracts the id from the hanji:// scheme", () => {
    expect(pageIdFromPageHref("hanji://page/abc123")).toBe("abc123");
    expect(pageIdFromPageHref("HANJI://PAGE/abc123?x#y")).toBe("abc123");
  });

  it("reads the old page scheme without preserving it in output", () => {
    expect(pageIdFromPageHref(`${LEGACY_HANJI_URI_PREFIX}page/abc123`)).toBe("abc123");
    expect(remapPageHref(`${LEGACY_HANJI_URI_PREFIX}page/old`, new Map([["old", "new"]]))).toBe(
      "hanji://page/new"
    );
    expect(remapPageHref(`${LEGACY_HANJI_URI_PREFIX}page/old`)).toBe("hanji://page/old");
  });

  it("decodes percent-encoded ids", () => {
    expect(pageIdFromPageHref("/p/a%20b")).toBe("a b");
    expect(pageIdFromPageHref("hanji://page/a%2Fb")).toBe("a/b");
  });
...
describe("remapPageHref", () => {
  ...
  it("remaps hanji://page links", () => {
    expect(remapPageHref("hanji://page/old", map)).toBe(
      "hanji://page/new"
    );
  });
  ...
  it("remaps percent-encoded source ids", () => {
    expect(remapPageHref("hanji://page/%ED%95%9C%EA%B8%80", map)).toBe(
      "hanji://page/korean-id"
    );
  });
