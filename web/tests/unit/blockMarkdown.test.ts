...
describe("blockMarkdown", () => {
  ...
  it("serializes date and person mentions as hanji:// links", () => {
    expect(
      blockMarkdown(
        makeBlock("paragraph", {
    ...
    ).toMatch(/^\[@.*\]\(hanji:\/\/date\/2001-06-01\)$/);
    expect(
      blockMarkdown(
        makeBlock("paragraph", {
          rich: [{ text: "@Ari", mention: "person", userId: "u1" }],
        })
      )
    ).toBe("[@Ari](hanji://person/u1)");
  });
...
describe("blockTreeMarkdown", () => {
  ...
  it("uses the active catalog for an empty generated tab label", async () => {
    await i18next.changeLanguage("ko");
    const tab = makeBlock("tab", { rich: [] }, { id: "tab-ko" });
    const label = makeBlock("paragraph", { rich: [], icon: "📁" }, {
      id: "label-ko",
      parentId: "tab-ko",
      plainText: "",
    });
    expect(blockTreeMarkdown(tab, [tab, label])).toBe("[Tabs]\n  [Tab: 📁 제목 없음]");
    expect(blockTreeHtml(tab, [tab, label])).toContain("제목 없음");
  });
});
...
describe("blockTreeHtml", () => {
  ...
  it("renders tab labels as data-attributed sections", () => {
    ...
    expect(html).toContain('data-hanji-block-type="tab"');
    expect(html).toContain('data-hanji-tab-label="true"');
    expect(html).toContain('data-hanji-tab-icon="📁"');
  });
});
...
describe("blocksClipboardHtml", () => {
  it("wraps content in the copy fragment shell", () => {
    expect(blocksClipboardHtml("<p>x</p>")).toBe(
      '<html><body><!--StartFragment--><div data-hanji-copy="true"><p>x</p></div><!--EndFragment--></body></html>'
    );
  });
});
