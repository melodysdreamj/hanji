...
on the web build and `HANJI_ALLOW_DEV_GUEST_LOGIN` on the backend, and the
...
- Browser API and authentication requests are always same-origin. The Vite
  development server proxies `/api` and `/admin` to the local EdgeBase worker;
  public builds cannot embed a different API origin from shell or `.env` input.
