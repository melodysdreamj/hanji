...
//
// Starts a canned fake Notionlike backend (node:http, ephemeral port) inside
// this process, points the stdio MCP server at it via NOTIONLIKE_EDGEBASE_URL,
...
// Strip any real NOTIONLIKE_/EDGEBASE_ config (tokens, policy files,
// read-only flags) so the run is fully hermetic and deterministic.
const childEnv = Object.fromEntries(
  Object.entries(process.env).filter(
    ([key]) => !key.startsWith("NOTIONLIKE_") && !key.startsWith("EDGEBASE_"),
  ),
);
childEnv.NOTIONLIKE_EDGEBASE_URL = baseUrl;
...
const client = new Client(
  { name: "notionlike-mcp-tool-invocation-smoke", version: "0.1.0" },
  { capabilities: {} },
);
...
function assertMcpHeaders(req) {
  ...
  assert.equal(req.headers["x-notionlike-client-source"], "mcp", `${req.path} client source header`);
  assert.equal(req.headers["x-notionlike-mcp-client-id"], "notionlike-mcp", `${req.path} client id header`);
  assert.equal(req.headers["x-notionlike-mcp-read-only"], "false", `${req.path} read-only header`);
...
try {
  ...
  assert.equal(auditReq.body.client?.clientId, "notionlike-mcp");
