...
const baseUrl = process.env.NOTIONLIKE_EDGEBASE_URL ?? "http://127.0.0.1:8787";
const DEFAULT_TOOL_TIMEOUT_MS = positiveInteger(
  process.env.NOTIONLIKE_MCP_SMOKE_TOOL_TIMEOUT_MS,
  30_000,
);
...
async function assertBackendReachable() {
  const response = await fetch(`${baseUrl}/api/health`, {
    ...
    throw new Error(
      `Could not reach Notionlike EdgeBase backend at ${baseUrl}. ` +
...
function serverEnv(auth) {
  return {
    ...
    NOTIONLIKE_MCP_AUTH_MODE: "token",
    NOTIONLIKE_MCP_ACCESS_TOKEN: auth.accessToken,
    NOTIONLIKE_MCP_ALLOW_ANONYMOUS: "false",
  };
}
...
const client = new Client(
  { name: "notionlike-mcp-live-smoke", version: "0.1.0" },
  { capabilities: {} },
);
...
async function startNotificationSourceClient() {
  ...
  notificationSourceClient = new Client(
    { name: "notionlike-mcp-live-smoke-notification-source", version: "0.1.0" },
    { capabilities: {} },
  );
...
async function startEmailShareClient(email, password) {
  ...
  emailShareClient = new Client(
    { name: "notionlike-mcp-live-smoke-email-share", version: "0.1.0" },
    { capabilities: {} },
  );
...
async function startPolicyClient(envExtra = {}) {
  ...
  policyClient = new Client(
    { name: "notionlike-mcp-live-smoke-policy", version: "0.1.0" },
    { capabilities: {} },
  );
...
async function cleanupCreatedFileUploads() {
  ...
  for (const uploadId of ids) {
    await callTool("delete_file", { uploadId }, 12000).catch(() => {});
  }
...
try {
  ...
  await startPolicyClient({
    ...
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: workspaceId,
  });
  ...
  const policyFilePath = join(tmpdir(), `notionlike-mcp-policy-${smokeRunId}.json`);
    ...
    JSON.stringify(
      {
        ...
        issuer: "notionlike-live-smoke",
        audience: "notionlike-edgebase-local",
        transport: "stdio",
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: policyFilePath,
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: workspaceId,
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: "",
    NOTIONLIKE_MCP_ALLOWED_DATABASE_IDS: "",
    NOTIONLIKE_MCP_SCOPES: "",
    NOTIONLIKE_MCP_CLIENT_ID: "",
    NOTIONLIKE_MCP_CLIENT_NAME: "",
  });
  ...
  assertIncludes(filePolicy.text, "issuer: notionlike-live-smoke", "file get_mcp_access_policy issuer response");
  assertIncludes(filePolicy.text, "audience: notionlike-edgebase-local", "file get_mcp_access_policy audience response");
  ...
  const expiredPolicyFilePath = join(tmpdir(), `notionlike-mcp-expired-policy-${smokeRunId}.json`);
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: expiredPolicyFilePath,
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
  ...
  const futurePolicyFilePath = join(tmpdir(), `notionlike-mcp-future-policy-${smokeRunId}.json`);
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: futurePolicyFilePath,
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: "",
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: "",
  ...
  const servicePrincipalPolicyFilePath = join(tmpdir(), `notionlike-mcp-service-principal-policy-${smokeRunId}.json`);
    ...
    JSON.stringify(
      {
        ...
        issuer: "notionlike-live-provisioner",
        audience: "notionlike-edgebase-local",
        transport: "stdio",
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: servicePrincipalPolicyFilePath,
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: "",
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: "",
  ...
  assertIncludes(servicePrincipalAudit, "policy issuer: notionlike-live-provisioner", "service-principal MCP audit response");
  assertIncludes(servicePrincipalAudit, "policy audience: notionlike-edgebase-local", "service-principal MCP audit response");
  ...
  const organizationPolicyFilePath = join(tmpdir(), `notionlike-mcp-organization-policy-${smokeRunId}.json`);
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: organizationPolicyFilePath,
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: "",
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: "",
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_READ_ONLY: "true",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: createdWorkspaceId,
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: scopedPageId,
  });
  ...
  const pageBody =
    ...
    `Styled **bold** and *italic* with [link](https://example.com/mcp-rich) plus [today](notionlike://date/2026-06-25) and [Ada](notionlike://person/user-ada) as \`code\` and ~~struck~~\n\n` +
    `- created through MCP\n` +
    `- suffix ${suffix}`;
  ...
  assertIncludes(filteredMcpClientAudit, "client id: notionlike-mcp", "filtered MCP client audit response");
  ...
  const importedMarkdown = await callTool("import_markdown_page", {
    ...
    markdown:
      ...
      `Styled **bold** and *italic* with [link](https://example.com/mcp-import) on [today](notionlike://date/2026-06-25) by [Ada](notionlike://person/user-ada) as \`code\` and ~~struck~~\n\n` +
      `$$\n` +
      `x + y = z\n` +
      `$$\n\n` +
      `| Feature | Status |\n` +
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_POLICY_FILE: "",
    NOTIONLIKE_MCP_CONSENT_FILE: "",
    NOTIONLIKE_MCP_READ_ONLY: "",
    NOTIONLIKE_MCP_ALLOW_WRITES: "",
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: workspaceId,
    NOTIONLIKE_MCP_ALLOWED_PAGE_IDS: "",
    NOTIONLIKE_MCP_ALLOWED_DATABASE_IDS: "",
    NOTIONLIKE_MCP_SCOPES: "notion_import",
    NOTIONLIKE_MCP_CLIENT_ID: "",
    NOTIONLIKE_MCP_CLIENT_NAME: "",
  });
  try {
  ...
  const deletedFile = await callTool("delete_file", { uploadId });
  ...
  await startPolicyClient({
    NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS: workspaceId,
    NOTIONLIKE_MCP_ALLOWED_DATABASE_IDS: customDatabaseId,
  });
