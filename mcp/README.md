...
```bash
NOTIONLIKE_MCP_ACCESS_TOKEN=...
```

Accepted token environment variables are checked in this order:
`NOTIONLIKE_MCP_ACCESS_TOKEN`, `NOTIONLIKE_EDGEBASE_ACCESS_TOKEN`, then
`EDGEBASE_ACCESS_TOKEN`. Values may be raw tokens or `Bearer ...` strings.

Set `NOTIONLIKE_MCP_AUTH_MODE=token` to require a provided token and fail fast
...
NOTIONLIKE_MCP_ALLOWED_WORKSPACE_IDS=workspace-a,workspace-b
NOTIONLIKE_MCP_ALLOWED_PAGE_IDS=page-a,page-b
NOTIONLIKE_MCP_ALLOWED_DATABASE_IDS=database-a,database-b
```
...
```bash
NOTIONLIKE_MCP_POLICY_FILE=/secure/notionlike-mcp-policy.json
```
...
```json
{
  ...
  "issuer": "notionlike-admin",
  "audience": "notionlike-edgebase",
  "transport": "stdio",
...
`NOTIONLIKE_MCP_CONSENT_FILE` is accepted as an alias. Environment allowlists
...
or after it expires.
Use `NOTIONLIKE_MCP_SCOPES=pages,comments` to narrow scopes from the environment
...
`NOTIONLIKE_MCP_ALLOW_WRITES=false` is also accepted as a read-only alias. Use
...
- an EdgeBase bearer token in `NOTIONLIKE_MCP_ACCESS_TOKEN`
...
The equivalent environment variables are `NOTIONLIKE_MCP_SUBJECT_TYPE`,
`NOTIONLIKE_MCP_SUBJECT_ID` or `NOTIONLIKE_MCP_SERVICE_PRINCIPAL_ID`,
`NOTIONLIKE_MCP_POLICY_ISSUER`, `NOTIONLIKE_MCP_POLICY_AUDIENCE`,
`NOTIONLIKE_MCP_TRANSPORT`, and `NOTIONLIKE_MCP_PROVISIONING_ID`. The stdio
transport follows current MCP authorization guidance by reading credentials
from the environment. A future hosted HTTP transport still needs OAuth
protected-resource metadata, audience validation, and no token passthrough
before it should be treated as hosted MCP parity.
...
blocks. Date mentions use `notionlike://date/YYYY-MM-DD` links, and person
mentions use `notionlike://person/user-id` links.
...
cd mcp
npm install
NOTIONLIKE_EDGEBASE_URL=http://localhost:8787 node src/index.mjs
```
...
with `NOTIONLIKE_MCP_AUTH_MODE=token`, checks read-only/workspace/database
...
`NOTIONLIKE_EDGEBASE_URL` (default: `http://127.0.0.1:8787`).

## Register with Claude Code

```bash
claude mcp add notionlike -- node /path/to/notionlike/mcp/src/index.mjs
```
...
```json
{
  "mcpServers": {
    "notionlike": {
      "command": "node",
      "args": ["/path/to/notionlike/mcp/src/index.mjs"],
      "env": {
        "NOTIONLIKE_EDGEBASE_URL": "http://localhost:8787",
        "NOTIONLIKE_MCP_AUTH_MODE": "token",
        "NOTIONLIKE_MCP_ACCESS_TOKEN": "..."
      }
    }
