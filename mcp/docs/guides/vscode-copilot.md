...
Register the Hanji MCP server with VS Code so Copilot agent mode can use its
tools, via `.vscode/mcp.json`.
...
- VS Code 1.99+ with GitHub Copilot and agent mode enabled.

## Configuration

Create `.vscode/mcp.json` in your workspace. VS Code uses a top-level `servers`
key and supports `inputs` so the token is prompted for and stored securely
  ...
  "inputs": [
    {
      "type": "promptString",
      "id": "hanji-token",
      "description": "EdgeBase access token for Hanji",
      "password": true
    }
  ],
  "servers": {
    "hanji": {
      ...
      "env": {
        ...
        "HANJI_MCP_ACCESS_TOKEN": "${input:hanji-token}"
      }
    }
  }
}
...
Replace `/path/to/hanji` with the absolute path to this repository. Click
**Start** above the server entry (or run **MCP: List Servers**), then pick the
Hanji tools from the tools button in Copilot Chat agent mode.
...
- **Anonymous fallback refused**: with `HANJI_MCP_AUTH_MODE=token` or
  ...
  is configured and shows an error state in **MCP: List Servers**. Provide a
  token, or drop those flags for local anonymous bootstrap.
- **Token expired** (401/unauthorized errors on every tool call): EdgeBase
  tokens expire; copy a fresh token from a signed-in session and restart the
  server from **MCP: List Servers** — VS Code re-prompts for `inputs` values
  via **MCP: Reset cached inputs** if needed. Check output with
  **MCP: Show Output** for details.
