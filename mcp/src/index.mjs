#!/usr/bin/env node
// Notionlike MCP server — exposes the local workspace (pages, blocks, search)
  ...
  (uri) =>
    resourceText(
      ...
      [
        ...
        "",
        "Last reviewed: 2026-07-04 against MCP authorization 2025-11-25 and MCP tools 2025-06-18.",
        "",
        "- Transport: stdio. Credentials are read from environment variables, matching the MCP authorization guidance for local stdio servers.",
        ...
        "- Remote hosted MCP: a future HTTP transport must add OAuth protected-resource metadata, audience validation, and no token passthrough before it can be marked hosted parity.",
      ].join("\n")
    )
);
/** @type {[string, ...string[]]} */
...
function stripNotionlikeId(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return "";
...
function selectedWorkspaceId(input = {}) {
  ...
  return String(raw ?? "").trim() ? stripNotionlikeId(raw) : "";
}
...
function hanjiScopeMetadata({
  ...
  return {
    scope: {
      ...
      conflict,
      note:
        "Notion MCP is typically bound to one connected workspace. Hanji MCP is account-scoped, so workspace_id is required for workspace-bound tools. Notion-compatible teamspace_id is accepted as an Hanji workspace_id alias.",
    },
  };
}
...
async function deletePageTree(pageId) {
  ...
  const result = await eb.del("pages", pageId);
  return { page: root, count: result.deletedIds?.length ?? 1 };
}
...
function propertyConfigForInput(type, input = {}, databaseId) {
  ...
  if (type === "relation") {
    return withDisplayConfig({ relationDatabaseId: input.relationDatabaseId ?? databaseId });
  }
  if (type === "formula") {
...
async function findDatabaseView(viewKey, databaseId) {
  const cleanViewKey = stripNotionlikeId(viewKey);
  if (databaseId) {
    const cleanDatabaseId = stripNotionlikeId(databaseId);
...
const NOTION_SEARCH_INPUT_SCHEMA = {
  ...
  teamspace_id: z.string().optional().describe("Notion-compatible alias for workspace_id. In Hanji this must be an Hanji workspace id."),
...
async function handleNotionSearch({
  ...
  try {
    ...
    if (data_source_url) {
      const databaseId = stripNotionlikeId(data_source_url);
      const db = await eb.getOne("pages", databaseId);
    ...
    if (page_url) {
      const rootId = stripNotionlikeId(page_url);
...
export async function handleNotionFetch({ id, workspace_id, teamspace_id, include_discussions = false, include_transcript = false }) {
  try {
    ...
    const entityId = stripNotionlikeId(id);
  ...
  async ({ workspace_id, teamspace_id }) => {
    try {
      ...
      return okJson({
        ...
        message:
          "Hanji does not provide a separate Notion AI meeting-notes data source. Use normal page/database search or an Hanji database dedicated to meetings.",
      });
    } catch (e) {
      return fail(e);
    }
...
server.registerTool(
  "import_markdown_page",
  {
    ...
    description:
      "Import Markdown as a Notionlike page through the backend product API. Supports headings, lists, to-dos, quotes, code, and nested list indentation.",
    inputSchema: {
...
server.registerTool(
  "import_csv_database",
  {
    ...
    description:
      "Import CSV as a typed Notionlike database through the backend product API. The first row is used as headers and column types are inferred.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
...
server.registerTool(
  "begin_notion_oauth_connection",
  {
    ...
    description:
      "Create a signed Notion OAuth authorization URL for a Notionlike workspace. Open the URL in a browser, then complete with the returned code and state.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
...
server.registerTool(
  "create_notion_import_connection",
  {
    ...
    description:
      "Store an encrypted Notion API connection for a Notionlike workspace. Requires the backend NOTIONLIKE_NOTION_IMPORT_SECRET to be configured.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
...
server.registerTool(
  "list_notion_import_connections",
  {
    ...
    description: "List stored Notion API import connections for a Notionlike workspace without exposing credentials.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
...
server.registerTool(
  "create_notion_import_job",
  {
    ...
    description:
      "Create a Notion API import job through the Notionlike product API. When a Notion token is provided, the backend performs the first accessible workspace discovery pass without storing the token.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
      parentPageId: z.string().optional().describe("Optional Notionlike parent page id for imported content"),
      connectionKind: z.enum(NOTION_IMPORT_CONNECTION_KINDS).optional().describe("Notion connection kind"),
...
server.registerTool(
  "list_notion_import_jobs",
  {
    ...
    description: "List recent Notion API import jobs for a Notionlike workspace.",
    inputSchema: {
      workspaceId: z.string().optional().describe("Workspace id; defaults to the current workspace"),
...
server.registerTool(
  "apply_notion_import_job",
  {
    ...
    description:
      "Apply a ready Notion API import job into local Notionlike pages, canonical databases, views, rows, blocks, file uploads, and durable mappings.",
    inputSchema: {
      jobId: z.string().describe("Ready Notion import job id"),
    },
  },
...
server.registerTool(
  ...
  async ({ workspace_id, teamspace_id, title, description, parent, schema }) => {
    try {
      ...
      const parentId = parent?.page_id ? stripNotionlikeId(parent.page_id) : null;
      if (parentId) {
...
server.registerTool(
  "list_databases",
  {
    ...
    description: "List local Notionlike databases with ids and row counts.",
    inputSchema: {},
  },
  async () => {
    try {
...
function parseNotionSqlColumnType(typeSpec, name = "Name") {
  ...
  if (upper.startsWith("RELATION")) return { ...base, type: "relation", relationDatabaseId: stripNotionlikeId(firstQuoted(spec)) };
...
export async function queryDataSourceSql(data) {
  ...
  const databaseId = stripNotionlikeId(parsed.dataSourceUrl);
...
async function queryDataSourceView(data) {
  const requiredWorkspace = await requireWorkspaceSelection(data, "_notion_query_data_sources");
  if (requiredWorkspace.errorResult) return { __workspaceErrorResult: requiredWorkspace.errorResult };
  const viewId = stripNotionlikeId(data.view_url);
...
server.registerTool(
  ...
  async ({ workspace_id, teamspace_id, database_id, parent_page_id, data_source_id, name, type, configure }) => {
    try {
      ...
      const databaseId = stripNotionlikeId(data_source_id || database_id);
      const db = await eb.getOne("pages", databaseId);
      ...
      let parent = null;
      if (parent_page_id) {
        const parentPageId = stripNotionlikeId(parent_page_id);
...
server.registerTool(
  "delete_database_row_forever",
  {
    ...
    description:
      "Permanently delete a database row and clean its row-page subtree, including child pages, blocks, comments, collaboration logs, and files.",
    inputSchema: { rowId: z.string().describe("Database row page id") },
  },
  async ({ rowId }) => {
    try {
...
server.registerTool(
  ...
  async ({ workspace_id, teamspace_id, data_source_id, title, description, is_inline, in_trash, statements }) => {
    try {
      ...
      if (requiredWorkspace.errorResult) return requiredWorkspace.errorResult;
      const databaseId = stripNotionlikeId(data_source_id);
      const db = await eb.getOne("pages", databaseId);
      if (!db || db.kind !== "database") return ok(`Data source ${data_source_id} not found.`);
...
export async function handleNotionCreatePages({ workspace_id, teamspace_id, parent, pages }) {
  ...
  try {
    ...
    if (requiredWorkspace.errorResult) return requiredWorkspace.errorResult;
    const parentPageId = parent?.page_id ? stripNotionlikeId(parent.page_id) : null;
    const databaseId = parent?.data_source_id
      ? stripNotionlikeId(parent.data_source_id)
      : parent?.database_id
        ? stripNotionlikeId(parent.database_id)
        : null;
...
const NOTION_DUPLICATE_PAGE_TOOL = {
  ...
  inputSchema: {
    workspace_id: z.string().optional().describe("Required Hanji workspace id. Call list_workspaces or _notion_get_teams first and choose one; calls without it return a workspace selection error."),
    workspaceId: z.string().optional().describe("Notionlike workspace id alias for workspace_id"),
    teamspace_id: z.string().optional().describe("Notion-compatible alias for workspace_id. In Hanji this must be an Hanji workspace id."),
...
async function handleNotionDuplicatePage({ workspace_id, workspaceId, teamspace_id, pageId, page_id, title, parentId, parent_id, parentType, parent_type }) {
  try {
    ...
    if (requiredWorkspace.errorResult) return requiredWorkspace.errorResult;
    const sourcePageId = stripNotionlikeId(pageId ?? page_id);
    ...
    const destinationParentId = parentId ? stripNotionlikeId(parentId) : parent_id ? stripNotionlikeId(parent_id) : undefined;
...
server.registerTool(
  "prepare_file_upload",
  {
    ...
    description:
      "Create a backend-validated EdgeBase file upload grant for a workspace, page, block, database, or database property target.",
    inputSchema: {
      workspaceId: z.string().optional(),
  ...
  async ({ workspaceId, pageId, blockId, databaseId, propertyId, scope, name, size, contentType }) => {
    try {
...
server.registerTool(
  ...
  async ({ workspaceId, pageId, blockId, databaseId, propertyId, scope, status, includeDeleted }) => {
    try {
...
server.registerTool(
  "delete_file",
  {
    title: "Delete file",
    description:
      "Delete an EdgeBase-backed stored file by upload id or storage key. The backend verifies workspace access and marks the upload record deleted.",
    inputSchema: {
  ...
  async ({ uploadId, key }) => {
    try {
      if (!uploadId && !key) throw new Error("Provide uploadId or key.");
      const file = await eb.deleteFile({ uploadId, key });
...
server.registerTool(
  ...
  async ({ workspaceId, limit, dryRun }) => {
    try {
      const result = await eb.cleanupExpiredFiles({ workspaceId, limit, dryRun });
...
server.registerTool(
  ...
  async ({ workspaceId, organizationId, maintenanceLimit }) => {
    try {
      ...
      const report = await eb.fileReport({ workspaceId, organizationId, maintenanceLimit });
...
server.registerTool(
  "list_notifications",
  {
    ...
    description:
      "List the current user's persisted Notionlike notification inbox through the backend notification API.",
    inputSchema: {
      workspaceId: z.string().optional(),
...
server.registerTool(
  ...
  async ({ uploadId, key, expiresIn }) => {
    try {
      if (!uploadId && !key) throw new Error("Provide uploadId or key.");
      const result = await eb.fileDownloadUrl({ uploadId, key, expiresIn });
      return ok(
...
async function handleNotionUpdatePage({
  ...
  try {
    ...
    if (requiredWorkspace.errorResult) return requiredWorkspace.errorResult;
    const targetPageId = stripNotionlikeId(pageId ?? page_id);
  ...
  async ({ pageId, page_id, parentId, parent_id, parentType, parent_type, afterPageId, after_page_id, beforePageId, before_page_id }) => {
    try {
      ...
      const result = await movePage(targetPageId, {
        parentId: parentId ? stripNotionlikeId(parentId) : parent_id ? stripNotionlikeId(parent_id) : undefined,
        parentType: parentType ?? parent_type,
        afterPageId: afterPageId ? stripNotionlikeId(afterPageId) : after_page_id ? stripNotionlikeId(after_page_id) : undefined,
        beforePageId: beforePageId ? stripNotionlikeId(beforePageId) : before_page_id ? stripNotionlikeId(before_page_id) : undefined,
      });
...
async function handleNotionMovePages({ workspace_id, teamspace_id, page_or_database_ids, page_ids, page_id, new_parent, parent, after_page_id, before_page_id }) {
  try {
    ...
    const parentId = databaseId
      ? stripNotionlikeId(databaseId)
      : parentPageId
        ? stripNotionlikeId(parentPageId)
        : null;
    ...
    for (const id of ids) {
      ...
      const result = await movePage(id, {
        parentId,
        parentType,
        afterPageId: after_page_id ? stripNotionlikeId(after_page_id) : undefined,
        beforePageId: before_page_id ? stripNotionlikeId(before_page_id) : undefined,
      });
      if (!result) {
        notFound.push(id);
        continue;
...
async function handleNotionGetComments({ workspace_id, teamspace_id, page_id, include_resolved = false, discussion_id }) {
  try {
    ...
    if (requiredWorkspace.errorResult) return requiredWorkspace.errorResult;
    const pageId = stripNotionlikeId(page_id);
    const page = await eb.getOne("pages", pageId);
...
server.registerTool(
  "delete_page_forever",
  {
    ...
    description:
      "Permanently delete a page subtree. Intended for pages already in trash; this cannot be undone.",
    inputSchema: { pageId: z.string() },
  },
  async ({ pageId }) => {
    try {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.error("notionlike-mcp ready (stdio)");
}
