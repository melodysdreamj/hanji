...
// Talks to the local Hanji backend's REST API (validated endpoints).
...
} from "./legacy-product-compat.mjs";

const BASE = hanjiEnv("HANJI_EDGEBASE_URL") || "http://localhost:8787";
...
function configuredPolicyFilePath() {
  return (
    hanjiEnv("HANJI_MCP_POLICY_FILE") ??
    hanjiEnv("HANJI_MCP_CONSENT_FILE") ??
    ""
  ).trim();
}
...
function configuredMcpPolicy() {
  ...
  const envWorkspaceIds = envCsvSet("HANJI_MCP_ALLOWED_WORKSPACE_IDS");
  const envPageIds = envCsvSet("HANJI_MCP_ALLOWED_PAGE_IDS");
  const envDatabaseIds = envCsvSet("HANJI_MCP_ALLOWED_DATABASE_IDS");
  const fileScopes = policyStringSet(file.data.scopes ?? file.data.allowedScopes ?? file.data.allowed_scopes);
  const envScopes = envCsvSet("HANJI_MCP_SCOPES");
  const subjectType = policySubjectType(
    hanjiEnv("HANJI_MCP_SUBJECT_TYPE") ??
      file.data.subjectType ??
      file.data.subject_type,
    file.path,
  );
  const readOnly = envFlag("HANJI_MCP_READ_ONLY", false) ||
    envFlag("HANJI_MCP_ALLOW_WRITES", true) === false ||
    fileReadOnly;
  return {
    readOnly,
    ...
    subjectId: optionalPolicyString(
      hanjiEnv("HANJI_MCP_SUBJECT_ID"),
      hanjiEnv("HANJI_MCP_SERVICE_PRINCIPAL_ID"),
      file.data.subjectId,
    ...
    issuer: optionalPolicyString(
      hanjiEnv("HANJI_MCP_POLICY_ISSUER"),
      file.data.issuer,
      file.data.iss,
    ),
    audience: optionalPolicyString(
      hanjiEnv("HANJI_MCP_POLICY_AUDIENCE"),
      hanjiEnv("HANJI_MCP_RESOURCE"),
      file.data.audience,
      file.data.aud,
      file.data.resource,
    ),
    transport: optionalPolicyString(
      hanjiEnv("HANJI_MCP_TRANSPORT"),
      file.data.transport,
    ) ?? "stdio",
    provisioningId: optionalPolicyString(
      hanjiEnv("HANJI_MCP_PROVISIONING_ID"),
...
function mcpClientMetadata(policy = configuredMcpPolicy()) {
  const envClientId = hanjiEnv("HANJI_MCP_CLIENT_ID")?.trim();
  const envClientName = hanjiEnv("HANJI_MCP_CLIENT_NAME")?.trim();
  const clientId = envClientId || policy.clientId?.trim() || "hanji-mcp";
  const clientName = envClientName || policy.clientName?.trim() || "Hanji MCP";
  return {
    source: "mcp",
    clientId,
    clientName,
...
function envToken() {
  const raw =
    hanjiEnv("HANJI_MCP_ACCESS_TOKEN") ??
    hanjiEnv("HANJI_EDGEBASE_ACCESS_TOKEN") ??
...
function mcpAuthMode() {
  const raw = (hanjiEnv("HANJI_MCP_AUTH_MODE") ?? "auto").trim().toLowerCase();
  if (raw === "token" || raw === "anonymous" || raw === "auto") return raw;
  throw new Error("HANJI_MCP_AUTH_MODE must be auto, token, or anonymous.");
}

function anonymousFallbackAllowed() {
  return hanjiEnv("HANJI_MCP_ALLOW_ANONYMOUS") !== "false";
}

function retryLimit() {
  const raw = Number(hanjiEnv("HANJI_MCP_HTTP_RETRIES") ?? 6);
  ...
  if (authMode === "token") {
    throw new Error(
      "MCP token auth is enabled but no access token was provided. " +
        "Set HANJI_MCP_ACCESS_TOKEN to a Hanji EdgeBase bearer token.",
    );
  }
  if (!anonymousFallbackAllowed()) {
    throw new Error(
      "MCP anonymous fallback is disabled. Set HANJI_MCP_ACCESS_TOKEN " +
...
async function api(path, { method = "GET", body, query, audit = true } = {}) {
  ...
  const requestInit = (accessToken) => {
    const headers = {
      ...
      "X-Hanji-Client-Source": client.source,
      "X-Hanji-MCP-Client-ID": client.clientId,
      "X-Hanji-MCP-Client-Name": client.clientName,
      "X-Hanji-MCP-Read-Only": client.readOnly ? "true" : "false",
    };
    optionalClientHeader(headers, "X-Hanji-MCP-Subject-Type", client.subjectType);
    optionalClientHeader(headers, "X-Hanji-MCP-Subject-ID", client.subjectId);
    optionalClientHeader(headers, "X-Hanji-MCP-Policy-Issuer", client.issuer);
    optionalClientHeader(headers, "X-Hanji-MCP-Policy-Audience", client.audience);
    optionalClientHeader(headers, "X-Hanji-MCP-Transport", client.transport);
    optionalClientHeader(headers, "X-Hanji-MCP-Provisioning-ID", client.provisioningId);
    return {
      method,
      headers,
    ...
    if (tokenIsStatic) {
      ...
      throw new Error(
        "EdgeBase rejected the configured MCP access token (HTTP 401) — the token has likely expired. " +
          "Provide a fresh token via HANJI_MCP_ACCESS_TOKEN.",
      );
    }
...
function rawTableUnsupported(operation, table) {
  throw new Error(
    `MCP ${operation} for table "${table}" is not routed through a Hanji backend function yet.`,
  );
}
...
export const eb = {
  ...
  async deleteDatabaseRow(rowId, opts = {}) {
    return api("/functions/database-row-mutation", {
      method: "POST",
      body: {
        action: "delete",
  ...
  async completeFileUpload(opts = {}) {
    const res = await api("/functions/file-mutation", {
      method: "POST",
      body: { action: "completeUpload", ...opts },
    });
...
function pageIdFromHref(href) {
  ...
  if (!raw) return "";
  const uriPageId = hanjiUriPayload(raw, "page");
  if (uriPageId) {
    try {
      return safeMentionId(decodeURIComponent(uriPageId));
    } catch {
      return safeMentionId(uriPageId);
    }
  }
  let path = raw;
  try {
...
function dateFromHref(href) {
  const date = hanjiUriPayload(href, "date");
  if (!date) return "";
  try {
...
function personIdFromHref(href) {
  const userId = hanjiUriPayload(href, "person");
  if (!userId) return "";
  try {
...
function spansToMarkdown(rich) {
  ...
  return rich
    .map((span) => {
      ...
      if (date) {
        return `${leading}[${body}](${markdownHref(hanjiUri("date", encodeURIComponent(date)))})${trailing}`;
      }
      const userId = span.mention === "person" ? safeMentionId(span.userId) : "";
      if (userId) {
        return `${leading}[${body}](${markdownHref(hanjiUri("person", encodeURIComponent(userId)))})${trailing}`;
      }
...
export function blockToMarkdown(b) {
  ...
  switch (b.type) {
    ...
    case "synced_block": {
      ...
      return id ? `[Synced block](${hanjiUri("block", id)})` : "[Synced block]";
    }
    case "button": {
...
export function markdownToBlocks(md) {
  ...
  while (i < lines.length) {
    ...
    } else if (matchHanjiPageLink(trimmed)) {
      const match = matchHanjiPageLink(trimmed);
      const label = match?.[1]?.trim() || "Link to page";
      const id = match?.[2] ?? "";
    ...
    } else if (matchHanjiSyncedBlock(trimmed)) {
      const id = matchHanjiSyncedBlock(trimmed)?.[1];
      emit({
        type: "synced_block",
