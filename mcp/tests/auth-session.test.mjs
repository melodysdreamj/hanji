...
describe("EdgeBase session lifecycle", () => {
  ...
  beforeEach(() => {
    ...
    for (const key of Object.keys(process.env)) {
      if (key.startsWith("NOTIONLIKE_") || key.startsWith("EDGEBASE_")) delete process.env[key];
    }
    process.env.NOTIONLIKE_EDGEBASE_URL = BASE;
    process.env.NOTIONLIKE_MCP_HTTP_RETRIES = "0";
    console.error = () => {};
  });
  ...
  it("fails 401s in static-token mode with fresh-token guidance and no retry", async () => {
    process.env.NOTIONLIKE_MCP_ACCESS_TOKEN = "static-token";
    const { eb } = await freshEdgeBase();
    const calls = installFetch((call) => {
    ...
    await assert.rejects(
      ...
      (error) => {
        ...
        assert.match(message, /NOTIONLIKE_MCP_ACCESS_TOKEN/);
